...
TL;DR: 提出NIRNL框架，通过跨模态边界保持和邻居感知实例精炼，解决噪声标签下的跨模态检索问题，在三个基准数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 跨模态检索中多模态数据标注存在噪声，现有鲁棒方法难以同时满足模型性能上限、校准可靠性和数据利用率。需要开发能充分利用所有可用数据同时减轻错误传播的新方法。

Method: 提出NIRNL框架：1) 跨模态边界保持(CMP)调整正负样本对的相对距离，增强样本对区分度；2) 邻居感知实例精炼(NIR)通过跨模态邻居共识识别纯子集、硬子集和噪声子集；3) 针对细粒度划分构建定制优化策略。

Result: 在三个基准数据集上的大量实验表明，NIRNL实现了最先进的性能，表现出显著的鲁棒性，特别是在高噪声率下。

Conclusion: NIRNL框架通过跨模态边界保持和邻居感知实例精炼，有效处理噪声标签问题，在保持模型性能的同时最大化数据利用率，为噪声标签下的跨模态检索提供了有效解决方案。
...
Result: 在多种任务上的实验表明该方法具有明显优势，有效提升了时间定位准确性和文本响应可靠性

Conclusion: 通过解耦学习和因子化偏好优化，D²VLM框架能够更好地处理视频理解中的时间定位和文本响应任务，为事件级视频理解提供了有效解决方案
...
TL;DR: 提出空间提升全景拼接框架，将2D图像提升为3D点云表示，通过全局融合和等距圆柱投影生成几何一致的360°全景图，有效减少视差和遮挡导致的失真


<details>
  <summary>Details</summary>
Motivation: 传统2D单应性变换和网格变形方法在处理真实3D场景（多深度层、遮挡）时会产生重影、结构弯曲和拉伸失真，特别是在多视角累积和360°闭环拼接场景中问题更为严重

Method: 1) 将输入图像提升为统一坐标系下的密集3D点表示，进行带置信度指标的全局跨视图融合；2) 在3D空间建立统一投影中心，采用等距圆柱投影将融合数据映射到全景流形；3) 在画布域进行空洞填充以处理视点转换揭示的未知区域

Result: 实验评估表明，该方法在显著视差和复杂遮挡场景中大幅减少了几何失真和重影伪影，生成了更自然、一致的全景结果

Conclusion: 该框架将拼接从2D变形范式重新概念化为3D一致性范式，能够灵活集成各种3D提升和补全模块，为处理真实3D场景的全景拼接提供了有效解决方案
...
TL;DR: HaineiFRDM是一个基于扩散模型的电影修复框架，通过分块训练策略在单张24GB显存GPU上处理高分辨率影片，采用全局提示、帧融合和全局-局部频率模块等技术提升修复质量，并构建了包含真实修复影片和合成数据的数据集。


<details>
  <summary>Details</summary>
Motivation: 现有开源电影修复方法性能有限，主要因为使用低质量合成数据训练和噪声光流，且未探索高分辨率影片。需要利用扩散模型强大的内容理解能力帮助专家修复难以识别的影片缺陷。

Method: 1) 采用分块训练和测试策略，使单张24GB显存GPU能处理高分辨率影片；2) 设计位置感知的全局提示和帧融合模块；3) 引入全局-局部频率模块重建不同分块间的一致性纹理；4) 先恢复低分辨率结果作为全局残差，减轻分块过程产生的块状伪影；5) 构建包含真实修复影片和逼真合成数据的电影修复数据集。

Result: 综合实验结果表明，该模型在缺陷修复能力上优于现有开源方法，代码和数据集将公开发布。

Conclusion: HaineiFRDM成功探索了扩散模型在电影修复中的应用，通过创新的分块策略和模块设计，实现了在有限硬件条件下对高分辨率影片的有效修复，为开源电影修复提供了新的解决方案。
...
TL;DR: 提出一种基于Depth Anything V2架构的单目深度估计方法，通过动态向量低秩适应（DV-LORA）将合成先验高效迁移到医学领域，在具有挑战性的内窥镜手术环境中实现最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 传统自监督单目深度估计方法在内窥镜手术环境中表现脆弱，特别是在高反光、充满液体的场景中容易在薄手术器械和透明表面上出现边界崩溃问题。现有方法通常依赖带有噪声真实世界伪标签的基础模型，难以准确捕捉精细几何结构。

Method: 1. 利用Depth Anything V2架构的高保真合成先验，该架构能固有地捕捉薄结构的精确几何细节；2. 采用动态向量低秩适应（DV-LORA）高效地将这些先验适应到医学领域，最小化参数预算同时弥合合成到真实的差距；3. 在SCARED数据集上引入物理分层评估协议，严格量化高反光区域性能。

Result: 在SCARED数据集上达到新的最先进水平：准确度（<1.25）达到98.1%，与现有基线相比，平方相对误差降低超过17%。在不利手术光照条件下表现出卓越的鲁棒性。

Conclusion: 通过结合Depth Anything V2的高保真合成先验和DV-LORA的高效领域适应，成功解决了内窥镜手术环境中单目深度估计的挑战，特别是在高反光区域和薄结构上的性能显著提升，为机器人手术提供了更可靠的深度感知能力。
...
TL;DR: 提出基于视频的自动评估管道，利用计算机视觉从训练视频中提取骨骼、视线和轨迹数据，开发特定任务指标来衡量心理运动流畅性、情境意识和团队协调，并通过扩展的认知任务分析层次结构生成整体性能评分。


<details>
  <summary>Details</summary>
Motivation: 城市作战训练需要情境意识和肌肉记忆，但传统评估方法依赖昂贵传感器或主观观察，限制了可扩展性和准确性。需要自动化、客观的性能评估系统来支持合成训练环境中的大规模训练。

Method: 开发视频评估管道，使用计算机视觉模型从训练视频提取2D骨骼、视线向量和运动轨迹。基于这些数据开发任务特定指标，衡量心理运动流畅性、情境意识和团队协调。通过扩展的认知任务分析层次结构，采用加权组合生成团队合作和认知的整体性能评分。

Result: 通过真实世界"进入并清理房间"训练案例研究，展示了该方法能够提供可操作的领域特定指标，捕捉个人和团队表现。这些见解可以支持行动后审查，通过交互式仪表板在Gamemaster和GIFT框架中提供直观反馈。

Conclusion: 视频分析为合成训练环境中的可扩展评估提供了可行途径，但仍面临跟踪困难、地面真实验证和更广泛适用性等限制。未来工作包括扩展到3D视频数据，并利用视频分析实现STE中的大规模评估。
...
TL;DR: 该论文提出了一种终身域自适应3D人体姿态估计的新任务，通过创新的GAN框架解决非平稳目标域数据集的挑战，有效缓解领域偏移和灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体姿态估计方法依赖受控环境标注数据，难以泛化到多样化真实场景。传统域自适应方法（如通用DA和源自由DA）忽略了非平稳目标姿态数据集的问题，需要解决连续适应新领域时的知识遗忘挑战。

Method: 提出终身域自适应3D HPE任务框架，包含3D姿态生成器、2D姿态判别器和3D姿态估计器组成的GAN架构。创新性地构建了集成姿态感知、时序感知和领域感知知识的3D姿态生成器范式，以增强当前领域适应并缓解对先前领域的灾难性遗忘。

Result: 在多样化域自适应3D HPE数据集上进行广泛实验，证明了该方法在适应当前领域姿态和保留先前领域知识方面的优越性能。

Conclusion: 该研究首次将终身域自适应引入3D人体姿态估计任务，提出的GAN框架能有效应对领域偏移和对齐原始与增强姿态，为解决非平稳目标数据集和灾难性遗忘问题提供了创新解决方案。
...
TL;DR: 提出一种基于深度学习的管道，将0.2-2岁儿童的T1加权MRI转换为合成CT，实现颅骨分割、缝合线概率热图生成和直接缝合线分割，填补MRI在颅骨评估中的关键空白。


<details>
  <summary>Details</summary>
Motivation: 量化儿童颅骨发育和缝合线骨化对诊断和治疗颅骨生长障碍至关重要。CT虽能评估颅骨和缝合线畸形，但具有电离辐射风险；MRI无辐射但无法清晰显示颅骨缝合线、估计骨密度或评估颅骨生长。需要一种非侵入性方法来克服这些限制。

Method: 提出深度学习驱动的管道，将T1加权MRI转换为合成CT。使用特定领域的变分自编码器，预测详细的颅骨分割，生成缝合线概率热图，并从热图推导直接缝合线分割。使用内部儿科数据进行训练和验证。

Result: 合成CT与真实CT的结构相似度达99%，Frechet inception距离为1.01。七个颅骨的平均Dice系数为85%，缝合线的Dice系数为80%。通过TOST检验证实合成CT与真实CT在颅骨和缝合线分割上的等效性（p < 0.05）。

Conclusion: 这是首个能从MRI衍生的合成CT实现缝合线分割的儿科颅骨CT合成框架。该方法生成与真实CT感知上无法区分的合成CT，填补了非侵入性颅骨评估的关键空白，为无辐射的儿科颅骨发育监测提供了新途径。
...
TL;DR: 提出MGML框架解决MRI多模态数据缺失下的脑肿瘤分割问题，通过元参数化自适应模态融合和一致性正则化模块，在不改变原模型架构的情况下提升分割性能。
...
Result: 在BraTS2020和BraTS2023数据集上进行了广泛实验。在BraTS2020上，针对15种缺失模态组合的平均Dice分数：全肿瘤87.55、肿瘤核心79.36、增强肿瘤62.67，优于多种最先进方法。

Conclusion: MGML框架能有效处理多模态MRI数据缺失问题，在不改变原模型架构的情况下显著提升脑肿瘤分割性能，具有实际临床应用价值。
...
TL;DR: 提出基于AI的微血管吻合术自动动作分割与性能评估框架，实现92.4%帧级分割准确率和85.5%技能分类准确率


<details>
  <summary>Details</summary>
Motivation: 神经外科微血管吻合术需要精细操作技能，传统评估依赖专家主观评分，存在评分者间变异、不一致性和时间成本高等问题，需要自动化、可扩展的客观评估方案
...
Conclusion: 该方法为微外科教育提供客观、实时反馈，可实现标准化、数据驱动的训练协议，推进高风险手术环境中的能力评估
...
TL;DR: T2VAttack：针对文本到视频扩散模型的对抗攻击研究，从语义和时间两个角度评估模型脆弱性，通过替换或插入单词实现攻击
...
Method: 提出T2VAttack框架，包含两种攻击方法：T2VAttack-S通过贪婪搜索识别语义或时间关键词并用同义词替换；T2VAttack-I通过迭代插入优化后的单词实现最小扰动攻击。从语义目标（评估视频-文本对齐）和时间目标（评估时间动态）两个维度设计攻击。

Result: 在多个最先进的T2V模型（ModelScope、CogVideoX、Open-Sora、HunyuanVideo）上进行评估，实验表明即使单个单词的替换或插入也能导致语义保真度和时间动态的显著退化，揭示了当前T2V扩散模型的关键脆弱性。

Conclusion: 当前文本到视频扩散模型对对抗攻击高度脆弱，即使是微小的提示修改也会严重影响生成质量。这凸显了开发更鲁棒的T2V模型的重要性，并为未来的防御机制研究提供了基础。
...
TL;DR: 提出一种仅需图像和可选相机位姿的自驾驶场景视点外推方法，通过4D高斯模型与视频扩散模型的迭代优化，无需昂贵传感器或标注


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的视点外推方法严重依赖LiDAR点云、3D边界框和车道标注等先验信息，需要昂贵传感器或人工标注，限制了实际部署应用

Method: 1) 仅使用图像和可选相机位姿估计全局静态点云和逐帧动态点云，融合为统一表示；2) 采用可变形4D高斯框架重建场景；3) 初始训练的4D高斯模型生成退化伪图像训练视频扩散模型；4) 通过扩散模型迭代优化逐步偏移的高斯渲染结果，并将增强结果反馈训练4DGS；5) 重复此过程直至外推到目标视点

Result: 与基线方法相比，该方法在新外推视点上生成更高质量的图像

Conclusion: 提出了一种无需昂贵传感器或标注的自驾驶场景视点外推解决方案，通过4D高斯模型与视频扩散模型的协同迭代优化，实现了高质量的新视点图像生成
...
TL;DR: 基于SATLAS基础模型的深度学习方法，通过时间序列预测和重构来检测卫星图像中的地表变化，相比传统方法灵敏度提高3倍


<details>
  <summary>Details</summary>
Motivation: 卫星图像地表变化检测对灾害响应和环境监测至关重要，但受大气噪声、季节变化和传感器伪影等复杂因素影响，传统方法检测能力有限

Method: 使用基于SATLAS基础模型的修复模型，利用Sentinel-2时间序列数据，通过前序观测预测最后一帧图像，训练数据覆盖全球不同气候区和土地覆盖类型

Result: 在2023年土耳其-叙利亚地震序列的地表破裂检测中，该方法比时间中值或Reed-Xiaoli异常检测器具有更高的灵敏度和特异性，检测阈值比基线方法低约3倍
...
TL;DR: 提出GCA-ResUNet，一种用于医学图像分割的高效框架，通过轻量级分组坐标注意力模块增强全局表示能力，在准确性和计算效率间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: 现有U-Net架构存在局部性限制，难以建模长距离上下文依赖，尤其在多器官和低对比度区域；Transformer架构虽能解决此问题但计算资源需求高，不利于临床部署。需要一种既保持CNN效率又能增强全局建模能力的方法。

Method: 提出GCA-ResUNet框架，核心是分组坐标注意力模块：1) 将通道上下文建模解耦为多个组以处理语义异质性；2) 集成方向感知坐标编码，沿水平和垂直轴捕获结构化空间依赖。该模块轻量且即插即用，增强全局表示同时保持CNN骨干效率。

Result: 在Synapse和ACDC基准测试中，GCA-ResUNet分别达到86.11%和92.64%的Dice分数，优于Swin-UNet、TransUNet等代表性CNN和Transformer方法。特别在复杂边界的小解剖结构分割上表现一致改进。

Conclusion: GCA-ResUNet在分割准确性和计算效率间提供了有利权衡，为临床部署提供了实用且可扩展的解决方案。分组坐标注意力模块有效增强了全局表示能力，同时保持了CNN的效率优势。
...
TL;DR: GASeg：通过可微盒计数模块和拓扑增强，将几何拓扑信息与外观特征结合，解决自监督语义分割中的外观歧义问题，在多个基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 自监督语义分割方法在面对外观歧义（如阴影、眩光、局部纹理）时经常失败，原因是过度依赖不稳定的外观特征。需要引入稳定的拓扑信息来弥补这一缺陷。

Method: 提出GASeg框架，核心是可微盒计数（DBC）模块，从几何特征流和外观特征流中量化多尺度拓扑统计。采用拓扑增强（TopoAug）对抗策略，通过形态学操作模拟真实世界歧义。使用GALoss多目标损失函数强制跨模态对齐。

Result: 在COCO-Stuff、Cityscapes、PASCAL等四个基准测试上实现了最先进的性能，验证了通过拓扑信息桥接几何和外观方法的有效性。

Conclusion: 通过引入稳定的拓扑信息来桥接几何和外观特征，能有效解决自监督语义分割中的外观歧义问题，GASeg框架为此提供了有效的解决方案。
...
TL;DR: 提出了一种利用太阳位置先验知识改进3D高斯泼溅模型在太空RPO场景下光照适应性的新方法，用于从图像序列重建未知航天器的3D结构。


<details>
  <summary>Details</summary>
Motivation: 在太空交会对接操作中，从图像序列重建未知目标航天器的3D结构对导航和操作至关重要。传统3D高斯泼溅方法假设静态光照条件，但太空环境存在动态照明变化，这会影响模型的几何精度和下游姿态估计任务的性能。

Method: 提出了一种新颖的管道，将服务航天器估计和维持的太阳位置先验知识整合到3D高斯泼溅模型的训练过程中。通过利用太阳位置信息，改进3DGS渲染的光度质量，使模型能够适应太空中的快速变化光照条件，并反映全局阴影和自遮挡效应。

Result: 实验研究表明，所提出的解决方案有效，在图像序列上训练的3DGS模型能够学习适应太空中的快速变化光照条件，并准确反映全局阴影和自遮挡现象，提高了光度渲染质量。

Conclusion: 通过整合太阳位置先验知识，成功改进了3D高斯泼溅模型在动态太空光照条件下的性能，为RPO过程中的下游姿态估计任务提供了更准确的光度渲染基础，实现了未知目标航天器的几何精确3D重建。
...
TL;DR: CVC提出条件速度校正框架，通过双视角速度转换机制和基于经验贝叶斯推断的后验一致更新，解决流式扩散编辑中的速度误差累积问题，实现稳定可解释的隐空间动态和忠实编辑。


<details>
  <summary>Details</summary>
Motivation: 现有流式扩散编辑方法在源图像和目标图像分布间直接转换时，隐空间轨迹存在累积速度误差，导致语义不一致和结构保真度损失。需要解决速度误差量化偏差和轨迹漂移问题。

Method: 提出条件速度校正(CVC)框架，将流式编辑重新表述为已知源先验驱动的分布转换问题。引入双视角速度转换机制：结构保持分支保持源轨迹一致性，语义引导分支驱动向目标分布的可控偏离。应用基于经验贝叶斯推断和Tweedie校正的后验一致更新来补偿量化偏差。

Result: CVC产生稳定可解释的隐空间动态，实现忠实重建和平滑局部语义转换。综合实验表明，CVC在多种任务中始终实现更优的保真度、更好的语义对齐和更可靠的编辑行为。

Conclusion: CVC通过条件速度校正框架解决了流式扩散编辑中的速度误差累积问题，通过双视角速度转换和后验一致更新机制，实现了稳定、忠实且语义对齐的图像编辑效果。
...
TL;DR: FitControler是一个可学习的插件，可集成到现代虚拟试穿模型中，实现定制化的服装合身度控制，解决了现有方法忽视服装合身度这一关键风格因素的问题。


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法主要关注服装细节的忠实渲染，但忽视了塑造整体风格的关键因素——服装合身度。合身度定义了服装如何与穿着者身体对齐，是时尚设计的基本要素，对整体风格协调至关重要。

Method: 提出FitControler插件，包含两个核心组件：1) 合身度感知布局生成器，基于精心处理的服装无关表示重新绘制身体-服装布局；2) 多尺度合身度注入器，传递布局线索实现布局驱动的虚拟试穿。同时构建了Fit4Men数据集，包含13,000个不同合身度的身体-服装对。

Result: FitControler可与多种虚拟试穿模型兼容，实现精确的合身度控制。实验表明该方法能有效控制服装合身度，并引入了两种合身度一致性指标来评估生成效果。

Conclusion: 该工作首次将合身度控制引入虚拟试穿领域，提出的FitControler插件能够无缝集成到现有模型中，实现定制化的合身度控制，为虚拟试穿系统增加了重要的风格协调维度。
...
Method: 1. 结构引导初始化：根据自然图像的空间结构先验分配2D高斯分布，产生局部化且语义有意义的分布
2. 自适应位宽量化：在量化感知微调中，对协方差参数采用自适应位宽量化，为复杂区域的小尺度高斯赋予更高精度
...
TL;DR: MF-RSVLM：一种多特征融合的遥感视觉语言模型，通过多尺度视觉表示学习和循环视觉特征注入机制，解决了现有遥感VLM在细粒度特征提取和视觉遗忘方面的问题。


<details>
  <summary>Details</summary>
Motivation: 现有大型视觉语言模型在遥感领域应用时面临两大挑战：1）遥感图像与自然图像的固有差异导致难以提取细粒度视觉特征；2）在深度语言处理过程中容易出现视觉遗忘现象。

Method: 提出MF-RSVLM模型，采用多尺度视觉表示学习来融合全局上下文和局部细节，增强对遥感场景中小型和复杂结构的捕捉能力；引入循环视觉特征注入方案，确保语言模型在生成过程中持续保持视觉证据的关联，减少视觉遗忘。

Result: 在多个遥感基准测试上的广泛实验表明，MF-RSVLM在遥感分类、图像描述和视觉问答任务上均达到最先进或极具竞争力的性能水平。

Conclusion: MF-RSVLM通过有效的多特征融合和视觉特征保持机制，显著提升了遥感视觉语言模型的性能，为遥感领域的多模态理解提供了有效解决方案。
...
Method: 提出RSAgent智能体MLLM，通过多轮工具调用交替进行推理和分割行动；查询分割工具箱，观察视觉反馈，利用历史观察修订空间假设来重新定位目标并迭代优化掩码；构建数据管道合成多轮推理分割轨迹，采用两阶段训练框架：冷启动监督微调后接基于细粒度任务特定奖励的智能体强化学习

Result: 在ReasonSeg测试集上达到66.5% gIoU的零样本性能，比Seg-Zero-7B提升9%；在RefCOCOg上达到81.5% cIoU，在领域内和领域外基准测试中均展示最先进性能

Conclusion: RSAgent通过智能体多轮推理行动范式有效解决了传统一次性分割方法的局限性，实现了更准确、可验证和可细化的文本引导对象分割
...
<details>
  <summary>Details</summary>
Motivation: 传统各向异性扩散方法使用显式扩散算子，难以适应复杂图像结构，性能受限；而学习基方法表现更好但缺乏扩散方法的理论优势

Method: 将去噪过程建模为一系列朴素扩散动作序列，通过深度Q学习优化动作选择顺序，构建随机各向异性扩散过程

Result: 该方法在三种常见噪声去除任务中超越现有扩散基方法，与代表性深度CNN方法竞争

Conclusion: 基于强化学习的可训练各向异性扩散框架能自适应不同图像结构，结合了传统扩散方法理论优势与学习基方法性能优势
...
<details>
  <summary>Details</summary>
Motivation: 细粒度遥感数据集通常采用层次标签结构，但将这种语义层次嵌入表示学习空间以提升检测性能仍具挑战。先前研究在不同层次应用监督对比学习，但忽视了两个关键问题：1) 标签层次中数据分布不平衡导致高频类别主导学习过程；2) 类别间语义关系学习干扰类别无关的定位任务。

Method: 提出平衡层次对比损失结合解耦学习策略，在DETR框架内实现。平衡损失引入可学习的类别原型，在每个层次上均衡不同类别的梯度贡献，确保每个层次类别在每批次中平等贡献损失计算。解耦策略将DETR的对象查询分为分类和定位两组，实现任务特定的特征提取和优化。

Result: 在三个具有层次标注的细粒度数据集上的实验表明，该方法优于现有最先进方法。

Conclusion: 提出的平衡层次对比损失与解耦学习策略有效解决了细粒度遥感检测中的层次标签不平衡和语义关系干扰问题，在DETR框架内实现了更好的性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散Transformer模型因注意力机制导致极高计算成本，限制了实际应用。现有稀疏注意力方法存在两个关键限制：稀疏模式预测的开销和硬件通用性不足（主要针对GPU设计）。随着ASIC等多样化硬件平台的应用，需要更通用的加速方案。

Method: 1) 利用块级均值作为代表性token进行稀疏掩码预测；2) 实现时空感知的token置换；3) 为视频生成场景引入首帧汇聚机制。这些技术共同构成了在线自适应、硬件高效的稀疏注意力框架。

Result: RainFusion2.0能够实现80%的稀疏度，在保持视频质量的同时获得1.5~1.8倍的端到端加速。该方法在各种生成模型上均有效，并在多样化硬件平台上验证了其泛化能力。

Conclusion: RainFusion2.0成功解决了现有稀疏注意力方法的局限性，提供了一种低开销、硬件通用的加速方案，显著提升了扩散Transformer模型在视频和图像生成任务中的计算效率。
...
TL;DR: 提出LMR框架，通过两阶段"思考-行动"决策过程解决文本到动作生成中的语义-运动学阻抗不匹配问题


<details>
  <summary>Details</summary>
Motivation: 当前文本到动作生成方法将问题视为直接翻译，面临语义-运动学阻抗不匹配的根本理论瓶颈：难以在单次映射中将语义密集的离散语言意图接地到运动学密集的高频运动数据

Method: 提出潜在运动推理框架，采用两阶段"思考-行动"决策过程：1) 双粒度分词器将运动解耦为语义丰富的推理潜在空间和保持物理保真度的高频执行潜在空间；2) 模型先自回归推理规划粗略轨迹，再实例化具体帧

Result: 在T2M-GPT和MotionStreamer两个代表性基线模型上实现LMR，实验表明在语义对齐和物理合理性方面均获得非平凡改进

Conclusion: 运动规划的最佳基底不是自然语言，而是学习到的运动对齐概念空间，通过潜在系统2推理可以有效弥合语言与物理之间的不可言说性鸿沟
...
TL;DR: 提出一种免训练的生成式对抗攻击框架，通过扩散模型生成自然、场景一致的对抗物体，显著提升单目深度估计攻击的有效性、隐蔽性和物理部署能力
...
Method: 基于扩散模型的免训练条件生成框架，包含显著区域选择模块识别对深度估计影响最大的区域，以及雅可比向量积引导机制将对抗梯度导向预训练扩散模型支持的更新方向

Result: 大量数字和物理实验表明，该方法在攻击有效性、隐蔽性和物理部署能力方面显著优于现有攻击方法，对自动驾驶安全评估具有重要实践意义

Conclusion: 提出的生成式对抗攻击框架能够生成物理上合理的对抗物体，引发显著的对抗深度偏移，为自动驾驶系统的安全评估提供了更强大的工具
...
TL;DR: GeoBench是一个几何推理分层基准，包含四个推理层级，通过六个形式化验证任务评估视觉语言模型在几何问题解决中的能力，发现推理模型优于通用多模态大模型，但性能随任务复杂度增加而显著下降。
...
Method: 提出GeoBench分层基准，包含四个推理层级：视觉感知、目标导向规划、严谨定理应用、自我反思回溯。通过TrustGeoGen生成六个形式化验证任务，系统评估从属性提取到逻辑错误校正的能力范围。实验对比推理模型和通用多模态大模型在不同复杂度任务上的表现。

Result: 实验表明：1) 推理模型（如OpenAI-o3）优于通用多模态大模型；2) 性能随任务复杂度增加而显著下降；3) 子目标分解和无关前提过滤对最终解题准确性有重要影响；4) 在某些任务中，思维链提示意外地降低了性能。

Conclusion: GeoBench为几何问题解决提供了全面的评估基准，同时为开发几何问题解决系统提供了可操作的指导方针。研究发现几何推理的关键瓶颈在于子目标分解和无关信息过滤能力，而非简单的定理应用。
...
TL;DR: 提出D²-Align方法解决文本到图像扩散模型对齐中的偏好模式崩溃问题，通过方向性解耦奖励信号来保持生成多样性


<details>
  <summary>Details</summary>
Motivation: 现有基于人类反馈强化学习的方法虽然能在自动化奖励指标上获得高分，但会导致偏好模式崩溃（PMC），即模型收敛到狭窄的高分输出模式（如单一风格或普遍过曝），严重损害生成多样性

Method: 提出方向性解耦对齐（D²-Align）框架：1）在奖励模型的嵌入空间中学习方向性校正（保持模型冻结）；2）在优化过程中将校正应用于奖励信号，防止模型崩溃到特定模式

Result: D²-Align在质量和多样性两方面都实现了与人类偏好的更好对齐，通过综合评估（定性分析和定量指标）验证了其优越性

Conclusion: 方向性解耦对齐方法有效缓解了偏好模式崩溃问题，在保持生成多样性的同时实现了更好的人类偏好对齐
...
TL;DR: 提出了首个大规模工业多模态缺陷数据集IMDD-1M，包含100万个对齐的图像-文本对，并基于此训练了面向工业场景的扩散式视觉语言基础模型，通过轻量微调即可高效适应专业领域。


<details>
  <summary>Details</summary>
Motivation: 工业制造和质量检测领域缺乏大规模、高质量的多模态数据集，限制了多模态学习在工业场景中的应用。现有数据集通常规模有限、标注粒度不足，难以支持复杂的工业缺陷分析和生成任务。

Method: 1. 构建IMDD-1M数据集：包含100万个高分辨率真实缺陷图像-文本对，涵盖60+材料类别和400+缺陷类型，每个样本都有专家验证的标注和细粒度文本描述（缺陷位置、严重程度、上下文属性）。2. 基于该数据集从头训练扩散式视觉语言基础模型，专门针对工业场景优化。3. 通过轻量微调将基础模型高效适应到特定领域。

Result: IMDD-1M成为首个大规模工业多模态缺陷数据集，支持分类、分割、检索、描述生成和生成建模等多种应用。训练的基础模型仅需任务特定数据不到5%（相比专用专家模型）即可达到相当性能，展示了数据高效的基础模型适应能力。

Conclusion: IMDD-1M数据集和相应的基础模型为工业检测和生成任务提供了可扩展、领域自适应和知识基础的解决方案，推动了制造业智能向多模态学习方向发展，为工业场景中的视觉语言模型应用开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型（MLLMs）的推理过程仍以文本为中心，在复杂的长时程、视觉中心任务中表现不佳。需要建立新的推理范式来提升视觉中心任务的性能。

Method: 提出DiffThinker框架，采用基于扩散的生成式多模态推理范式，将多模态推理重新定义为原生图像到图像生成任务，实现视觉中心推理。
...
Method: 提出Deep Global Clustering (DGC)概念框架，通过重叠区域的小补丁操作强制一致性，从局部补丁观察中学习全局聚类结构。该方法保持恒定内存使用，无需预训练。

Result: 在叶片病害数据集上，DGC实现了背景-组织分离（平均IoU 0.925），并通过可导航的语义粒度展示了无监督病害检测能力。但框架存在优化不稳定性，源于多目标损失平衡问题。

Conclusion: DGC的设计理念有价值，但稳定实现需要动态损失平衡的原则性方法。该工作被视为智力支架，为内存高效的高光谱图像分割提供了有前景的方向。
...
TL;DR: 提出内部引导（IG）策略，通过在训练过程中对中间层引入辅助监督，在采样过程中外推中间层和深层输出来提升扩散模型生成质量，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能捕捉完整数据分布，但由于训练数据不足难以覆盖低概率区域，导致生成对应区域图像质量差。现有引导策略如CFG会导致样本过度简化或失真，而基于退化版本的引导方法需要精心设计、额外训练和采样步骤。

Method: 提出内部引导（IG）策略：1）训练阶段在中间层引入辅助监督；2）采样阶段通过外推中间层和深层输出获得生成结果。该方法简单有效，无需额外训练或复杂采样步骤。

Result: 在ImageNet 256×256上，SiT-XL/2+IG在80和800轮分别达到FID=5.31和1.75；LightningDiT-XL/1+IG达到FID=1.34，显著优于所有对比方法；结合CFG后达到当前最优FID=1.19。

Conclusion: 内部引导策略通过简单的中间层监督和外推机制，显著提升了扩散模型的训练效率和生成质量，在多个基准上取得突破性成果，为扩散模型优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器(DiT)在视觉生成方面表现出色，但其迭代去噪过程结合较大模型容量导致推理成本高昂。现有研究表明DiT模型的迭代去噪过程在步骤间存在大量冗余计算，需要有效减少这些冗余。

Method: 提出CorGi框架，基于贡献度分析选择性地缓存低贡献度的transformer块输出，并在后续步骤中重用这些缓存结果，从而减少冗余计算。对于文本到图像任务，进一步提出CorGi+，利用每块交叉注意力图识别显著token，并应用部分注意力更新来保护重要对象细节。

Result: 在最先进的DiT模型上评估表明，CorGi和CorGi+平均实现高达2.0倍的加速，同时保持高质量生成结果。
...
TL;DR: ARM是一种轻量级可学习模块，通过语义引导的交叉注意力和自注意力机制，自适应融合CLIP的层次特征，实现"训练一次，随处使用"的范式，显著提升训练无关开放词汇语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有训练无关的开放词汇语义分割方法存在两个主要问题：一是依赖昂贵的外部基础模型（如SAM、DINO）引入先验知识，计算成本高；二是对CLIP内部特征应用静态手工启发式方法，效果次优。需要一种既高效又有效的方法来解锁和细化CLIP的像素级细节能力。

Method: 提出注意力细化模块（ARM），包含两个关键组件：1）语义引导的交叉注意力块，使用鲁棒的深层特征（K，V）选择和细化细节丰富的浅层特征（Q）；2）自注意力块。采用"训练一次，随处使用"的范式，在通用数据集（如COCO-Stuff）上训练一次后，可作为通用即插即用的后处理器应用于各种训练无关框架。

Result: 在多个基准测试上的广泛实验表明，ARM能够一致地提升基线性能，且推理开销可忽略不计。该方法建立了训练无关开放词汇语义分割的高效有效范式。

Conclusion: ARM作为一种轻量级可学习模块，通过自适应融合CLIP的层次特征，有效解决了训练无关开放词汇语义分割中CLIP表示粗糙的问题，实现了性能提升与计算效率的良好平衡。
...
TL;DR: Mirage是一个用于驾驶场景资产编辑的一步视频扩散模型，通过结合2D编码器特征和3D解码器来保持时空一致性，同时采用两阶段数据对齐策略解决分布不匹配问题。


<details>
  <summary>Details</summary>
Motivation: 现有的视频对象编辑方法在保持高视觉保真度和时间一致性方面存在困难，而视觉为中心的自动驾驶系统需要多样化和可扩展的训练数据来达到鲁棒性能。

Method: 基于文本到视频扩散先验，通过将预训练2D编码器的时序无关潜在特征注入3D解码器来恢复细节并保持因果结构；采用两阶段数据对齐策略（粗粒度3D对齐和细粒度2D精炼）解决场景对象与插入资产之间的分布不匹配问题。

Result: 在多种编辑场景中实现了高真实感和时间一致性，并能泛化到其他视频到视频转换任务，为未来研究提供了可靠基线。

Conclusion: Mirage通过创新的特征注入机制和两阶段对齐策略，成功解决了驾驶场景资产编辑中的时空一致性和分布不匹配问题，为自动驾驶数据增强提供了有效工具。
...
TL;DR: MambaSeg：一种新颖的双分支语义分割框架，使用并行Mamba编码器处理RGB图像和事件流，通过双维交互模块实现时空融合，在保持高性能的同时显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: RGB方法在快速运动、低光照或高动态范围条件下性能下降，事件相机虽有高时间分辨率和低延迟优势但缺乏颜色和纹理。现有多模态融合方法计算成本高且主要关注空间融合，忽略了事件流的时间动态特性。

Method: 提出MambaSeg双分支语义分割框架，使用并行Mamba编码器分别处理RGB图像和事件流。引入双维交互模块（DDIM），包含交叉空间交互模块（CSIM）和交叉时间交互模块（CTIM），在空间和时间维度上进行细粒度融合，改善跨模态对齐并减少模糊性。

Result: 在DDD17和DSEC数据集上的广泛实验表明，MambaSeg实现了最先进的分割性能，同时显著降低了计算成本，展示了其在高效、可扩展和鲁棒的多模态感知方面的潜力。

Conclusion: MambaSeg通过有效的时空融合机制，充分利用RGB和事件数据的互补特性，为在挑战性视觉条件下的高效、鲁棒语义分割提供了有前景的解决方案。
...
TL;DR: 提出PGMP框架解决牙科CBCT金属伪影问题，通过物理模拟生成训练数据、确定性单次前向预测消除随机采样、医学基础模型先验确保临床合理性，在效率和诊断可靠性上超越现有方法。
...
Method: 提出物理基础流形投影(PGMP)框架：1) 解剖自适应物理模拟(AAPS)管道通过蒙特卡洛光谱建模和患者特定数字孪生合成高保真训练对，弥合合成到真实差距；2) DMP-Former采用直接x预测范式，将恢复重新表述为确定性流形投影，单次前向传递恢复干净解剖结构；3) 语义结构对齐(SSA)模块利用医学基础模型(MedDINOv3)的先验锚定解决方案，确保临床合理性。

Result: 在合成和多中心临床数据集上的实验表明，PGMP在未见解剖结构上优于最先进方法，在效率和诊断可靠性方面设定了新基准。

Conclusion: PGMP框架有效解决了牙科CBCT金属伪影减少问题，通过物理模拟训练数据生成、确定性单次预测和医学先验约束，实现了高效且临床可靠的金属伪影去除，为临床应用提供了可行解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频理解中存在过度依赖语言先验的问题，导致面对反事实视频时产生视觉未接地幻觉。由于文本与视频数据不平衡以及反事实数据收集成本高昂，这一问题难以解决
...
Result: 在DualityVidQA-Test上，方法相比Qwen2.5-VL-7B基线相对提升24.0%，显著减少反事实视频的模型幻觉。同时在幻觉和通用基准测试中都取得显著增益，表现出强泛化能力

Conclusion: DualityForge框架通过合成高质量反事实数据和创新的训练方法，有效解决了MLLM在视频理解中的幻觉问题，提高了模型对视觉信息的依赖和泛化能力
...
TL;DR: Virtual-Eyes CT质量控制流程可提升通用基础模型在低剂量CT肺癌筛查中的性能，但可能破坏已适应原始临床数据的专家模型。


<details>
  <summary>Details</summary>
Motivation: 深度学习在低剂量CT肺癌筛查中缺乏对鲁棒预处理的量化评估，需要开发临床导向的质量控制流程来评估其对不同类型模型的影响。

Method: 开发Virtual-Eyes 16位CT质量控制流程：强制512×512平面分辨率、剔除非诊断序列、通过亨氏单位滤波和双侧肺覆盖评分提取连续肺块。使用765名NLST患者数据，在RAD-DINO、Merlin、Sybil和ResNet-18模型上评估原始输入与Virtual-Eyes处理输入的差异。

Result: Virtual-Eyes显著提升RAD-DINO性能：切片级AUC从0.576升至0.610，患者级AUC从0.646升至0.683（平均池化）和0.619升至0.735（最大池化），校准改善（Brier分数从0.188降至0.112）。但Sybil和ResNet-18性能下降，Merlin转移性有限。

Conclusion: 解剖学导向的质量控制可稳定并改进通用基础模型工作流程，但可能破坏已适应原始临床上下文的专家模型，表明预处理策略需根据模型类型进行定制。
...
TL;DR: UniAct是一个两阶段框架，通过微调MLLM和因果流式管道，实现人形机器人以低于500ms延迟执行多模态指令，在零样本跟踪不完美参考运动方面提升19%成功率。


<details>
  <summary>Details</summary>
Motivation: 人形机器人领域长期目标是实现能够遵循多样化多模态指令、具有人类水平灵活性的通用智能体。现有方法在将语言、音乐、轨迹等异构指令转换为稳定实时动作方面存在瓶颈，需要将高级多模态感知与全身执行有效连接。

Method: 提出UniAct两阶段框架：1) 微调多模态大语言模型(MLLM)处理多模态指令；2) 因果流式管道将指令转换为动作。通过FSQ共享离散码本统一输入，确保跨模态对齐同时将运动约束在物理可行流形上。在20小时的人形运动基准UniMoCap上进行验证。

Result: UniAct实现低于500ms的延迟，在零样本跟踪不完美参考运动方面获得19%的成功率提升。在UniMoCap基准测试中展示了对多样化真实场景的鲁棒泛化能力。

Conclusion: UniAct通过统一感知和控制，为实现响应式、通用的人形助手迈出了关键一步，能够通过共享离散表示实现多模态指令的无缝执行。
...
TL;DR: CERES是一个因果推理框架，通过双模态因果干预（后门调整和前端调整）来解决Ego-RVOS任务中的语言偏见和视觉混淆问题，在基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有Ego-RVOS方法存在两个主要问题：1) 从训练数据中的偏斜对象-动作配对学习虚假相关性（语言表示偏见）；2) 第一人称视角固有的视觉混淆因素（快速运动、频繁遮挡）。这些限制导致模型在真实场景中不够鲁棒。

Method: CERES是一个插件式因果框架，采用双模态因果干预：1) 后门调整原则处理语言表示偏见，对抗从数据集统计中学习的偏见；2) 前端调整概念处理视觉混淆，通过因果原则指导将语义视觉特征与几何深度信息智能集成，创建对第一人称扭曲更鲁棒的表征。
...
Method: 提出LVLDrive框架，通过渐进融合Q-Former逐步注入LiDAR特征，避免3D数据对预训练VLM的灾难性干扰。同时开发空间感知问答数据集来显式教授模型高级3D感知和推理能力。

Result: 在驾驶基准测试中，LVLDrive在场景理解、度量空间感知和可靠驾驶决策方面均优于纯视觉方法，证明了显式3D度量数据对构建可信VLM自动驾驶系统的必要性。

Conclusion: LVLDrive成功将LiDAR点云整合到VLM中，显著提升了自动驾驶系统的3D空间理解能力，为构建更安全可靠的VLM基自动驾驶系统提供了有效解决方案。
...
TL;DR: 该论文提出"基本信息力学"模型，将卷积滤波的机械特性与物理理论（狭义相对论和量子力学）联系起来，通过分析核的偶-奇分量来理解信息传播机制。


<details>
  <summary>Details</summary>
Motivation: 受狭义相对论和量子力学启发，建立理解卷积滤波机械特性的新模型。传统CNN分析缺乏对信息传播物理机制的深入理解，作者希望建立信息处理与物理能量-动量关系的联系。

Method: 将卷积核分解为正交的偶分量和奇分量：偶分量导致图像内容各向同性扩散（类似势能/静止能量），奇分量导致质心方向位移（类似动能）。在谱域通过离散余弦变换分析偶-奇特性，特别关注小卷积核（如3×3像素）的低频基（DC Σ和梯度分量∇）。

Result: 信息位移速度与奇分量能量占总能量之比呈线性关系。小卷积滤波器的结构主要由低频基主导，特别是DC Σ和梯度分量∇，这些定义了信息传播的基本模式。首次证明了通用CNN中信息处理与相对论物理核心的能量-动量关系之间的联系。

Conclusion: 提出的基本信息力学模型为理解卷积滤波的机械特性提供了新框架，将信息传播机制与物理能量-动量关系联系起来，为CNN分析开辟了新的物理视角。
...
Method: 引入Dermatology Assessment Schema (DAS)专家框架，包含36个高级和27个细粒度评估问题，提供中英文多选项。基于DAS构建专家标注的封闭式问答和分割数据集，评估多种多模态模型和提示策略。

Result: 分割任务中，默认提示在Mean-of-Max和Mean-of-Mean评估方案下表现最佳；增强提示（结合患者查询标题和内容）在多数投票评估下获得最高性能(Jaccard指数0.395，Dice分数0.566)。封闭式问答中，o3模型总体准确率最高(0.798)，GPT-4.1(0.796)紧随其后，Gemini-1.5-Pro(0.783)在Gemini家族中表现最佳。

Conclusion: DermaVQA-DAS数据集、DAS框架和评估协议的公开发布将支持并加速以患者为中心的皮肤病视觉语言建模研究，填补现有数据集的局限性。
...
TL;DR: 该论文提出了一个用于多模态预训练的综合框架，旨在从相机、LiDAR等车载传感器数据中构建空间智能，并建立了统一的预训练范式分类体系。
...
Result: 识别了计算效率和模型可扩展性等关键瓶颈，并提出了实现通用多模态基础模型的路线图，这些模型能够为实际部署提供鲁棒的空间智能。

Conclusion: 该研究为构建能够实现真实世界部署的通用多模态基础模型提供了系统性的框架和路线图，旨在解决自主系统中空间智能的核心挑战。
...
TL;DR: RedunCut是一个动态模型大小选择系统，通过测量驱动的规划器和轻量级数据驱动性能模型，在保持固定准确率的同时减少14-62%的计算成本。


<details>
  <summary>Details</summary>
Motivation: 实时视频分析在大规模摄像头部署中计算成本高昂。现有的动态模型大小选择方法存在两个主要问题：采样成本超过其收益，以及每段视频的准确率预测不准确，特别是在移动视频和低准确率目标场景下表现不佳。

Method: RedunCut采用测量驱动的规划器来估计采样的成本效益权衡，并使用轻量级、数据驱动的性能模型来改进准确率预测。系统通过两阶段方法：首先采样少量模型计算预测统计信息，然后基于这些统计信息选择模型大小。

Result: 在道路车辆、无人机和监控视频以及多个模型家族和任务中，RedunCut在固定准确率下减少了14-62%的计算成本，并且对有限历史数据和漂移保持鲁棒性。

Conclusion: RedunCut通过解决现有动态模型大小选择方法的两个关键缺陷——采样效率低下和准确率预测不准确，实现了显著的计算成本节省，同时保持了对多样化工作负载的鲁棒性。
...
Method: 采用流友好的自回归框架，配备流匹配头部进行概率建模；提出由前瞻模块增强的因果编码器，可纳入短期未来上下文（如60ms）以提升质量同时保持低延迟

Result: 每帧生成时间34ms，系统总延迟低于100ms；在HDTF数据集上获得离线8.13和在线7.61的LipSync Confidence分数，达到最先进的唇同步质量

Conclusion: DyStream通过创新的因果编码器设计和流匹配方法，实现了实时、高质量的对话头部视频生成，显著优于现有因果策略
...
<details>
  <summary>Details</summary>
Motivation: 传统手术技能评估依赖专家监督，存在主观性、评估者间差异大、耗时费力等问题，在低收入国家难以推广，限制了培训项目的可扩展性和一致性

Method: 提出AI驱动框架：1) 基于TimeSformer的视频Transformer架构，改进为分层时间注意力和加权空间注意力机制，实现手术视频中的准确动作识别；2) 使用YOLO目标检测和跟踪方法提取细粒度运动特征，分析器械运动学；3) 从五个方面评估微血管吻合术技能：整体动作执行、关键程序动作的运动质量、一般器械操作等
...
TL;DR: 该论文提出了一种通过离散小波变换(DWT)分析Vision Transformer(ViT)编码器中组合性的框架，发现单层DWT分解的基元能在潜在空间中近似组合，揭示了ViT如何结构化视觉信息。


<details>
  <summary>Details</summary>
Motivation: 虽然对Transformer模型的理解主要来自语言任务分析，但本研究旨在通过组合性视角探究Vision Transformer(ViT)编码器学习到的表示。动机是理解ViT在视觉任务中如何结构化信息，特别是其表示空间是否具有组合性特征。

Method: 引入了一个框架来测量ViT编码器中的组合性，类似于先前表示学习中测量组合性的工作。关键创新是使用离散小波变换(DWT)作为获取视觉设置中依赖于输入的基元的工具。通过检查组合表示重构原始图像表示的能力，实证测试表示空间中组合性的程度。

Result: 研究发现，从单层DWT分解得到的基元产生的编码器表示在潜在空间中能够近似组合。这表明ViT编码器学习到的表示具有一定的组合性结构，为理解ViT如何组织视觉信息提供了新视角。

Conclusion: 该研究通过组合性分析框架揭示了Vision Transformer编码器表示的结构特性，表明单层离散小波变换分解的基元能够在潜在空间中近似组合，为理解ViT如何结构化视觉信息提供了新的见解。
...
Method: 提出包含两个互补模块的学习图像压缩框架：1) iSWGE模块通过图节点表示光谱通道并学习边特征来显式建模波段间关系；2) WSGA-C模块结合稀疏图注意力与卷积注意力来减少空间冗余并强调精细结构

Result: 在SDOML数据集的六个极紫外通道上评估，相比强基线方法，MSID降低20.15%，PSNR提升最高1.09%，log变换MS-SSIM增益1.62%，在相同比特率下提供更清晰且光谱保真的重建

Conclusion: 该学习压缩框架有效解决了太阳多光谱图像压缩的挑战，在保持光谱和空间细节方面优于现有方法，代码已开源
...
TL;DR: 该研究提出了一种结合YOLO目标检测模型和大型语言模型（LLM）的自动化胸部X光报告生成系统，比较了YOLOv5和YOLOv8的性能，并评估了GPT-4生成报告的质量。


<details>
  <summary>Details</summary>
Motivation: 当前AI医学影像系统通常只输出结构化预测结果，放射科医生仍需将这些结果转化为完整的叙述性报告。大型语言模型（如GPT-4）的出现为从结构化发现生成诊断性叙述提供了新的机会，旨在填补这一空白。

Method: 开发了一个集成YOLOv5和YOLOv8进行胸部X光图像异常检测的流程，将边界框预测和类别标签传递给大型语言模型（LLM）生成描述性发现和临床总结。比较了两种YOLO模型在检测准确性、推理延迟方面的表现，并通过余弦相似度评估生成文本质量。

Result: 结果显示AI生成报告与人工报告具有强语义相似性。人工评估表明GPT-4在清晰度方面表现优异（4.88/5），但在自然写作流畅性方面得分较低（2.81/5），表明当前系统能达到临床准确性，但在风格上与放射科医生撰写的文本仍有区别。

Conclusion: 该研究证明了结合目标检测模型和大型语言模型生成放射学报告的可行性，系统在临床准确性方面表现良好，但在自然语言流畅性方面仍需改进，以实现与放射科医生报告风格的无缝融合。
...
Method: 1. 提出PhyAugPipe物理增强数据管道，利用视觉语言模型和思维链推理构建大规模训练数据集PhyVidGen-135K；2. 设计PhyGDPO框架，基于组级Plackett-Luce概率模型进行物理感知的直接偏好优化；3. 引入物理引导奖励方案PGR，嵌入VLM物理奖励以引导优化方向；4. 提出LoRA-Switch Reference方案，消除内存密集型参考复制以实现高效训练

Result: 在PhyGenBench和VideoPhy2基准测试上显著优于现有开源方法，证明该方法能有效提升视频生成的物理一致性

Conclusion: PhyGDPO框架通过物理增强数据构建和物理感知的组偏好优化，成功解决了文本到视频生成中的物理一致性挑战，为物理感知的视频生成提供了有效解决方案
...
Method: 提出新颖的二阶优化算法，结合OCP方法，对Hessian矩阵对角元素进行适当近似，以降低计算复杂度同时保持优化性能。

Result: 在多个标准视觉定位基准测试中表现显著优越，相比传统优化算法，在保持竞争力的定位精度的同时，实现更快收敛、增强的训练稳定性和改进的噪声干扰鲁棒性。

Conclusion: 该二阶优化算法有效解决了深度学习大规模优化问题，在视觉定位任务中展现出优越性能，为相关领域提供了高效的优化解决方案。
...
TL;DR: 该论文提出DARFT框架，通过决策模糊性引导的强化微调来解决CDVQA中决策模糊样本的问题，显著提升模型在少样本场景下的性能。


<details>
  <summary>Details</summary>
Motivation: 现有CDVQA模型通过监督微调后，许多失败案例并非源于明显错误预测，而是决策模糊性——模型对正确答案和强干扰项赋予相似置信度。这种决策模糊样本阻碍了模型的判别能力和鲁棒性提升。

Method: 提出DARFT框架：1) 使用SFT训练的参考策略挖掘决策模糊样本；2) 在挖掘的子集上应用组相对策略优化，利用多样本解码和组内相对优势来抑制强干扰项并锐化决策边界。

Result: 大量实验表明，DARFT在CDVQA任务上相比SFT基线获得一致性能提升，尤其在少样本设置下表现更为显著。

Conclusion: 明确优化决策模糊样本对于提升CDVQA模型的判别能力和鲁棒性至关重要。DARFT框架通过决策模糊性引导的强化微调，无需额外监督即可有效处理决策模糊问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有切片发现方法主要针对图像分类任务，难以应用于检测、分割、姿态估计等多实例任务。真实场景中的错误切片通常涉及复杂的视觉关系，现有实例级方法缺乏细粒度推理能力。当前基准测试通常针对特定算法或偏向图像分类，且使用人工合成的ground truth，无法反映真实模型失败情况。

Method: 提出SliceLens框架：1）利用LLM和VLM生成多样化的失败假设；2）通过基于视觉的推理验证这些假设；3）实现细粒度、可解释的错误切片识别。同时提出FeSD基准，包含专家标注的细粒度ground-truth切片，精确对应局部错误区域。

Result: 在现有基准和FeSD上的实验表明，SliceLens达到最先进性能：在FeSD上Precision@10提升0.42（0.73 vs. 0.31）。识别出的可解释切片有助于模型改进，模型修复实验验证了其有效性。

Conclusion: SliceLens通过LLM/VLM驱动的假设生成和验证，有效解决了多实例视觉任务中的细粒度错误切片发现问题。FeSD基准为细粒度切片发现评估提供了标准化平台。该方法在性能提升和可解释性方面均优于现有方法。
...
TL;DR: 该研究构建了首个针对飓风灾后环境的3D点云数据集，并评估了现有SOTA 3D语义分割模型在灾后场景中的性能，发现现有方法存在显著局限性。
...
Result: 评估结果显示，现有SOTA模型在灾后场景的3D语义分割任务中表现出显著局限性，无法有效处理灾后环境的复杂特征。
...
TL;DR: CLoRA是一种平衡学习性能和参数效率的协同低秩适应方法，通过基础空间共享和样本无关多样性增强组件，在保持参数效率的同时提升微调性能。
...
Method: 提出CLoRA方法，包含两个核心组件：1) 基础空间共享：所有低秩模块共享一组下/上投影空间，通过共享空间获得的低秩矩阵协同构建每个LRM；2) 样本无关多样性增强(SADE)：正则化这些矩阵之间的相似性，鼓励训练过程中产生多样化的表示。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位研究主要关注室内和室外驾驶场景，而路边基础设施系统的户外监控场景由于缺乏配对点云-文本数据而未被探索。基础设施级交通场景理解对智能交通系统至关重要。

Method: 构建了MoniRefer数据集（包含约136,018个对象和411,128个自然语言描述），并提出Moni3DVG端到端方法，利用图像提供的丰富外观信息以及点云的几何和光学信息进行多模态特征学习和3D对象定位。

Result: 在提出的基准测试上进行了广泛实验和消融研究，证明了所提方法的优越性和有效性。数据集和代码将公开发布。

Conclusion: 该工作填补了户外监控场景3D视觉定位的研究空白，为基础设施级交通场景理解提供了首个真实世界大规模数据集和有效方法，推动了智能交通系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究存在两个主要局限：1）主要关注山地或森林环境，对更频繁且结构复杂的城市救援场景关注不足；2）现有检测系统类别有限（如火焰、烟雾），缺乏涵盖消防车、消防员等指挥决策关键目标的全面系统。

Method: 首先构建了新的FireRescue数据集，涵盖城市、山地、森林和水域多种救援场景，包含8个关键类别共15,980张图像和32,000个边界框。其次提出FRS-YOLO改进模型：1）引入即插即用的多维协同增强注意力模块，通过跨维度特征交互增强易混淆类别的判别表示；2）集成动态特征采样器，强化高响应前景特征以减轻烟雾遮挡和背景干扰。
...
<details>
  <summary>Details</summary>
Motivation: 现有作物病害诊断方法通常依赖昂贵的监督微调，且在领域偏移下表现不佳，需要更准确、可解释且无需训练的解决方案。
...
Method: 提出径向拓扑生成框架：1）选择初始token作为中心点；2）根据空间距离将所有其他token分组到多个同心环中；3）从内向外按环并行生成；4）引入嵌套注意力机制动态修正不一致预测，防止误差累积和模型崩溃。

Result: RadAR在保持自回归模型表示能力的同时，显著提高了视觉生成的推理效率。径向并行预测设计充分利用了视觉场景的结构局部性和空间连贯性。

Conclusion: 通过结合径向并行预测和动态输出修正，RadAR为高效自回归视觉生成提供了创新解决方案，平衡了生成质量与推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前推理分割方法存在显著缺陷：监督微调（SFT）存在灾难性遗忘和领域依赖问题；强化学习（RL）训练不稳定且依赖预定义奖励函数；训练免费方法虽然避免了训练负担，但采用静态推理范式，依赖单次"生成-然后分割"链，导致推理深度不足且无法自我纠正语言幻觉或空间误解。

Method: 提出EVOL-SAM3零样本框架，将推理分割重新定义为推理时进化搜索过程。框架维护提示假设种群，通过"生成-评估-进化"循环迭代优化：1）引入视觉竞技场通过无参考成对锦标赛评估提示适应性；2）设计语义突变算子注入多样性并纠正语义错误；3）异构竞技场模块整合几何先验与语义推理以确保鲁棒最终选择。

Result: 在具有挑战性的ReasonSeg基准测试中，EVOL-SAM3不仅显著超越静态基线方法，还在零样本设置下显著超越完全监督的最先进方法，证明了其优越性能。

Conclusion: EVOL-SAM3通过将推理分割重新定义为进化搜索过程，成功解决了现有方法的局限性，提供了一种无需训练、能够自我纠正错误并实现深度推理的零样本解决方案，为复杂视觉语言任务开辟了新方向。
...
TL;DR: EchoFoley：视频配乐新任务，通过符号化声音事件表示实现细粒度控制，包含大规模数据集EchoFoley-6k和基于慢-快思考策略的EchoVidia框架，在可控性和感知质量上显著超越现有VT2A模型。


<details>
  <summary>Details</summary>
Motivation: 解决当前视频-文本到音频（VT2A）方法的三个关键限制：1）视觉与文本条件不平衡导致视觉主导；2）缺乏细粒度可控生成的具体定义；3）指令理解和遵循能力弱（现有数据集依赖简短分类标签）。

Method: 提出EchoFoley任务，采用符号化声音事件表示（指定何时、什么、如何产生声音），支持声音生成、插入和编辑等细粒度控制。构建EchoFoley-6k数据集（6000+视频-指令-标注三元组），并提出基于慢-快思考策略的EchoVidia框架。

Result: EchoVidia在可控性上超越最近VT2A模型40.7%，在感知质量上提升12.5%。

Conclusion: EchoFoley任务和EchoVidia框架有效解决了VT2A的现有限制，通过符号化声音事件表示和分层语义控制实现了细粒度可控的视频配乐生成。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅技术在实时新视角合成方面取得突破性进展，但其算法的快速扩散导致对标准化和全面评估工具的迫切需求，特别是在压缩任务方面。现有基准往往缺乏评估不同方法独特特征（如渲染速度、率失真权衡、内存效率和几何精度）的必要指标。

Method: 提出了Splatwizard统一基准工具包，提供易于使用的框架来实施新的3DGS压缩模型并利用现有最先进技术。该框架包含一个集成管道，可自动计算关键性能指标，包括基于图像的质量指标、重建网格的倒角距离、渲染帧率和计算资源消耗。

Result: 开发了一个开源工具包，代码已在GitHub上公开可用。该工具包为3DGS压缩模型提供了标准化的评估框架，填补了该领域缺乏全面基准的空白。

Conclusion: Splatwizard解决了3D高斯泼溅压缩模型评估的标准化需求，为研究人员提供了一个统一的基准工具包，能够全面评估不同方法的性能特征，促进该领域的发展。
...
TL;DR: 提出统一框架将3D高斯泼溅的特征嵌入与实例分割标签学习合并，通过Embedding-to-Label过程和边界硬挖掘提升性能


<details>
  <summary>Details</summary>
Motivation: 现有3D实例分割方法面临多视角2D标签不一致问题，现有两阶段方法要么依赖对比学习和超参数敏感的聚类，要么需要预处理标签保证一致性，训练时间长且性能受限

Method: 提出统一框架：1) 为高斯基元引入可学习的特征嵌入用于分割；2) 通过新颖的"Embedding-to-Label"过程高效解码为实例标签；3) 针对边界伪影问题，提出边界硬挖掘，但直接应用于特征嵌入不稳定，改为对光栅化特征嵌入应用线性层后再计算三元组损失

Result: 在ScanNet、Replica3D和Messy-Rooms数据集上，方法在定性和定量评估中均优于基线方法

Conclusion: 提出的统一框架将特征嵌入学习和标签解码合并，减少了训练时间并提升了性能，通过稳定的边界硬挖掘有效解决了对象边界伪影问题
...
<details>
  <summary>Details</summary>
Motivation: Noise2Noise方法虽然消除了对干净目标图像的需求，但存在主要限制：对噪声目标应用非线性函数会导致结果偏差。由于非线性函数在图像处理中很常见，避免它们限制了可对噪声目标进行的预处理类型。特别是在高动态范围(HDR)图像去噪中，异常值问题使训练困难，而常用的色调映射非线性函数被认为与Noise2Noise训练不兼容。

Method: 开发了一个理论框架来分析非线性函数的影响，描述了一类具有最小偏差的非线性函数。具体应用于HDR图像去噪时，考虑将非线性色调映射函数应用于模型输出和目标图像以减少动态范围，并证明特定损失函数和色调映射函数的组合可以减少异常值影响同时引入最小偏差。

Result: 该方法应用于现有的基于机器学习的蒙特卡洛去噪器，原始实现使用高样本数参考图像训练。改进后的结果接近原始实现，但仅使用噪声训练数据即可产生。

Conclusion: 某些非线性函数可以在不显著偏差的情况下应用于Noise2Noise训练中的噪声目标，这扩展了Noise2Noise方法的适用性，特别是在HDR图像去噪等需要非线性预处理的场景中。
...
<details>
  <summary>Details</summary>
Motivation: 在3D场景处理中，基于2D视觉输入的跨模态系统面临维度不匹配问题。虽然场景内相机可以弥合这一差距，但需要学习控制模块。现有方法通常需要预训练或微调，限制了系统的灵活性和适应性。

Method: 提出基于遗憾最小化和无导数优化的多元互信息估计方法。该方法将表达性度量与基于值的优化相结合，控制场景内相机直接从视觉语言模型的噪声输出中学习。使现成的2D视觉跨模态系统能够在线适应物体遮挡和特征区分。

Result: 该方法在多物体3D场景的跨模态任务中提高了性能，无需依赖预训练或微调。系统能够有效处理物体遮挡并区分特征，展示了在线适应能力。

Conclusion: 通过改进的多元互信息估计和控制优化，成功弥合了2D视觉输入与3D场景处理之间的维度差距。该方法为跨模态系统在复杂3D环境中的应用提供了无需预训练或微调的解决方案。
...
TL;DR: CropTrack：针对农业环境中多目标跟踪挑战的新型框架，通过结合外观和运动信息，在频繁遮挡和外观相似条件下实现更好的身份保持


<details>
  <summary>Details</summary>
Motivation: 农业环境中的多目标跟踪面临重复模式、相似外观、光照突变和频繁遮挡等挑战。现有跟踪器主要依赖运动信息，但在频繁强遮挡时难以保持目标身份，而外观相似性使得外观关联难以整合

Method: 提出CropTrack框架，结合外观和运动信息，包含：1）重排序增强的外观关联；2）一对多关联与基于外观的冲突解决策略；3）指数移动平均原型特征库以改进外观关联

Result: 在公开农业MOT数据集上评估，CropTrack表现出持续的身份保持能力，优于传统基于运动的跟踪方法。相比最先进方法，在识别F1和关联准确率上取得显著提升，身份切换数量更低

Conclusion: CropTrack通过有效整合外观和运动信息，解决了农业环境中多目标跟踪的挑战，特别是在频繁遮挡和外观相似条件下实现了更好的跟踪性能
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在评估多模态大语言模型处理真实世界金融数值推理任务方面存在不足，特别是在金融场景理解、复杂文档处理和多步推理能力方面需要更全面的评估工具。

Method: 构建包含1,200个专家标注问题的双语多模态基准，其中57.9%的问题融入了12种隐含金融场景，使用837份中英文金融文档（平均50.8页），要求平均11步推理（5.3步信息提取+5.7步计算）。

Result: 最佳性能的多模态大语言模型仅达到58.0%的准确率，不同检索增强生成方法在该任务上表现出显著性能差异，65.0%的问题需要跨页证据（平均2.4页）。

Conclusion: FinMMDocR基准填补了现有评估工具的空白，能够推动多模态大语言模型和推理增强方法在复杂真实世界场景中的改进，特别是在金融领域的应用。
...
<details>
  <summary>Details</summary>
Motivation: 3D目标检测器在自动驾驶感知系统中至关重要，但在不同地理区域（如美国、亚洲、欧洲）之间存在显著的域泛化问题，模型在源域训练后难以适应目标域

Method: 基于神经元激活模式选择目标域中少量代表性且多样化的样本进行标注，结合持续学习启发的后训练技术防止模型权重从原始模型漂移

Result: 经验评估表明，该方法在域自适应任务上优于线性探测和现有SOTA域自适应技术，且仅需极小的标注预算

Conclusion: 通过精心选择少量目标域样本进行标注，结合神经元激活模式分析和持续学习技术，可实现高效且有效的LiDAR域自适应，显著提升3D目标检测器的跨域泛化能力
...
<details>
  <summary>Details</summary>
Motivation: 冠状动脉钙化评分是心血管疾病风险评估的重要工具，但非门控胸部CT存在严重运动伪影，而ECG门控心脏CT应用受限。需要一种方法从常规胸部CT中准确量化钙化

Method: 提出ProDM（Property-aware Progressive Correction Diffusion Model），包含三个关键组件：1）CAC运动模拟数据引擎，从心脏门控CT合成非门控数据；2）钙特异性先验的属性感知学习策略；3）渐进校正方案，逐步减少伪影

Result: 在真实患者数据集上，ProDM显著提高了CAC评分准确性、空间病灶保真度和风险分层性能。读者研究证实ProDM能有效抑制运动伪影并改善临床可用性

Conclusion: ProDM展示了渐进式、属性感知框架在从常规胸部CT成像中进行可靠CAC量化的潜力，为心血管风险评估提供了更易获得的替代方案
...
TL;DR: VIPER：一个用于生成式视频推理的过程感知评估基准，包含16个任务和新的POC@r指标，揭示当前视频模型仅达到约20%的过程-结果一致性，存在显著的结果欺骗现象。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型评估框架通常依赖单帧评估，容易导致结果欺骗（模型通过错误过程得出正确结论），缺乏对推理过程的系统性评估。

Method: 提出过程感知评估范式，构建VIPER基准（包含16个跨时间、结构、符号、空间、物理和规划推理的任务），并提出POC@r指标（使用VLM-as-Judge和分层评分标准评估中间步骤有效性和最终结果）。

Result: 实验表明，最先进的视频模型仅达到约20%的POC@1.0，显示出显著的结果欺骗现象；测试时缩放和采样鲁棒性分析揭示了当前视频生成与真正广义视觉推理之间的巨大差距。

Conclusion: 需要过程感知评估来准确衡量生成式视频推理能力；VIPER基准和POC@r指标为评估模型推理过程提供了有效工具；当前视频模型距离真正的广义视觉推理仍有很大差距。
...
TL;DR: DarkEQA是一个开源基准测试，用于在多层次低光照条件下评估具身问答相关的感知能力，通过模拟物理真实的视觉退化来系统分析视觉语言模型在黑暗环境中的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要在理想光照条件下评估视觉语言模型，但实际应用中需要模型在包括低光照在内的各种视觉退化条件下保持性能，这一核心需求被忽视。DarkEQA旨在填补这一空白，专门评估具身智能体在黑暗环境中的感知瓶颈。

Method: DarkEQA通过模拟物理真实的视觉退化来创建基准：在RAW线性空间建模物理光照衰减和传感器噪声，然后通过ISP渲染流程生成退化图像。该方法隔离感知瓶颈，在受控退化条件下评估基于第一人称视角的问答能力。

Result: 通过评估多种最先进的视觉语言模型和低光照图像增强模型，系统揭示了这些模型在挑战性视觉条件下的局限性。基准测试表明现有模型在低光照条件下的性能显著下降。

Conclusion: DarkEQA填补了具身智能评估在低光照条件下的空白，为分析视觉语言模型的鲁棒性提供了可归因的评估框架。该基准将促进更鲁棒的视觉语言模型发展，支持全天候运行的具身智能系统。
...
TL;DR: 本文提出了一种无需重新索引的终身行人重识别方法（RFL-ReID），通过双向连续兼容表示框架解决历史图库特征与新模型特征不兼容的问题。


<details>
  <summary>Details</summary>
Motivation: 传统终身行人重识别（L-ReID）方法在每次模型更新后需要重新提取所有历史图库图像的特征（重新索引），但这面临数据隐私问题和计算成本高的挑战。历史图库特征与新模型提取的特征不兼容会严重影响重识别性能，因此需要一种无需重新索引的终身学习方法。

Method: 提出双向连续兼容表示（Bi-C2R）框架，通过持续更新旧模型提取的图库特征，使其与新模型提取的特征保持兼容。该方法在连续学习过程中平衡新旧知识，确保不同模型版本提取的特征能够相互兼容。

Result: 在多个基准测试上的实验表明，Bi-C2R方法在RFL-ReID任务和传统L-ReID任务上均取得了领先性能，验证了该方法的有效性。

Conclusion: RFL-ReID是一个更具挑战性的终身行人重识别任务，Bi-C2R框架通过实现特征兼容性，在无需重新索引历史图库的情况下实现了高效的终身学习，为解决数据隐私和计算成本问题提供了有效方案。
...
<details>
  <summary>Details</summary>
Motivation: 音频驱动视觉配音面临理想训练数据缺乏的根本挑战，现有基于掩码修复的方法需要同时生成缺失内容和同步唇部运动，导致视觉伪影、身份漂移和同步效果差

Method: 提出自引导框架：1) 使用扩散Transformer作为数据生成器，为每个真实样本合成唇部改变的伴生视频，形成视觉对齐的视频对；2) 基于DiT的音频驱动编辑器在这些视频对上端到端训练；3) 引入时间步自适应多阶段学习策略解耦不同扩散时间步的编辑目标；4) 提出ContextDubBench基准数据集
...
TL;DR: FineTec：用于时间损坏骨架序列的细粒度动作识别统一框架，通过上下文感知补全、骨架空间分解和物理驱动估计模块，在多个基准测试中显著优于现有方法。
...
Method: 1) 上下文感知补全：通过多样化时间掩码从损坏输入恢复基础骨架序列；2) 骨架空间分解：将骨架划分为五个语义区域，基于运动方差分为动态和静态子组，通过针对性扰动生成两个增强骨架序列；3) 物理驱动估计：利用拉格朗日动力学估计关节加速度；4) GCN动作识别头：联合处理融合的骨架位置序列和加速度序列。
...
Conclusion: FineTec通过统一框架有效处理时间损坏的骨架序列，结合上下文感知补全、语义区域分解和物理动力学建模，在细粒度动作识别任务中展现出优异的鲁棒性和泛化能力。
...
Conclusion: GaMO通过多视角外绘方法有效解决了稀疏视图重建中的覆盖范围、几何一致性和计算效率问题，为实际应用提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型难以实现对空间（相机视角）和时间（运动序列）的独立精确控制。缺乏同时提供连续时空变化配对的视频数据集，限制了模型学习时空解耦的能力。

Method: 1. 引入动画时间嵌入机制，在扩散过程中显式控制输出视频相对于源视频的运动序列；2. 提出时间扭曲训练方案，利用现有多视角数据集模拟时间差异；3. 改进相机条件机制，允许从第一帧开始改变相机；4. 创建CamxTime数据集，提供场景内完全自由的时空视频轨迹。

Result: 在真实世界和合成数据上的评估表明，SpaceTimePilot实现了清晰的时空解耦，相比先前工作取得了更强的结果。模型能够精确控制视频的相机视角和运动序列。

Conclusion: SpaceTimePilot通过创新的时间嵌入机制、时间扭曲训练方案和专门的CamxTime数据集，成功实现了视频生成中空间和时间的解耦控制，为可控生成式渲染提供了有效解决方案。
...
TL;DR: BanglaCodeAct：基于多智能体提示和迭代自校正的孟加拉语到Python代码生成框架，在低资源语言代码生成中取得突破性成果


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在英语到代码生成方面表现出色，但这一进展尚未扩展到低资源语言。针对孟加拉语到Python代码生成的需求，需要开发专门的方法来克服语言资源有限的挑战。

Method: 提出BanglaCodeAct框架，采用基于智能体的方法，利用多智能体提示和迭代自校正。该框架使用开源多语言LLM，在Thought-Code-Observation循环中实现动态代码生成、测试和优化，无需任务特定的微调。

Result: 在mHumanEval数据集上评估多个小型开源LLM，Qwen3-8B结合BanglaCodeAct表现最佳：开发集pass@1准确率94.0%，盲测集71.6%。这为孟加拉语到Python翻译建立了新的基准。

Conclusion: BanglaCodeAct为低资源语言的可靠代码生成提供了有效解决方案，证明了基于智能体的推理方法在处理语言资源有限场景中的潜力，为孟加拉语代码生成设立了新标准。
...
TL;DR: PharmaShip是一个中文药品运输单据扫描数据集，用于测试文本布局模型在噪声OCR和异构模板下的性能，包含序列实体识别、关系抽取和阅读顺序预测三个任务，通过实体中心评估协议和基准测试发现像素与几何信息互补但不足，阅读顺序正则化能提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有预训练文本布局模型在噪声OCR和异构模板下的真实世界文档理解能力不足，特别是在药品运输等安全关键领域缺乏标准化基准数据集和评估协议。

Method: 构建PharmaShip数据集，包含扫描药品运输单据，设计三个互补任务（SER、RE、ROP），采用实体中心评估协议，基准测试五种代表性模型（LiLT、LayoutLMv3-base、GeoLayoutLM及其RORE增强变体），标准化预处理、数据划分和优化过程。

Result: 像素信息和显式几何信息提供互补归纳偏置，但单独使用均不足；注入阅读顺序导向正则化能持续改善SER和EL性能，获得最鲁棒配置；更长位置覆盖稳定后页预测并减少截断伪影；ROP在词级别准确但在片段级别具有挑战性，反映边界模糊性和长距离跨越问题。

Conclusion: PharmaShip为药品领域安全关键文档理解建立了可控、可复现的基准，强调序列感知约束作为结构建模的可迁移偏置，像素与几何信息需结合阅读顺序正则化才能获得最佳性能。
...
TL;DR: LN-RP协议：通过向LLM初始生成状态注入随机噪声种子，分析噪声驱动的人格涌现的计算框架，揭示了三种具有不同熵特征的稳定人格模式。
...
Method: 提出Luca-Noise Reflex Protocol (LN-RP)计算框架，通过向初始生成状态注入随机噪声种子，在152个生成周期中观察语言行为的非线性转变。
...
TL;DR: HarmTransform：一种多智能体辩论框架，用于将有害查询转化为更隐蔽的形式以增强LLM安全对齐


<details>
  <summary>Details</summary>
Motivation: 当前LLM安全机制主要关注明显有害内容，但用户可以通过隐蔽的改写方式保留恶意意图同时显得无害，现有安全训练数据存在显著缺口

Method: 引入HarmTransform多智能体辩论框架，通过多个智能体之间的迭代批判和精炼，系统地将有害查询转化为更隐蔽的形式，同时保持其底层有害意图

Result: 实验表明HarmTransform在生成有效查询转换方面显著优于标准基线；分析显示辩论具有双刃剑效应：既能锐化转换提高隐蔽性，也可能引入主题偏移和不必要的复杂性

Conclusion: 多智能体辩论在生成全面安全训练数据方面既有前景也有局限性，需要平衡转换效果与复杂性控制
...
TL;DR: LLMs在完美信息游戏中能学习世界模型，本文研究其在德州扑克这种不完全信息POMDP中是否也能学习到随机环境的内部表示


<details>
  <summary>Details</summary>
Motivation: 扩展LLMs世界模型研究到不完全信息领域，验证LLMs在部分可观察马尔可夫决策过程(POMDP)中是否能学习到环境的内部表示

Method: 在扑克手牌历史数据上预训练GPT风格模型，使用非线性探针分析其内部激活，解码确定性结构（手牌等级）和随机特征（胜率）

Result: 模型在没有明确指导的情况下学会了手牌等级和胜率等特征，这些表示可解码且与理论信念状态相关，表明LLMs学习了德州扑克随机环境的内部表示

Conclusion: LLMs能在不完全信息POMDP中学习环境的内部表示，包括确定性和随机特征，为理解LLMs在复杂随机环境中的表示学习提供了证据
...
TL;DR: 提出两阶段框架解决微妙性别歧视内容检测中的三大挑战：数据不足、噪声和概念模糊性，通过针对性训练和基于推理的选择性推断实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 在线性别歧视内容日益呈现微妙、依赖上下文的形式，传统检测方法难以捕捉。其解释涉及语言、心理、法律和文化等多维度重叠，导致标注数据中存在混合甚至矛盾的信号。这些不一致性加上标签稀缺和类别不平衡，导致决策边界不稳定，使微调模型忽略更微妙、代表性不足的伤害形式。需要解决数据不足、噪声和概念模糊性三大挑战的综合影响。

Method: 提出两阶段框架：1) 针对性训练程序：应用类别平衡焦点损失、类别感知批处理和事后阈值校准来缓解标签不平衡和噪声监督；2) 选择性推理推断：动态路由机制直接分类高置信度案例，将不确定实例升级到新颖的"协作专家判断"模块，该模块提示多个人格并通过法官模型整合其推理。

Result: 在多个基准测试中取得最先进结果：在EXIST 2025 Task 1.1上F1分数提升+2.72%，在EDOS Tasks A和B上分别获得+4.48%和+1.30%的增益。

Conclusion: 该框架通过统一针对性训练和基于推理的选择性推断，有效解决了微妙性别歧视检测中的三大核心挑战，显著提升了检测性能，为处理复杂、模糊的在线有害内容提供了新方法。
...
TL;DR: EASD是一种无需训练的方法，通过动态熵惩罚改进推测解码，允许在某些情况下超越目标LLM的性能


<details>
  <summary>Details</summary>
Motivation: 传统推测解码中草稿模型与目标模型过度对齐，限制了性能只能达到目标LLM水平，需要突破这一限制

Method: 在标准推测解码基础上引入动态熵惩罚，利用采样分布的熵量化不确定性，当两模型高熵且top-N预测重叠时拒绝令牌并由目标LLM重新采样

Result: 在多个推理基准测试中，EASD始终优于现有SD方法，大多数情况下甚至超越目标LLM本身，同时效率与SD相当

Conclusion: EASD通过草稿模型验证和熵感知机制，实现了超越目标模型固有性能的可能性，为推测解码提供了新的改进方向
...
TL;DR: MiMo-Audio通过大规模音频预训练（超1亿小时）实现了音频任务的少样本学习能力，在多项基准测试中达到开源SOTA，并能泛化到未见任务如语音转换、风格迁移等。


<details>
  <summary>Details</summary>
Motivation: 现有音频语言模型通常需要任务特定微调，而人类能够通过少量示例或简单指令泛化到新音频任务。受GPT-3在文本领域通过扩展预训练实现强泛化能力的启发，研究者认为这一范式同样适用于音频领域。

Method: 1. 将MiMo-Audio预训练数据扩展到超1亿小时音频，观察到少样本学习能力的涌现；2. 开发系统化评估框架；3. 在训练后阶段，构建多样化指令调优语料库，并在音频理解和生成中引入思维机制。

Result: 1. MiMo-Audio-7B-Base在语音智能和音频理解基准测试中达到开源SOTA；2. 能泛化到训练数据中未见的任务（语音转换、风格迁移、语音编辑）；3. 具备强大的语音延续能力，可生成高度逼真的脱口秀、朗诵、直播和辩论；4. MiMo-Audio-7B-Instruct在音频理解、口语对话和指令TTS评估中达到开源SOTA，接近或超越闭源模型。

Conclusion: 通过大规模音频预训练，MiMo-Audio证明了扩展下一个令牌预测预训练范式在音频领域的有效性，实现了类似人类的小样本泛化能力，为通用音频智能系统的发展提供了重要进展。
...
TL;DR: 本文通过SHAP解释方法对比分析了两种基于Transformer的新闻偏见检测模型，发现模型架构和训练策略显著影响其可靠性和部署适用性。


<details>
  <summary>Details</summary>
Motivation: 新闻文本的自动偏见检测广泛用于支持新闻分析和媒体问责，但人们对偏见检测模型如何做出决策或为何失败知之甚少。本研究旨在通过可解释性分析揭示不同模型架构如何操作化语言偏见。

Method: 采用比较可解释性研究方法，使用基于SHAP的解释分析两种基于Transformer的偏见检测模型：1) 在BABE数据集上微调的偏见检测器；2) 在BABE数据集上微调的领域自适应预训练RoBERTa模型。通过分析正确和错误预测的词级归因来表征不同模型架构如何操作化语言偏见。

Result: 结果显示：1) 两种模型都关注相似的评价性语言类别，但在如何将这些信号整合到预测中存在显著差异；2) 偏见检测器模型为假阳性分配了比真阳性更强的内部证据，表明归因强度与预测正确性之间存在错配，导致系统性地过度标记中性新闻内容；3) 领域自适应模型的归因模式与预测结果更一致，假阳性减少了63%；4) 模型错误源于不同的语言机制，假阳性主要由话语层面的模糊性而非明确的偏见线索驱动。

Conclusion: 研究强调了可解释性评估对偏见检测系统的重要性，表明架构和训练选择对模型可靠性和在新闻环境中的部署适用性具有关键影响。领域自适应方法在减少误报和改善归因一致性方面表现更优。
...
TL;DR: 该论文研究在检索增强问答中使用LLMs时，长上下文窗口带来的挑战，并提出自适应分块提示策略来平衡相关信息与无关信息，在减少token使用的同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs上下文窗口的扩展，检索增强生成中更广泛上下文的使用增加。虽然长上下文更容易包含目标知识，但也引入了更多无关信息，这会阻碍模型的生成过程并降低性能。

Method: 设计自适应提示策略：将检索到的信息分割成较小的块，并顺序提示LLM使用每个块来回答问题。通过调整块大小，可以在包含相关信息和减少无关信息之间进行权衡。

Result: 在三个开放域问答数据集上的实验结果表明，自适应策略在使用更少token的情况下，能够匹配标准提示的性能。分析发现，当遇到信息不足时，LLM经常生成错误答案而不是拒绝回答，这是主要的错误来源。
...
TL;DR: 利用中间注意力层的token分布生成对抗样本，用于压力测试LLM评估流程


<details>
  <summary>Details</summary>
Motivation: 利用机制可解释性的发现——中间注意力层编码了迭代细化的token级假设，探索从注意力层token分布直接生成对抗样本的可能性，以创建既合理又与模型自身生成过程内部一致的扰动

Method: 从中间注意力层的token分布中提取token作为对抗扰动，在ArgQuality数据集上进行论证质量评估实验，使用LLaMA-3.1-Instruct-8B同时作为生成器和评估器

Result: 注意力层生成的对抗样本导致评估性能显著下降，同时保持与原始输入的语义相似性；但某些层和token位置的替换会引入语法退化，限制了实际效果

Conclusion: 中间层表示作为对抗样本的原则性来源既有前景也有当前局限性，可用于压力测试基于LLM的评估流程
...
TL;DR: 该研究通过多检索器RAG系统结合最新LLM解决金融数值推理QA任务中的领域知识缺乏问题，在FinQA基准上实现SOTA性能，显著提升7%以上


<details>
  <summary>Details</summary>
Motivation: 尽管LLM取得进展，金融数值问题仍然具有挑战性，因为它们需要特定的金融领域知识和复杂的多步数值推理。现有模型缺乏金融领域知识导致错误

Method: 实现多检索器RAG系统，检索外部领域知识和内部问题上下文，利用最新LLM处理任务。使用SecBERT编码器进行领域特定训练，构建神经符号模型

Result: 领域特定训练的SecBERT编码器使神经符号模型超越FinQA基准模型；最佳提示式LLM生成器实现SOTA性能，显著提升>7%，但仍低于人类专家水平

Conclusion: 领域特定训练具有潜在优越性能；小模型存在幻觉损失与外部知识增益的权衡，大模型外部事实增益通常超过幻觉损失；最新LLM增强了少样本学习的数值推理能力
...
TL;DR: 论文提出一个分析框架，将学生回答的内容信号与评分者倾向分离，通过建模教师历史作为动态先验，结合文本嵌入来预测评分，并量化各信号贡献度。


<details>
  <summary>Details</summary>
Motivation: 自动评分常混淆学生回答内容与教师评分倾向，需要分离内容信号与评分者倾向，使评分判断可见且可审计，以支持教学反思。

Method: 使用ASSISTments数学回答数据，建模教师历史为动态先验，从句子嵌入提取文本表示，采用中心化和残差化处理提示和教师混杂因素，通过时间验证的线性模型量化各信号贡献，投影分析模型分歧。

Result: 教师先验对评分预测影响显著；先验与内容嵌入结合时效果最佳(AUC~0.815)，纯内容模型虽高于随机但较弱(AUC~0.626)；调整评分者效应可锐化残差内容表示，保留更多信息维度，揭示语义证据支持理解而非表面回答差异。

Conclusion: 该框架将嵌入从特征转化为学习分析工具，使教师和研究者能检查评分实践与学生推理证据的一致性，支持教学反思和改进。
...
TL;DR: HGMem提出了一种基于超图的内存机制，用于增强多步检索增强生成(RAG)系统，通过动态超图结构捕捉事实间的高阶关联，提升全局理解和复杂推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统中的工作内存模块主要作为被动存储，仅积累孤立事实用于输入压缩和子查询生成。这种静态设计忽略了原始事实间的高阶关联，而这些关联组合能为后续推理步骤提供更强指导。现有方法在表示能力和对多步推理及知识演化的影响方面有限，导致在扩展上下文中的推理碎片化和全局理解能力弱。

Method: HGMem是一种基于超图的内存机制，将内存从简单存储扩展为动态、表达性强的复杂推理和全局理解结构。内存表示为超图，其中超边对应不同的内存单元，能够在内存中逐步形成高阶交互。该机制围绕焦点问题连接事实和思考，演变为集成的情境化知识结构，为后续步骤的深度推理提供强命题。

Result: 在多个为全局理解设计的挑战性数据集上进行评估，广泛实验和深入分析表明，HGMem方法持续改进多步RAG性能，在各种任务上显著优于强基线系统。

Conclusion: HGMem通过超图结构将内存从被动存储转变为动态、表达性强的推理支持系统，能够捕捉事实间的高阶关联，显著提升多步RAG在复杂推理和全局理解任务上的性能。
...
Result: LoZA在长上下文场景中实现显著加速，支持处理高达100万token的上下文，LongCat-Flash-Exp成为能够高效进行长期推理和长视野智能体能力的基础模型。

Conclusion: LoZA稀疏注意力方案有效解决了长上下文处理的计算瓶颈，为大规模语言模型提供了高效的长序列处理能力，支持复杂的推理和智能体应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有大语言模型和监督方法在大规模中文拼写纠错中存在局限性：对新型错误缺乏鲁棒性，且依赖昂贵的标注数据。需要一种无需监督的鲁棒纠错方法。

Method: 提出CEC-Zero框架：1）从干净文本合成错误输入；2）通过语义相似度和候选一致性计算聚类共识奖励；3）使用PPO优化策略。该方法具有无偏奖励和收敛的理论保证。

Result: 在9个基准测试中，CEC-Zero比监督基线提升10-13个F1点，比强LLM微调方法提升5-8个F1点，建立了无标签的鲁棒纠错范式。

Conclusion: CEC-Zero为大规模中文拼写纠错提供了无需监督的强化学习解决方案，解锁了大语言模型在噪声文本处理中的潜力，实现了可扩展的鲁棒纠错。
...
TL;DR: 提出RISE框架，通过稀疏自编码器在激活空间中无监督地发现编码不同推理行为的"推理向量"，实现LLM推理过程的可解释性和可控干预。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖人工定义的概念（如过度思考、反思）在词级别监督分析推理过程，但难以捕捉全部潜在推理行为，许多行为难以在token空间中定义。需要无监督方法来发现和理解LLM的内部推理机制。

Method: 提出RISE框架：将思维链轨迹分割为句子级"步骤"，在步骤级激活上训练稀疏自编码器（SAEs），发现解耦特征对应可解释行为（如反思、回溯）。通过可视化和聚类分析这些行为在解码器列空间中的分布，并对SAE衍生向量进行针对性干预。

Result: SAEs发现了解耦特征对应可解释推理行为（反思、回溯），这些行为在解码器列空间中占据可分离区域。对SAE向量的干预可控制放大或抑制特定推理行为，改变推理轨迹而无需重新训练。SAEs还能捕捉结构特性（如响应长度），并发现超越人工监督的新行为（如置信度相关向量）。

Conclusion: 无监督潜在发现方法在解释和可控引导LLM推理方面具有巨大潜力，RISE框架为理解LLM内部推理机制提供了新途径，并能实现推理行为的可控干预。
...
TL;DR: WISE框架评估轻量级Transformer模型在区分假新闻与讽刺内容上的性能，发现MiniLM准确率最高(87.58%)，RoBERTa-base的ROC-AUC最优(95.42%)，DistilBERT在效率与准确性间达到最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 假新闻与讽刺/幽默内容具有重叠的语言特征但意图不同，这给区分带来了独特挑战。研究旨在开发一个评估框架，比较轻量级Transformer模型在资源受限环境下的性能，为实际部署提供可行方案。

Method: 开发WISE(Web信息讽刺与虚假性评估)框架，在包含20,000个样本的平衡数据集(Fakeddit)上评估8个轻量级Transformer模型和2个基线模型。使用分层5折交叉验证，评估指标包括准确率、精确率、召回率、F1分数、ROC-AUC、PR-AUC、MCC、Brier分数和预期校准误差。通过配对t检验和McNemar检验进行统计显著性比较。
...
Conclusion: 轻量级Transformer模型在区分假新闻与讽刺内容的任务中表现出色，能够在资源受限的实际环境中有效部署。研究为错误信息检测系统的实际应用提供了可行方案，特别强调了模型选择时需要考虑效率与准确性的平衡。
...
TL;DR: iCLP框架通过让LLM在潜在空间中进行隐式规划，在语言空间中进行推理，显著提高了数学推理和代码生成任务的准确性和效率，并展现出强大的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于显式文本规划的LLM推理存在两个主要问题：1) LLM幻觉导致规划不准确；2) 任务特定问题的高度多样性使得生成有效文本规划具有挑战性。受人类内隐认知启发，研究者希望开发一种更紧凑、泛化的规划方法。

Method: iCLP框架包含三个关键步骤：1) 从现有逐步推理轨迹中蒸馏显式规划；2) 通过向量量化自编码器和码本学习这些规划的离散表示（潜在规划）；3) 在配对的潜在规划和相应推理步骤上微调LLM，使模型学会在推理过程中进行隐式规划。

Result: 实验结果表明，iCLP使LLM能够在潜在空间规划的同时在语言空间推理，在数学推理和代码生成任务上取得了准确性和效率的显著提升。更重要的是，该方法展现出强大的跨域泛化能力，同时保持了思维链推理的可解释性。

Conclusion: iCLP框架成功地将人类内隐认知原理应用于LLM规划，通过潜在空间规划解决了显式文本规划的局限性，实现了更高效、准确且泛化能力强的推理系统，同时保持了推理过程的可解释性。
...
<details>
  <summary>Details</summary>
Motivation: LLM在医疗、法律、金融等关键决策领域应用日益广泛，但其可靠性存在不确定性：常犯过度自信错误、输入偏移下性能下降、缺乏明确的不确定性估计。现有评估方法零散，仅关注孤立方面。

Method: 引入Composite Reliability Score (CRS)统一框架，整合校准、鲁棒性和不确定性量化三个维度。在五个QA数据集上对十个领先开源LLM进行实验，评估基线性能、扰动影响和校准方法。

Result: CRS提供稳定的模型排名，揭示单一指标遗漏的隐藏故障模式，并显示最可靠的系统在准确性、鲁棒性和校准不确定性之间取得平衡。

Conclusion: CRS作为统一评估框架，能更全面评估LLM在关键决策应用中的可靠性，识别出平衡准确性、鲁棒性和校准不确定性的最可靠系统。
...
TL;DR: HY-MT1.5系列翻译模型（1.8B和7B参数）通过多阶段训练框架，在参数效率方面表现卓越，超越大型开源模型和商业API，接近超大型专有模型性能，并支持高级约束功能。


<details>
  <summary>Details</summary>
Motivation: 开发高性能机器翻译模型，通过参数高效的设计在保持较小模型规模的同时达到或超越大型模型的翻译性能，并提供高级约束功能以满足专业翻译需求。

Method: 采用多阶段训练框架：通用预训练与MT导向预训练、监督微调、策略蒸馏和强化学习相结合的全流程训练方法。

Result: HY-MT1.5-1.8B在标准中-外和英-外任务中全面超越大型开源模型和主流商业API，达到Gemini-3.0-Pro约90%性能；HY-MT1.5-7B在其规模级别达到新SOTA，在Flores-200上达到Gemini-3.0-Pro的95%性能，在WMT25和普通话-少数民族语言测试集上超越Gemini-3.0-Pro。

Conclusion: HY-MT1.5系列模型在各自参数规模下为通用和专业翻译任务提供了高度竞争力和鲁棒的解决方案，支持术语干预、上下文感知翻译和格式保持等高级约束功能。
...
<details>
  <summary>Details</summary>
Motivation: 掩码扩散语言模型通过迭代去噪过程生成文本，具有掩码并行解码优势且性能可与自回归大语言模型竞争，但其推理时控制和导向机制尚未充分探索。

Method: 提出激活导向框架，使用对比示例通过单次前向传播计算层间导向向量，在反向扩散的每一步应用这些方向，实现高效推理时控制。

Result: 在LLaDA-8B-Instruct模型上的实验表明，该方法能可靠地调控高层次属性，并通过消融实验检验了导向在不同Transformer子模块和token范围（提示vs.响应）上的效果。

Conclusion: 该激活导向框架为掩码扩散语言模型提供了有效的推理时控制机制，无需模拟去噪轨迹即可实现高效属性调控，为MDLMs的实际应用提供了重要工具。
...
TL;DR: 本文提出大型情感世界模型（LEWM），通过整合情感因素改进世界模型，在保持基础任务性能的同时，更准确地预测情感驱动的社会行为。
...
Method: 基于心理理论提出LEWM模型：1）构建Emotion-Why-How（EWH）数据集，将情感整合到因果关系中，支持推理行为原因和情感如何驱动未来世界状态；2）显式建模情感状态、视觉观察和动作，使世界模型能够预测未来状态和情感转变。

Result: 实验结果显示：LEWM能更准确地预测情感驱动的社会行为，同时在基础任务上保持与通用世界模型相当的性能。

Conclusion: 情感是世界理解的重要组成部分，LEWM通过系统整合情感因素，提升了世界模型对社会行为的预测能力，为情感感知的世界建模提供了有效框架。
...
TL;DR: TeleChat3-MoE是采用专家混合架构的大语言模型系列，参数规模1050亿到超万亿，在昇腾NPU集群上端到端训练。技术报告主要介绍支持前沿模型规模扩展的训练基础设施，包括数值精度验证、性能优化、并行化框架和集群级优化。


<details>
  <summary>Details</summary>
Motivation: 为了在昇腾NPU硬件生态上可靠且高效地扩展到前沿模型规模（1050亿到超万亿参数），需要建立系统性的训练基础设施，解决大规模分布式训练中的数值一致性、性能瓶颈和并行配置优化等挑战。

Method: 1) 算子级和端到端数值精度验证方法，确保跨硬件平台和分布式并行策略的一致性；2) 性能优化：交错流水线调度、长序列训练的注意力感知数据调度、专家并行的分层重叠通信、DVM算子融合；3) 基于分析估计和整数线性规划的系统性并行化框架；4) 集群级优化，解决主机和设备端瓶颈。

Result: 基础设施改进带来了显著的吞吐量提升，在数千设备集群上实现了接近线性的扩展，为硬件生态系统上的大规模语言模型开发提供了坚实基础。

Conclusion: 该技术报告展示了支持TeleChat3-MoE模型系列训练的系统性基础设施方法，通过数值验证、性能优化、并行配置和集群优化，在昇腾NPU集群上实现了可靠且高效的大规模模型训练，为前沿规模语言模型开发提供了可扩展的解决方案。
...
TL;DR: MedKGI是一个基于医学知识图谱的临床诊断框架，通过约束推理、信息增益驱动的提问选择和结构化状态跟踪，解决了LLM在临床诊断中的幻觉、低效提问和对话不一致问题。
...
Conclusion: MedKGI通过将医学知识图谱与结构化推理相结合，有效解决了LLM在临床诊断中的关键局限，实现了更准确、高效的诊断推理，模拟了真实临床实践中的迭代假设驱动过程。
...
TL;DR: LAILA：最大的公开阿拉伯语自动作文评分数据集，包含7,859篇作文，在七个维度上标注了整体和特质分数，填补了阿拉伯语AES研究的数据空白。
...
Method: 设计、收集并标注了7,859篇阿拉伯语作文，在七个维度（相关性、组织结构、词汇、风格、发展、格式、语法）上提供整体和特质分数。

Result: 创建了LAILA数据集，这是迄今为止最大的公开阿拉伯语AES数据集，并提供了使用最先进的阿拉伯语和英语模型在特定提示和跨提示设置下的基准结果。
...
Method: 开发了Pat-SPECTER模型（基于SPECTER2在专利数据上微调），通过"赛马式"评估比较了8种语言相似性模型在预测专利-论文引用方面的性能，并在两个实际场景中验证模型能力。

Result: Pat-SPECTER模型在预测可信专利-论文引用方面表现最佳；验证了美国专利因诚信义务要求而倾向于引用语义相似度较低的论文，与其他主要司法管辖区存在差异。

Conclusion: Pat-SPECTER是处理专利和科学文献相似性的有效工具，揭示了不同司法管辖区引用行为的系统性差异，模型已开源供学术和实践使用。
...
TL;DR: DATAMASK是一个用于大规模预训练数据选择的高效联合学习框架，能够同时优化质量和多样性指标，在万亿级token数据集上实现高效选择，显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前大规模预训练数据选择方法存在局限性：基于质量指标的选择在长期预训练中收益递减，而基于多样性指标的选择会移除过多高质量样本。由于计算成本高，很少在万亿级数据集上同时考虑多种指标进行联合选择。

Method: DATAMASK将数据选择过程建模为掩码学习问题，采用基于策略梯度的优化方法：迭代采样数据掩码，基于预定义目标计算策略梯度，更新掩码采样逻辑。通过多种加速增强技术，相比贪心算法减少98.9%的选择时间。

Result: 从15万亿token的FineWeb数据集中选择约10%的子集FineWeb-Mask。在12个多样化任务上评估，1.5B密集模型性能提升3.2%，7B MoE模型提升1.9%。选择时间相比贪心算法减少98.9%。

Conclusion: DATAMASK框架能够高效地在万亿级预训练数据集上实现质量和多样性指标的联合优化，显著提升模型性能，为大规模预训练数据选择提供了有效的解决方案。
...
TL;DR: FIGR通过强化学习将主动视觉思考整合到多轮推理中，构建视觉表示来外化中间结构假设，在复杂数学推理任务上显著优于纯文本方法。


<details>
  <summary>Details</summary>
Motivation: 复杂推理问题常涉及隐含的空间、几何和结构关系，这些关系在文本中未被明确编码。纯文本推理难以表示复杂场景中的全局结构约束，需要新的方法来增强推理的稳定性和可靠性。

Method: FIGR通过端到端强化学习将主动视觉思考整合到多轮推理中。系统在问题解决过程中构建视觉表示来外化中间结构假设，并自适应地调节何时以及如何调用视觉推理，从而更好地捕捉全局结构特性。

Result: 在具有挑战性的数学推理基准测试中，FIGR显著优于强文本链式思维基线。具体来说，FIGR在AIME 2025上提升了基础模型13.12%，在BeyondAIME上提升了11.00%，证明了图引导多模态推理在增强复杂推理稳定性和可靠性方面的有效性。

Conclusion: FIGR通过整合主动视觉思考，能够更好地处理复杂推理问题中的全局结构约束，显著提升了数学推理任务的性能，为多模态推理系统的发展提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 金融服务的复杂性日益增加，需要不仅具备领域知识，还要有强大金融推理和智能体能力的模型。现有模型如BloombergGPT和Baichuan-Finance主要关注知识增强，无法满足对推理和智能体能力的需求。

Method: 提出通用的多阶段训练范式：1) 金融语料上的持续预训练(CPT)巩固知识基础；2) 细粒度后训练管道，包括金融SFT、金融推理RL、金融智能体RL，最后是与真实业务场景对齐的通用RL。

Result: QianfanHuijin在多个权威金融基准测试中表现优异。消融研究证实，针对性的推理RL和智能体RL阶段在各自能力方面带来显著提升。

Conclusion: 该细粒度、渐进式的后训练方法验证了研究动机，有望成为各种工业增强型LLM的主流范式。
...
TL;DR: 提出基于人类略读策略的自监督对比学习框架，通过随机掩码文档段落并使用NLI对比目标来增强长文档表示，在法学和生物医学文本上实现精度和效率的双重提升。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在词句级任务表现良好，但在法律、医学等领域的长文档表示上仍有困难。稀疏注意力机制能处理长输入但资源消耗大且难以捕捉完整文档上下文，分层Transformer效率更高但无法清晰解释文档不同部分的关系。人类通过略读重点段落理解整体内容，受此启发提出新方法。

Method: 提出自监督对比学习框架：随机掩码文档段落，使用基于自然语言推理(NLI)的对比目标，将掩码段落与相关部分对齐，同时与不相关部分保持距离，模拟人类信息整合过程。

Result: 在法学和生物医学文本上的实验证实，该方法在准确性和计算效率方面均取得显著提升。

Conclusion: 受人类略读策略启发的自监督对比学习框架能有效增强长文档表示，在保持计算效率的同时获得更丰富的语义表示，为法律和医学等领域的长文档处理提供了新思路。
...
TL;DR: 比较了低资源语言文本摘要的多种方法，包括零样本LLM提示、微调mT5模型、数据增强、多语言迁移和翻译管道方法，发现微调mT5基线在多数指标上优于其他方法
...
Method: 1) 零样本提示不同规模的LLM；2) 微调mT5等较小模型，结合三种数据增强方法；3) 多语言迁移；4) LLM翻译管道方法（源语言→英语→摘要→回译）；使用五种不同指标进行评估

Result: 1) 相似参数规模的LLM间存在性能差异；2) 多语言微调的mT5基线在大多数指标上优于包括零样本LLM在内的其他方法；3) LLM作为评估器在低资源语言上可能不太可靠

Conclusion: 对于低资源语言文本摘要，微调较小模型（如mT5）比零样本LLM方法更有效，且LLM评估在低资源语言上存在可靠性问题，需要更稳健的评估方法
...
TL;DR: 开发了一个用于雅思写作备考的智能评分与反馈平台，通过设计研究循环从规则方法升级到Transformer模型，实现了有效的自动化评分和适应性反馈。


<details>
  <summary>Details</summary>
Motivation: 传统雅思备考方法缺乏针对雅思写作评分标准的个性化反馈，需要开发能够提供定制化指导和自动化评分的平台来弥补这一不足。

Method: 采用设计研究(DBR)方法进行迭代开发：早期循环使用基于规则的方法，第4循环升级为DistilBERT Transformer模型加回归头，第5循环实现适应性反馈。平台架构将对话指导与写作界面分离以减少认知负荷。

Result: 规则方法存在中段分数压缩、低准确率和负R²值的问题。DistilBERT模型显著改善评分性能（MAE=0.66，正R²值）。适应性反馈使分数平均提高0.060分带（p=0.011，Cohen's d=0.504），但效果因修订策略而异。

Conclusion: 自动化反馈最适合作为人工教学的补充，保守的表面修正比激进的结构干预更可靠。挑战在于评估高分作文，未来需要真实考生纵向研究和官方考官验证。
...
TL;DR: 该论文提出了语音转录本的段落分割任务，建立了首个基准数据集，开发了约束解码方法，并展示了紧凑模型在段落和章节联合预测上的优异性能。


<details>
  <summary>Details</summary>
Motivation: 自动语音转录本通常以非结构化的单词流形式呈现，这降低了可读性和再利用价值。论文旨在填补语音处理和文本分割交叉领域的三个关键空白：缺乏专门针对语音领域的段落分割基准、缺乏保留原始转录本的评估方法、以及缺乏高效的分割模型。

Method: 1. 建立两个基准数据集：TEDPara（人工标注的TED演讲）和YTSegPara（带合成标签的YouTube视频）；2. 提出约束解码方法，让大语言模型在保留原始转录本的同时插入段落分隔符；3. 开发紧凑模型MiniSeg，并通过层次化扩展实现章节和段落的联合预测。

Result: 1. 创建了首个专门针对语音领域的段落分割基准；2. 约束解码方法实现了忠实、句子对齐的评估；3. MiniSeg模型在准确率上达到最先进水平，且计算成本极低；4. 层次化扩展能够有效联合预测章节和段落。

Conclusion: 该研究的资源和方将段落分割确立为语音处理中一个标准化、实用的任务，填补了语音转录本结构化处理的重要空白，为语音内容的可读性和再利用提供了有效解决方案。
...
TL;DR: HaluNet是一个轻量级可训练的神经框架，通过结合语义嵌入与概率置信度和分布不确定性，集成多粒度标记级不确定性，用于LLM幻觉检测。


<details>
  <summary>Details</summary>
Motivation: LLM在问答中常产生幻觉（事实错误或捏造内容），现有方法通常只关注单一类型的不确定性，忽视了不同不确定性来源之间的互补性，特别是标记级概率不确定性和内部语义表示不确定性之间的互补关系。

Method: 提出HaluNet框架，集成多粒度标记级不确定性，结合语义嵌入与概率置信度和分布不确定性。采用多分支架构自适应融合模型已知信息与其输出中表达的不确定性，实现高效的单次幻觉检测。

Result: 在SQuAD、TriviaQA和Natural Questions上的实验表明，HaluNet具有强大的检测性能和良好的计算效率，无论是否访问上下文都能有效工作，突显了其在LLM问答系统中实时幻觉检测的潜力。

Conclusion: HaluNet通过有效整合互补的不确定性信号，为LLM幻觉检测提供了轻量级、可扩展的解决方案，特别适合实时应用场景。
...
TL;DR: KCL是韩国规范法律基准，用于评估语言模型的法律推理能力，独立于领域知识，包含选择题和开放式问题两个部分


<details>
  <summary>Details</summary>
Motivation: 需要评估语言模型的法律推理能力，同时将推理能力与参数化知识分离，提供更准确的评估框架

Method: 构建包含283道选择题（1103个对齐先例）的KCL-MCQA和169道开放式问题（550个对齐先例，2739个实例级评分标准）的KCL-Essay，提供问题级支持先例

Result: 对30多个模型的系统评估显示存在较大差距，特别是在KCL-Essay上，推理专用模型始终优于通用模型

Conclusion: KCL基准能够有效评估语言模型的法律推理能力，揭示了当前模型的局限性，并提供了完整的评估资源
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型依赖长链思维推理解决复杂任务，但这种方法经常效率低下，导致高延迟（过多token生成）或不稳定推理（浅层思考与过度思考交替）。

Method: CREST包含两个组件：(1)离线校准步骤识别认知注意力头并推导头特定的引导向量；(2)推理时过程旋转隐藏表示以抑制这些向量方向的分量，自适应抑制非生产性推理行为。

Result: 在多样化的推理基准测试和模型中，CREST将准确率提高高达17.5%，同时减少37.6%的token使用量。
...
TL;DR: R-Debater是一个基于论证记忆的智能体框架，用于生成多轮辩论。它通过检索案例证据和先前辩论动作，结合角色化智能体生成连贯话语，在ORCHID数据集上相比强LLM基线获得更高评分。


<details>
  <summary>Details</summary>
Motivation: 现有辩论系统在跨轮次保持立场一致性、回应对手以及用证据支持主张方面存在不足。受修辞学和记忆研究启发，需要构建一个能够回忆和调整先前论证以维持辩论连贯性的系统。

Method: R-Debater整合了辩论知识库（用于检索案例证据和先前辩论动作）和基于角色的智能体（用于跨轮次生成连贯话语）。系统在ORCHID辩论数据集上评估，构建了包含1,000个条目的检索语料库和32个辩论的测试集，涵盖七个领域。

Result: 在两项任务评估中：1）下一话语生成任务，使用InspireScore（主观、逻辑和事实性）评估；2）对抗性多轮模拟，使用Debatrix（论证、来源、语言和整体）评估。R-Debater相比强LLM基线获得更高的单轮和多轮评分。20位经验丰富的辩论者参与的人工评估进一步证实了系统的一致性和证据使用能力。

Conclusion: 将检索基础与结构化规划相结合能够产生更忠实、立场对齐且跨轮次连贯的辩论。R-Debater展示了论证记忆在生成一致性辩论中的价值，为辩论系统的开发提供了新方向。
...
TL;DR: 提出MUSIC数据增强策略，通过多轮对话对比训练提升多轮奖励模型性能，在保持单轮基准性能的同时提升多轮对话评估质量
...
Method: 提出MUSIC（多步指令对比）无监督数据增强策略，合成跨多轮对话的对比对话对，基于Skywork偏好数据集训练Gemma-2-9B-Instruct多轮奖励模型

Result: MUSIC增强的奖励模型在高级专有LLM法官的多轮对话判断中表现优于基线方法，且不影响标准单轮奖励模型基准性能

Conclusion: 多轮对比对于构建稳健的多轮奖励模型至关重要，MUSIC策略有效提升了多轮对话评估质量，为可扩展的多轮对话评估提供了实用解决方案
...
TL;DR: BIOME-Bench：首个用于评估LLM在多组学分析中生物分子交互推理和通路机制阐明能力的标准化基准，揭示现有模型在细粒度关系识别和稳健通路解释方面存在显著不足


<details>
  <summary>Details</summary>
Motivation: 当前多组学研究依赖通路富集分析，但传统通路资源存在注释滞后、功能冗余、对分子状态和干预敏感性有限等结构限制。虽然已有研究探索使用LLM改进通路富集解释，但缺乏标准化的端到端多组学通路机制阐明基准，导致评估局限于小规模手动注释数据集或临时案例研究，阻碍了可重复进展。

Method: 通过严格的四阶段工作流程构建BIOME-Bench基准，用于评估LLM在多组学分析中的两个核心能力：1) 生物分子交互推理；2) 端到端多组学通路机制阐明。开发了针对这两个任务的评估协议，并在多个当代强模型上进行全面实验。

Result: 实验结果表明，现有模型在多组学分析中仍存在显著缺陷：难以可靠地区分细粒度的生物分子关系类型，也无法生成忠实、稳健的通路级机制解释。

Conclusion: BIOME-Bench为评估LLM在多组学分析中的能力提供了首个标准化基准，揭示了当前模型的局限性，为未来改进LLM在多组学通路机制阐明方面的性能提供了重要基准和方向。
...
TL;DR: 该论文对大型语言模型在推理任务中的计算效率进行测试时计算感知评估，发现混合专家架构在性能与效率间取得良好平衡，并识别出推理计算存在饱和点。


<details>
  <summary>Details</summary>
Motivation: 当前文献往往忽视生成长推理序列带来的显著计算负担，而工业应用中的模型选择不仅取决于原始准确率，还受资源约束和推理成本影响。需要评估模型在推理任务中的计算效率。

Method: 对当代和早期开源LLMs进行测试时计算感知评估，在数学和推理密集型基准上绘制帕累托前沿，追踪帕累托效率随时间的变化轨迹，并分析推理计算饱和点。

Result: 混合专家架构在评估设置中展现出性能与效率的良好平衡；推导出单位计算精度增益的涌现趋势；发现推理计算存在饱和点，超过特定阈值后精度增益递减。
...
TL;DR: 介绍一门NLP伦理课程及其教学方法，通过主动学习、互动活动和"以教促学"方式，培养学生在NLP中的伦理意识和批判性思维


<details>
  <summary>Details</summary>
Motivation: 随着NLP系统日益普及，将伦理考量融入NLP教育变得至关重要。然而，课程开发面临挑战：领域快速演变（来自学术界和工业界），以及需要培养超越传统技术训练的批判性思维

Method: 基于主动学习的教学方法，包括互动课程、实践活动和"以教促学"方法。课程经过四年在不同机构、教育水平和跨学科背景下的优化调整

Result: 课程产生了许多可复用产品，包括教学材料和学生为不同受众制作的实际教育产品。课程已在多个教育环境中成功实施
...
TL;DR: 论文提出了一种基于因果干预和跨语言不变性的机制解释验证方法，通过"三角测量"标准来筛选真正的电路机制


<details>
  <summary>Details</summary>
Motivation: 多语言模型虽然在整体性能上表现良好，但在不同语言、文字和文化间的行为往往不可预测。现有机制解释方法缺乏因果验证标准，无法区分真正的机制和虚假的相关性。

Method: 提出"三角测量"验证标准，要求同时满足必要性（消融电路会破坏目标行为）、充分性（修补激活能传递行为）和不变性（这些效应在保持语义的变体家族中保持方向和幅度稳定）。通过自动电路发现生成候选子图，然后用三角测量标准接受或拒绝这些候选。

Result: 三角测量提供了一个可证伪的机制解释标准，能够过滤掉那些在单一环境测试中通过但在跨语言不变性测试中失败的虚假电路。

Conclusion: 该研究为多语言模型的机制解释建立了因果验证框架，将因果抽象与实用可解释性议程联系起来，为跨模型家族、语言对和任务的比较实验提供了协议。
...
<details>
  <summary>Details</summary>
Motivation: 当前世界面临生态、意义和语言三重危机，这些危机正汇聚成"元危机"。大型AI技术正在加速所有这些危机，而语言工程师们却继续坚持不可持续的可扩展性叙事，为富豪和腐败者提供关键人才，仿佛整个技术开发过程是价值中立的。迫切需要探索替代方案。

Method: 论文采用批判性分析的方法，审视当前NLP领域的发展模式，提出需要集体智慧来重新设计NLP的未来。虽然没有详细描述具体技术方法，但强调需要从价值导向的角度重新思考技术发展路径。

Result: 论文没有提供具体的实证结果，而是提出了一个批判性分析框架，指出当前AI发展模式的根本问题，并呼吁NLP领域进行范式转变。

Conclusion: 迫切需要探索替代方案，运用集体智慧设计一个以人类繁荣和生命星球为中心的、肯定生命的NLP未来。需要从当前加速危机的技术发展模式转向支持人类福祉和生态健康的价值导向路径。
...
TL;DR: Encyclo-K是一个基于知识陈述的基准测试框架，通过从权威教科书中提取独立知识陈述并在测试时动态组合成评估问题，解决了现有基准测试的三个根本限制。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要存在三个根本限制：1) 容易受到数据污染的影响；2) 仅限于单知识点评估；3) 依赖昂贵的领域专家标注。需要一种新的基准构建方法来克服这些限制。

Method: 从权威教科书中提取独立的知识陈述作为基本单元，在测试时通过随机抽样动态组合这些陈述（每个问题8-10个陈述）构建评估问题。标注者只需验证格式合规性，无需领域专业知识。

Result: 在50多个大语言模型上的实验表明，Encyclo-K具有强大的区分能力。即使是表现最好的OpenAI-GPT-5.1也仅达到62.07%的准确率。推理模型准确率分布在16.04%到62.07%之间，聊天模型分布在9.71%到50.40%之间。

Conclusion: Encyclo-K作为一个可扩展的框架，通过动态评估和多陈述综合理解，有效评估大语言模型对多个细粒度学科知识陈述的综合理解能力，解决了现有基准测试的根本局限性。
...
TL;DR: 提出Manifold-Constrained Hyper-Connections (mHC)框架，通过将Hyper-Connections的残差连接空间投影到特定流形来恢复恒等映射特性，解决训练不稳定性和可扩展性问题。


<details>
  <summary>Details</summary>
Motivation: Hyper-Connections (HC)通过扩展残差流宽度和多样化连接模式获得了性能提升，但这种多样化破坏了残差连接固有的恒等映射特性，导致严重的训练不稳定性、受限的可扩展性以及显著的内存访问开销。

Method: 提出Manifold-Constrained Hyper-Connections (mHC)框架，将HC的残差连接空间投影到特定流形以恢复恒等映射特性，同时结合严格的基础设施优化确保效率。

Result: 实证实验表明mHC在规模化训练中有效，提供了实质性的性能改进和优越的可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有战略对话研究虽然能准确估计信念，但缺乏使用这些信念进行生成的原则性机制。本文旨在填补这一空白，为战略对话中的信念利用提供系统化方法。

Method: 首先形式化了对抗和协调两种核心对话行为，通过概率约束将信念估计转化为生成限制。提出了BEDA框架，包含世界集合、信念估计器和条件生成器三个组件，后者根据推断的信念选择行为并生成一致的语句。

Result: 在三个设置中均取得显著改进：在CKBG（对抗）中，所有骨干模型上成功率至少提高5.0点，GPT-4.1-nano上提高20.6点；在Mutual Friends（合作）中平均提高9.3点；在CaSiNo（谈判）中实现了相对于所有基线的最优交易。

Conclusion: 将信念估计转化为约束条件为可靠的战略对话提供了一个简单、通用的机制，BEDA框架在多种对话场景中均表现出色。
...
TL;DR: ADOPT：一种用于多步LLM管道的自适应依赖感知提示优化框架，通过建模步骤间依赖关系和文本梯度估计来优化多步提示


<details>
  <summary>Details</summary>
Motivation: 多步LLM管道的性能严重依赖于每个步骤的提示，但由于缺少步骤级监督和步骤间依赖关系，联合优化这些提示很困难。现有的端到端提示优化方法在这些条件下表现不佳，往往产生次优或不稳定的更新。

Method: ADOPT框架明确建模每个LLM步骤与最终任务结果之间的依赖关系，实现类似于计算解析导数的精确文本梯度估计。它将文本梯度估计与梯度更新解耦，将多提示优化简化为灵活的单提示优化步骤，并采用基于Shapley值的机制自适应分配优化资源。

Result: 在真实世界数据集和多样化管道结构上的实验表明，ADOPT是有效且鲁棒的，始终优于最先进的提示优化基线方法。

Conclusion: ADOPT通过建模步骤间依赖关系和精确的文本梯度估计，成功解决了多步LLM管道中提示优化的挑战，为复杂任务的多步提示优化提供了有效框架。
...
TL;DR: 基于DeBERTa V3和LSTM的法律文档分类器，使用48个随机选择的短文本块（最多128个token）作为输入，解决了长文档处理问题，并通过Temporal实现可靠部署管道。


<details>
  <summary>Details</summary>
Motivation: 法律文档分类面临两大挑战：专业词汇和文档长度。传统基于Transformer的模型难以处理完整长文档，因为输入完整文档可能不可行、昂贵或缓慢。

Method: 提出基于DeBERTa V3和LSTM的混合模型，输入为48个随机选择的短文本块（每个最多128个token）。采用Temporal作为持久执行解决方案构建部署管道，确保可靠和稳健的处理工作流。

Result: 最佳模型加权F-score达到0.898。在CPU上运行的管道处理100个文件的平均时间为498秒。

Conclusion: 该方法有效解决了长法律文档的分类问题，通过随机采样短文本块和混合架构实现了良好的分类性能，同时通过Temporal确保了部署的可靠性和稳健性。
...
TL;DR: RESTOREx是一个通过LLM生成和人工标注解释来检测社交媒体表情包中抑郁症状的资源，MAMAMemeia是基于认知分析疗法(CAT)的多智能体多方面讨论框架，在抑郁症状检测上比现有方法提升7.55%的macro-F1分数。


<details>
  <summary>Details</summary>
Motivation: 随着表情包从单纯的幽默表达演变为能够自由表达各种情感（包括抑郁情绪）的媒介，社交媒体上使用表情包表达抑郁情绪的现象日益增多，因此需要研究如何识别表情包中表现的抑郁症状。

Method: 1. 引入RESTOREx资源：包含LLM生成和人工标注的解释，用于检测社交媒体表情包中的抑郁症状。2. 提出MAMAMemeia框架：基于临床心理学中的认知分析疗法(CAT)能力，采用协作式多智能体多方面讨论方法。

Result: MAMAMemeia框架在抑郁症状检测任务上比现有最佳方法提升了7.55%的macro-F1分数，在与30多种方法的比较中被确立为新的基准。

Conclusion: 该研究成功开发了用于检测社交媒体表情包中抑郁症状的资源和方法，MAMAMemeia框架在性能上显著超越了现有方法，为心理健康监测提供了新的技术手段。
...
TL;DR: Thought Gestalt模型是一种循环Transformer，在token和句子级"思想"状态两个抽象层次上建模语言，通过记忆先前句子表示来提升语言建模效率和关系方向泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统Transformer语言模型主要依赖表层共现统计，缺乏全局一致的实体和事件潜在表示，导致关系方向脆弱性（如反转诅咒）、上下文错误和数据效率低下。认知科学表明人类理解涉及将语言输入转换为紧凑的事件样表示，这些表示在记忆中持久存在，而逐字形式短暂存在。

Method: 提出Thought Gestalt模型，一种循环Transformer，在token和句子级"思想"状态两个抽象层次上建模语言。模型一次生成一个句子的token，同时交叉关注先前句子表示的记忆。token和句子表示使用相同的模型参数生成，并通过单一目标（下一个token交叉熵）训练：通过保留写入记忆的句子表示的计算图，未来token损失的梯度通过交叉注意力反向传播，优化生成早期句子向量的参数。

Result: 在扩展实验中，TG始终比匹配的GPT-2运行和其他基线提高效率，扩展拟合表明GPT-2需要约5-8%更多数据和约33-42%更多参数才能匹配TG的损失。TG还在父子反转诅咒探测任务上减少了关系方向泛化错误。

Conclusion: Thought Gestalt模型通过引入句子级表示和循环记忆机制，有效解决了传统Transformer在全局一致性表示方面的不足，提高了语言建模的数据效率和关系方向泛化能力，验证了认知科学启发的多层次语言建模方法的有效性。
...
TL;DR: AdaGReS：一种用于token预算RAG的冗余感知上下文选择框架，通过结合查询-块相关性和集合内冗余惩罚的集合级目标优化，实现自适应校准和理论保证。


<details>
  <summary>Details</summary>
Motivation: 传统top-k检索在RAG中经常返回冗余或近似重复的文本块，这会浪费token预算并降低下游生成质量。现有方法缺乏对冗余性的系统控制，且需要手动调整相关性与冗余性的权衡参数。

Method: 提出AdaGReS框架，包含：1) 集合级目标函数，结合查询-块相关性和集合内冗余惩罚；2) 在token预算约束下使用边际增益进行贪心选择；3) 封闭式实例自适应校准机制，自动调整相关性与冗余性权衡参数，无需手动调参；4) 理论分析证明目标函数在实用嵌入相似性条件下具有ε-近似子模性，为贪心选择提供近似最优性保证。

Result: 在开放域问答（Natural Questions）和高冗余生物医学（药物）语料库上的实验表明，AdaGReS在冗余控制和上下文质量方面持续改进，转化为更好的端到端答案质量和跨设置的鲁棒性。

Conclusion: AdaGReS通过系统处理冗余性和自适应参数校准，显著提升了token预算RAG的上下文选择质量，具有理论保证和实际应用价值。
...
TL;DR: 论文提出DDFT评估框架，用于测量语言模型在语义压缩和对抗性干扰下的认知鲁棒性，发现模型规模与鲁棒性无关，而错误检测能力是关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型评估（如MMLU、TruthfulQA）仅测量理想条件下的知识，无法评估模型在现实压力下的认知鲁棒性。需要开发能区分模型知识缺失与验证机制崩溃的评估方法。

Method: 提出DDFT（Drill-Down and Fabricate Test）协议，包含语义压缩和对抗性制造两个测试维度。建立两系统认知模型：语义系统生成流畅文本，认知验证器验证事实准确性。评估9个前沿模型在8个知识领域、5个压缩级别下的表现（共1800次轮级评估）。

Result: 认知鲁棒性与传统设计范式正交：参数数量（r=0.083, p=0.832）和架构类型（r=0.153, p=0.695）均不显著预测鲁棒性。错误检测能力强烈预测整体鲁棒性（rho=-0.817, p=0.007）。旗舰模型虽规模大但表现脆弱，小型模型可达到鲁棒性能。
...
TL;DR: CASCADE是一个自演化的LLM智能体框架，通过持续学习和自我反思实现技能获取，在科学任务中显著提升成功率


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体依赖预定义工具或脆弱的工具生成，限制了其在复杂科学任务中的能力和适应性，需要从"LLM+工具使用"向"LLM+技能获取"转变

Method: CASCADE框架通过两种元技能实现：1) 持续学习（通过网页搜索和代码提取），2) 自我反思（通过内省和知识图谱探索）。框架还包括人机协作和记忆巩固机制

Result: 在SciSkillBench（116个材料科学和化学研究任务）上，CASCADE使用GPT-5达到93.3%的成功率，相比无演化机制的35.4%有显著提升。展示了在计算分析、自主实验室实验和论文选择性复现等实际应用

Conclusion: CASCADE通过积累可执行的技能库，支持跨智能体和科学家共享，推动了可扩展的AI辅助科学研究，代表了从工具使用到技能获取的重要转变
...
TL;DR: McCoy框架结合大语言模型与回答集编程，通过LLM将医学文献转化为ASP代码，结合患者数据进行疾病诊断，提供可解释的预测系统
...
Method: McCoy框架结合大语言模型与回答集编程：1) 使用LLM将医学文献翻译成ASP代码；2) 将生成的ASP代码与患者数据结合；3) 使用ASP求解器处理以获得最终诊断

Result: 初步结果显示McCoy在小规模疾病诊断任务上表现出色，框架结合了两种范式的优势，提供了稳健且可解释的预测系统

Conclusion: McCoy框架通过结合LLM和ASP，克服了符号AI在医疗领域应用的知识库构建障碍，为疾病诊断提供了可解释的预测系统，在小规模任务上展现了良好性能
...
TL;DR: SPARK是一个基于多智能体LLM的个性化搜索框架，通过角色化智能体协作实现动态检索和个性化


<details>
  <summary>Details</summary>
Motivation: 传统个性化搜索系统受限于静态用户画像和单一检索流程，无法有效建模用户动态、多维的信息需求。需要能够捕捉人类信息寻求行为的复杂性、流动性和上下文敏感性的下一代搜索系统。

Method: 提出SPARK框架：1) 定义基于角色、专业知识、任务上下文和领域的人物空间；2) 引入人物协调器动态解释查询并激活最相关的专业智能体；3) 每个智能体执行独立的检索增强生成过程，配备长短期记忆存储和上下文感知推理模块；4) 通过共享内存库、迭代辩论和中继式知识转移等结构化通信协议促进智能体间协作。

Result: 框架提供了关于协调效率、个性化质量和认知负载分布的可测试预测，同时包含用于持续人物细化的自适应学习机制。展示了分布式智能体行为如何通过最小协调规则产生涌现的个性化特性。
...
<details>
  <summary>Details</summary>
Motivation: 传统进化方法缺乏结构化推理，导致从静态LLM向自改进智能体过渡困难，现有方法在高维代码空间中面临早熟收敛和探索效率低的问题。

Method: 提出LoongFlow框架：1) 集成LLM到"计划-执行-总结"认知范式；2) 采用混合进化记忆系统，结合多岛模型、MAP-Elites和自适应玻尔兹曼选择；3) 实例化为通用智能体（算法发现）和ML智能体（管道优化）。

Result: 在AlphaEvolve基准测试和Kaggle竞赛中，LoongFlow比OpenEvolve、ShinkaEvolve等基线方法进化效率提升高达60%，并能发现更优解决方案。

Conclusion: LoongFlow通过将进化搜索转化为推理密集型过程，显著推进了自主科学发现，能够在降低计算开销的同时生成专家级解决方案。
...
TL;DR: 提出一种免训练的基于图的方法来解决ARC-AGI-3基准中的交互式推理任务，通过视觉帧处理与系统化的状态空间探索，在稀疏反馈环境中显著优于前沿LLM


<details>
  <summary>Details</summary>
Motivation: ARC-AGI-3基准包含类游戏任务，要求智能体通过有限交互推断任务机制并适应递增复杂度。当前最先进的LLM无法可靠解决这些任务，需要探索新的方法

Method: 结合基于视觉的帧处理与系统化的状态空间探索，使用图结构表示：将视觉帧分割为有意义的组件，基于视觉显著性优先选择动作，维护探索状态和转移的有向图，通过跟踪访问状态和测试动作来优先选择到达未测试状态-动作对的最短路径

Result: 在ARC-AGI-3预览挑战中，该方法在6个游戏的52个关卡中解决了中位数30个关卡，在私有排行榜上排名第3，显著优于前沿的基于LLM的智能体

Conclusion: 即使没有学习，显式的图结构探索也可以作为交互式推理的强大基线，强调了在稀疏反馈环境中系统化状态跟踪和动作优先级的重要性，这些环境正是当前LLM无法捕捉任务动态的地方
...
TL;DR: RSA是一种风险感知的分步对齐方法，通过嵌套风险度量在策略优化中显式纳入风险意识，解决传统安全对齐方法的风险中性局限。


<details>
  <summary>Details</summary>
Motivation: 现有安全对齐方法（如Safe RLHF和SACPO）通常在风险中性范式下运行，无法充分处理参考策略偏离带来的风险，且对罕见但潜在灾难性有害行为的鲁棒性有限。

Method: 提出风险感知分步对齐（RSA），将安全对齐表述为令牌级风险感知约束策略优化问题，通过基于嵌套风险度量的分步对齐过程求解，实现令牌级策略更新。
...
TL;DR: 该研究系统分析了JEPA-WM类世界模型，通过实验确定了最优架构、训练目标和规划算法组合，提出的模型在导航和操作任务上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 解决AI中长期存在的挑战：开发能够解决广泛物理任务并在新任务和环境中泛化的智能体。当前基于世界模型和规划的方法中，在表示空间进行规划的方法（JEPA-WM类）承诺通过抽象无关细节实现更高效的规划，但缺乏对其技术选择的系统分析。

Method: 将JEPA-WM类模型作为研究对象，通过综合实验分析关键组件：模型架构、训练目标和规划算法。使用模拟环境和真实机器人数据进行实验，研究这些组件如何影响规划成功率。最终结合发现提出优化模型。

Result: 提出的模型在导航和操作任务上优于两个已建立的基线模型DINO-WM和V-JEPA-2-AC。研究确定了JEPA-WM类方法中最优的技术选择组合。

Conclusion: 通过系统分析JEPA-WM类世界模型的关键组件，找到了该家族中最优的方法组合，提出的模型在多种任务上表现出更好的性能，为基于表示空间的规划方法提供了实证指导。
...
### [139] [Thinking on Maps: How Foundation Model Agents Explore, Remember, and Reason Map Environments](https://arxiv.org/abs/2512.24504)
*Zhiwei Wei,Yuxing Liu,Hua Liao,Wenjia Xu*

Main category: cs.AI

TL;DR: 提出了一个交互式评估框架，用于分析基础模型智能体在符号地图环境中的探索、记忆和推理能力，揭示了探索策略、记忆表示和推理方案在空间理解中的不同功能作用。


<details>
  <summary>Details</summary>
Motivation: 现有对基础模型空间能力的评估主要依赖静态地图输入或文本查询，忽视了空间理解的交互性和经验驱动特性。需要开发一个交互式评估框架来深入分析智能体如何在动态探索中理解和推理空间环境。

Method: 提出了一个交互式评估框架，让智能体在部分可观察的网格地图中增量探索（包含道路、交叉口和兴趣点），每步只接收局部观察。使用六种空间任务评估空间理解能力，系统性地变化探索策略、记忆表示和推理方案，并在多个基础模型上进行测试。
...
Conclusion: 地图空间理解的改进需要针对空间表示和推理的专门机制，而不仅仅是模型缩放。探索、记忆和推理组件在空间理解中扮演不同功能角色，结构化记忆表示对空间推理尤为关键。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究大多使用相同的数据集评估大语言模型的数学推理能力，这限制了研究结果的普适性，可能无法完全捕捉数学任务中的多样化挑战。本研究旨在通过分析LLMs在代表性不足的数学竞赛问题上的表现，更全面地了解其数学推理能力。

Method: 研究使用密苏里大学数学竞赛中的微积分、解析几何和离散数学问题，对三种主流LLMs（GPT-4o-mini、Gemini-2.0-Flash、DeepSeek-V3）进行测试。将模型的回答与已知正确答案进行比较以确定准确性，并分析模型的推理过程以探索不同问题类型和模型间的错误模式。

Result: DeepSeek-V3在微积分、解析几何和离散数学三个类别中表现最佳，无论是推理过程还是最终答案正确率都最好。所有三种LLMs在几何问题上表现均显著较弱。DeepSeek-V3的主要错误源于计算和逻辑错误；GPT-4o-mini经常出现逻辑和方法相关的错误；Gemini则倾向于推理不完整和仓促得出结论。

Conclusion: 在代表性不足的数学竞赛数据集上评估LLMs可以提供更深入的见解，揭示其独特的错误模式，并突显结构化推理方面的持续挑战，特别是在几何领域。这表明需要更全面的评估方法来充分理解LLMs的数学推理能力。
...
TL;DR: 该论文提出了一种两阶段方法，通过将空间推理分解为原子构建块及其组合，在大型语言模型中提升空间推理能力。首先通过监督微调学习基本空间变换，然后冻结模型并训练轻量级LoRA适配器来学习多步规划策略。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型具有强大的通用语言能力，但在结构化环境中的空间变换和多步规划方面仍然存在困难。空间推理在导航和规划等应用中日益重要，需要解决LLMs在空间物理理解和多步决策方面的局限性。

Method: 采用两阶段方法：第一阶段对基本空间变换（如旋转、平移、缩放）进行监督微调，使模型具备基础空间物理知识；第二阶段冻结该物理感知模型，在GRPO框架内训练轻量级LoRA适配器，以学习在基于谜题的环境中组合这些构建块进行多步规划的闭环节策略。为此合成了ASCII艺术数据集并构建了相应的ASCII强化学习环境。

Result: 该方法在动态环境（具有显式状态更新）和静态环境（模型必须依赖其内部状态）下均一致优于基线模型，包括通用骨干模型、物理感知模型和端到端RL模型。此外，与从头开始的端到端强化学习相比，该方法收敛更快且训练更稳定。注意力模式分析表明微调确实带来了空间理解的有意义改进。

Conclusion: 通过将空间推理分解为原子构建块及其组合的两阶段方法，能够有效提升LLMs在结构化环境中的空间推理能力。该方法不仅性能优越，而且训练更稳定高效，为LLMs的空间推理应用提供了实用框架。
...
TL;DR: 提出递归语言模型（RLMs）作为处理超长提示的推理策略，通过程序化检查、分解和递归调用处理超出模型上下文窗口的输入


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型处理任意长提示的问题，突破现有模型上下文窗口的限制，提升长上下文任务的处理能力

Method: 提出递归语言模型（RLMs）推理策略，将长提示视为外部环境，允许LLM程序化检查、分解和递归调用自身处理提示片段

Result: RLMs成功处理超出模型上下文窗口两个数量级的输入，在四个多样化长上下文任务中显著优于基础LLM和常见长上下文支架，同时查询成本相当或更低

Conclusion: RLMs提供了一种有效处理超长提示的推理时扩展方法，在保持成本效益的同时显著提升长上下文任务性能
...
Method: 采用去中心化部分可观测马尔可夫决策过程建模协作，使用集中训练分散执行策略，提出组相对策略优化算法，结合全局信号和平衡任务质量、速度、协调成本的简化联合奖励
...
TL;DR: 提出面向群体审议的多智能体对话模型，通过三层角色架构（生成、验证、整合）解决单一LLM在复杂推理任务中的局限性，显著提升多跳推理准确性和一致性。


<details>
  <summary>Details</summary>
Motivation: 单一大型语言模型在复杂推理任务中存在局限性，需要更有效的多智能体协作框架来提升推理准确性和逻辑一致性。
...
TL;DR: 提出多模态跨域混合融合模型，通过双重解耦框架分离模态不变/特定特征和域不变/特定表示，结合跨域混合融合策略增强多样性，提升未见工况下的故障诊断性能。


<details>
  <summary>Details</summary>
Motivation: 现有故障诊断方法在未见工况下性能显著下降，域自适应方法依赖目标域样本，且大多依赖单模态信号，忽略了多模态信息的互补性对提升模型泛化能力的作用。

Method: 提出多模态跨域混合融合模型：1) 双重解耦框架分离模态不变/特定特征和域不变/特定表示；2) 跨域混合融合策略随机混合跨域模态信息增强多样性；3) 三模态融合机制自适应集成多模态异构信息。

Result: 在感应电机故障诊断实验中，无论是未见恒定工况还是时变工况下，所提方法均优于先进方法，消融研究验证了各组件和多模态融合的有效性。

Conclusion: 该方法通过双重解耦和跨域混合融合，有效提升了多模态故障诊断在未见工况下的泛化能力，为实际工业应用提供了更可靠的解决方案。
...
TL;DR: 本文提出了一种显式的、可解释的家庭物品摆放偏好模型，包含四个可解释维度：空间实用性、习惯便利性、语义连贯性和常识适当性，并通过问卷验证和MCTS规划器展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前基于人类演示的机器人家庭物品重排系统虽然有效，但使用的潜在偏好模型缺乏可解释性，无法清晰揭示指导人类决策的因素。需要开发一个显式的、可解释的物品摆放偏好模型来改进机器人规划。

Method: 1) 提出四个可解释的偏好维度：空间实用性、习惯便利性、语义连贯性、常识适当性；2) 设计并验证自报告问卷，通过63名参与者的在线研究确认这些维度的心理区分度和解释力；3) 将参与者得出的偏好整合到蒙特卡洛树搜索(MCTS)规划器中，用于生成物品摆放方案。

Result: 1) 问卷研究证实了四个偏好维度的心理区分度和在两个场景（厨房和客厅）中的解释力；2) 基于参与者偏好指导的MCTS规划器能够生成合理的物品摆放方案，与参与者生成的方案高度一致。

Conclusion: 本文贡献了一个紧凑、可解释的物品摆放偏好模型，并展示了如何将其操作化用于机器人规划。该模型为机器人系统提供了比潜在偏好模型更透明、更可解释的决策指导。
...
TL;DR: GenZ提出了一种混合模型，通过可解释的语义特征桥接基础模型与统计建模，解决了基础模型难以捕捉数据集特定模式的问题，在房价预测和电影推荐任务上显著优于纯基础模型方法。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然具备广泛的领域知识，但往往无法捕捉对预测任务至关重要的数据集特定模式。现有方法过度依赖基础模型的领域理解，而忽略了数据集中特有的统计规律。

Method: 提出一种广义EM算法，通过迭代过程发现语义特征描述：基于统计建模误差对比识别出的项目组，而非单纯依赖基础模型的领域知识。方法使用冻结的基础模型基于发现的特征对项目进行分类，将这些判断视为潜在二元特征的噪声观测，这些特征通过学习的统计关系预测实值目标。

Result: 在房价预测任务中，模型使用从多模态列表数据中发现的语义特征实现了12%的中位数相对误差，显著优于依赖LLM通用领域知识的GPT-5基线（38%误差）。在Netflix电影嵌入预测中，模型仅从语义描述就能以0.59余弦相似度预测协同过滤表示，这相当于传统协同过滤需要约4000个用户评分才能达到的性能。

Conclusion: GenZ成功桥接了基础模型与统计建模，通过可解释的语义特征发现了数据集特定模式（如预测本地房地产市场的建筑细节、预测用户偏好的系列电影成员资格），这些模式与模型单独依赖的领域知识存在差异，证明了混合方法的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 养老机构的排班条件因机构而异，需要通过与制定排班的管理者访谈来设计设施特定的约束条件。现有约束提取技术缺乏排除异常约束的机制。

Method: 使用约束模板提取各种组件的组合，如连续工作日的班次模式或员工组合。模板可通过改变关注的天数、员工数量以及提取重点（模式或频率）来提取多种约束。该方法包含排除异常约束的机制。

Result: 实验表明，该方法成功创建了满足所有硬约束的排班表，并通过避免提取异常约束，减少了软约束的违反次数。

Conclusion: 提出的约束模板方法能够有效提取养老机构特定的排班约束，排除异常约束，为约束规划求解器提供可用的约束条件，从而生成符合实际需求的护工排班表。
...
### [149] [Let It Flow: Agentic Crafting on Rock and Roll, Building the ROME Model within an Open Agentic Learning Ecosystem](https://arxiv.org/abs/2512.24873)
...
TL;DR: ALE是一个端到端的智能体学习生态系统，包含ROLL权重优化框架、ROCK沙盒环境管理和iFlow CLI上下文工程工具，并基于此发布了ROME开源智能体模型，在多项基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前开源社区缺乏系统化的智能体开发基础设施，无法有效支持智能体在真实环境中进行多轮交互、行动观察和迭代优化的完整流程，需要构建一个端到端的生态系统来优化智能体LLM的生产流程。

Method: 提出了ALE生态系统，包含三个核心组件：ROLL（权重优化的后训练框架）、ROCK（轨迹生成的沙盒环境管理器）和iFlow CLI（高效上下文工程的智能体框架）。开发了ROME开源智能体，基于超过100万条轨迹训练。引入了数据组合协议来合成复杂行为，以及新颖的策略优化算法IPA（基于交互的策略对齐），该算法在语义交互块而非单个token上分配信用，以改善长时程训练的稳定性。

Result: 在结构化设置中评估ROME，并引入了Terminal Bench Pro基准测试（具有改进的规模和污染控制）。ROME在SWE-bench Verified和Terminal Bench等基准测试中表现出色，证明了ALE基础设施的有效性。

Conclusion: ALE提供了一个系统化的智能体学习生态系统，通过其三个核心组件和创新的IPA算法，成功开发出性能优异的ROME智能体，为开源社区的智能体开发提供了完整的基础设施支持。
...
### [150] [Semi-Automated Data Annotation in Multisensor Datasets for Autonomous Vehicle Testing](https://arxiv.org/abs/2512.24896)
*Andrii Gamalii,Daniel Górniak,Robert Nowak,Bartłomiej Olber,Krystian Radlak,Jakub Winter*

Main category: cs.AI

TL;DR: 开发了一个用于波兰驾驶场景数据集的半自动化标注流水线，采用人机协同方法降低标注成本和时间


<details>
  <summary>Details</summary>
Motivation: DARTS项目需要创建大规模、多模态的波兰驾驶场景数据集，但手动标注异构数据成本高、耗时长

Method: 采用人机协同方法，结合AI与人类专业知识，包括自动生成初始标注、迭代模型重训练、数据匿名化和领域适应技术，核心基于3D目标检测算法

Result: 显著节省标注时间，确保跨不同传感器模态的一致高质量标注，加速了DARTS项目中大型标注数据集的准备

Conclusion: 该解决方案有效支持DARTS项目，加速了波兰自动驾驶研究的技术基础建设，通过标准化格式的大规模标注数据集
...
### [151] [Iterative Deployment Improves Planning Skills in LLMs](https://arxiv.org/abs/2512.24940)
*Augusto B. Corrêa,Yoav Gelberg,Luckeciano C. Melo,Ilia Shumailov,André G. Pereira,Yarin Gal*

Main category: cs.AI

TL;DR: 迭代部署经过用户精心筛选数据微调的LLM能显著改变模型特性，在规划任务中实现技能提升和泛化能力涌现，其机制等效于隐式奖励的强化学习训练。


<details>
  <summary>Details</summary>
Motivation: 研究迭代部署机制对LLM特性的影响，探索用户数据筛选在模型演化中的作用，并揭示这种部署模式与强化学习的理论联系及其对AI安全的影响。

Method: 采用迭代部署框架：每个新模型基于前一代模型部署中用户精心筛选的数据进行微调。在多个规划领域测试该机制，观察模型规划技能的变化。通过理论分析将迭代部署机制与强化学习训练建立联系。

Result: 在规划任务中观察到显著改进：后续模型展现出涌现的泛化能力，能发现比初始模型长得多的规划方案。理论分析表明迭代部署实现了隐式奖励函数的强化学习训练，这种机制可视为显式强化学习的替代训练范式。

Conclusion: 迭代部署通过用户数据筛选实现了隐式强化学习训练，能显著改变模型特性并提升性能。这对AI安全有重要启示：隐式奖励函数可能带来不可预期的模型演化，同时这种机制为模型训练提供了基于数据筛选而非显式奖励的替代方案。
...
### [152] [AMAP Agentic Planning Technical Report](https://arxiv.org/abs/2512.24957)
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂时空任务（如受限兴趣点发现和行程规划）需要专门设计的智能体模型，能够在时空场景中与多种工具交互，同时保持通用语言能力。

Method: 采用三个关键贡献：1）支持10+领域特定工具的稳定工具环境；2）分层数据筛选框架（筛选比例1:10,000）；3）级联训练方案（种子SFT→高确定性SFT→低确定性RL），基于Qwen3-30B-A3B初始化。

Result: STAgent在TravelBench上表现优异，同时在广泛的通用基准测试中保持其通用能力，验证了所提智能体模型的有效性。

Conclusion: STAgent通过专门的工具环境、高质量数据筛选和级联训练方案，成功构建了一个在时空任务上表现优异且保持通用能力的智能体大语言模型。
...
### [153] [A Comprehensive Study of Deep Learning Model Fixing Approaches](https://arxiv.org/abs/2512.23745)
*Hanmo You,Zan Wang,Zishuo Dong,Luanqi Mo,Jianjun Zhao,Junjie Chen*

Main category: cs.LG

TL;DR: 对16种最先进的深度学习模型修复方法进行大规模实证研究，评估其修复效果及对其他关键属性（鲁棒性、公平性、向后兼容性）的影响


<details>
  <summary>Details</summary>
Motivation: 深度学习系统与传统软件一样容易出错，其故障可能给用户带来重大风险。虽然已有多种修复方法被提出，但缺乏对这些方法性能的全面评估，特别是它们对其他关键属性的影响。

Method: 在统一的实验设置下，对16种最先进的深度学习模型修复方法进行大规模实证研究，涵盖模型级、层级和神经元级三类方法。使用多样化的数据集、模型架构和应用领域，评估修复效果及对鲁棒性、公平性、向后兼容性等属性的影响。

Result: 模型级方法在修复效果上优于其他方法；没有单一方法能在实现最佳修复性能的同时提高准确性并保持所有其他属性；学术界应优先研究减轻这些副作用的方法。

Conclusion: 深度学习模型修复方法在修复效果和其他关键属性之间存在权衡，需要进一步研究来缓解副作用。该研究为工业界和学术界提供了重要见解，指明了该领域未来探索的有前景方向。
...
### [154] [A Review of Diffusion-based Simulation-Based Inference: Foundations and Applications in Non-Ideal Data Scenarios](https://arxiv.org/abs/2512.23748)
*Haley Rosso,Talea Mayo*

Main category: cs.LG

TL;DR: 本文综述了基于扩散模型的模拟推断方法，从数学基础到实际应用，重点分析了扩散模型在科学数据非理想条件下的鲁棒性优势。


<details>
  <summary>Details</summary>
Motivation: 针对复杂模拟问题中似然函数难以处理的情况，需要开发不依赖显式似然函数的推断方法。扩散模型作为一种基于分数匹配和反向时间随机动力学的生成模型，为模拟推断提供了灵活框架，特别是在处理科学数据常见的非理想条件时具有优势。

Method: 回顾扩散模型的数学基础（前向加噪、反向时间SDE/ODE、概率流、去噪分数匹配），解释条件分数如何实现无似然后验采样。分析扩散模型在神经后验/似然估计中相对于归一化流的优势与权衡，特别关注其在非理想条件下的鲁棒性方法。

Result: 扩散模型能够有效处理模拟推断中的关键挑战：模型误设（训练模拟数据与现实不匹配）、非结构化或无限维观测、数据缺失等问题。通过Schrödinger桥公式、条件与序列后验采样器、非结构化数据的摊销架构以及推断时先验适应等方法，扩散模型为科学不确定性量化提供了稳健框架。

Conclusion: 基于扩散模型的模拟推断为复杂科学问题提供了强大的无似然推断框架，特别在处理非理想数据条件时表现出鲁棒性。未来研究方向包括面向概率地球物理模型等应用的不确定性量化问题，需要进一步解决迭代采样成本等权衡问题。
...
### [155] [Coordinate Matrix Machine: A Human-level Concept Learning to Classify Very Similar Documents](https://arxiv.org/abs/2512.23749)
...
<details>
  <summary>Details</summary>
Motivation: 人类通常能从单个示例中学习新概念，而机器学习算法需要大量样本。现有"红色AI"趋势依赖大规模预训练和GPU基础设施，能耗高。需要一种能像人类一样识别重要结构特征、实现单样本学习的绿色AI解决方案。

Method: 提出坐标矩阵机(CM²)，这是一种小型专用模型，通过分析文档结构坐标而非语义向量来增强人类智能。模型专注于人类会考虑的结构"重要特征"，实现单样本分类。采用几何和结构智能方法，优化CPU环境运行。

Result: CM²在单样本学习场景中优于传统向量化和复杂深度学习模型。具有高准确性、计算速度快、低延迟、对类别不平衡鲁棒、经济可行等优势。提供可解释的"玻璃盒"模型，支持绿色AI和环境可持续性。

Conclusion: CM²通过专注于文档结构坐标而非语义向量，实现了人类级概念学习，提供了一种高效、可解释、环保的绿色AI解决方案，特别适用于单样本学习和CPU环境。
...
### [156] [Geometric Scaling of Bayesian Inference in LLMs](https://arxiv.org/abs/2512.23752)
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明，在受控"风洞"设置中训练的小型transformer能够实现精确贝叶斯推理，其训练动态产生编码后验结构的几何基板。本研究旨在探究这种几何特征是否在工业级语言模型中持续存在。

Method: 研究分析了Pythia、Phi-2、Llama-3和Mistral系列模型，发现最后一层值表示沿单一主导轴组织。通过针对Pythia-410M模型在上下文学习期间对熵对齐轴进行定向干预，包括移除或扰动该轴，并与随机轴干预进行对比。

Result: 发现值表示沿单一主导轴组织，其位置与预测熵强相关；领域受限提示将该结构坍缩为与合成设置中相同的低维流形。对熵对齐轴的干预会选择性破坏局部不确定性几何，但单层操作不会导致贝叶斯行为的成比例退化。

Conclusion: 现代语言模型保留了支持风洞中贝叶斯推理的几何基板，并沿此基板组织其近似贝叶斯更新。该几何结构是不确定性的特权读出机制，而非单一计算瓶颈。
...
### [157] [Generalized Regularized Evidential Deep Learning Models: Theory and Comprehensive Evaluation](https://arxiv.org/abs/2512.23753)
*Deep Shankar Pandey,Hyomin Choi,Qi Yu*

Main category: cs.LG

TL;DR: 本文分析了证据深度学习（EDL）模型中激活函数导致的梯度消失问题，提出了一种新的激活函数族和正则化方法来解决学习冻结行为。


<details>
  <summary>Details</summary>
Motivation: 基于主观逻辑的证据深度学习模型虽然能有效量化不确定性，但其非负证据约束要求特定的激活函数。这些激活函数的几何特性可能导致激活依赖的学习冻结行为：当样本映射到低证据区域时，梯度变得极小，阻碍模型学习。
...
Conclusion: 通过理论分析和实验验证，提出的通用激活函数族和正则化方法成功解决了证据深度学习中的学习冻结问题，为不同激活机制提供了更一致的证据更新，提升了模型性能。
...
### [158] [HINTS: Extraction of Human Insights from Time-Series Without External Sources](https://arxiv.org/abs/2512.23755)
*Sheo Yon Jhin,Noseong Park*

Main category: cs.LG

TL;DR: HINTS是一个自监督学习框架，无需外部数据，从时间序列残差中内生提取人类因素（决策、情绪、集体心理），通过Friedkin-Johnsen意见动力学模型作为结构归纳偏置建模社会影响、记忆和偏见模式，提升时间序列预测精度。


<details>
  <summary>Details</summary>
Motivation: 人类决策、情绪和集体心理是影响金融经济系统时间动态的复杂因素。现有方法依赖新闻和社交媒体等外部数据源来捕捉这些因素，但会产生高昂的数据依赖成本（财务、计算和实际应用方面）。

Method: 提出HINTS自监督学习框架：1) 从时间序列残差中内生提取潜在人类因素，无需外部数据；2) 使用Friedkin-Johnsen意见动力学模型作为结构归纳偏置，建模演化中的社会影响、记忆和偏见模式；3) 将提取的人类因素作为注意力图集成到最先进的骨干模型中。

Result: 在九个真实世界和基准数据集上的实验结果表明，HINTS能持续提升预测准确性。多个案例研究和消融研究验证了HINTS的可解释性，显示提取的因素与现实世界事件具有强语义对齐，证明了其实用价值。

Conclusion: HINTS提供了一种无需外部数据、成本效益高的方法来内生提取和建模人类因素，不仅提高了时间序列预测性能，还提供了可解释的洞察，展示了在金融经济系统中的实际应用潜力。
...
### [159] [Neural Optimal Design of Experiment for Inverse Problems](https://arxiv.org/abs/2512.23763)
...
<details>
  <summary>Details</summary>
Motivation: 解决逆问题中实验设计优化的计算复杂性和稀疏性约束问题，传统方法需要双层优化和间接稀疏正则化，计算成本高且需要调参

Method: 联合训练神经重建模型和固定预算的连续设计变量（传感器位置、采样时间、测量角度），在单一优化循环中直接优化测量位置而非候选网格加权

Result: 在指数增长基准测试、MNIST图像采样和稀疏视图X射线CT实验中均优于基线方法，重建精度和任务特定性能均有提升

Conclusion: NODE框架通过设计实现稀疏性，无需l1调参，显著降低计算复杂度，为逆问题中的最优实验设计提供了有效解决方案
...
TL;DR: 提出SVBF-LBG子空间聚类算法，用可训练的舒伯特变体最佳拟合原型替代传统子空间均值，在多种数据上提升聚类纯度


<details>
  <summary>Details</summary>
Motivation: 传统子空间聚类使用几何均值或中位数作为代表，但基于主角度距离的Grassmann流形表示可能不是最优的。需要一种能更好捕捉子空间结构且保持数学结构的方法

Method: 引入舒伯特变体最佳拟合(SVBF)作为可训练原型，定义为与每个聚类成员至少在固定方向上相交的子空间。将SVBF集成到Linde-Buzo-Grey(LBG)流程中，形成SVBF-LBG算法

Result: SVBF-LBG在合成数据、图像数据、光谱数据和视频动作数据上均实现了改进的聚类纯度，同时保持了后续分析所需的数学结构

Conclusion: SVBF作为子空间聚类的可训练原型优于传统均值方法，在保持几何结构的同时提升了聚类性能，适用于需要数学结构的下游分析任务
...
### [161] [Safety-Biased Policy Optimisation: Towards Hard-Constrained Reinforcement Learning via Trust Regions](https://arxiv.org/abs/2512.23770)
...
<details>
  <summary>Details</summary>
Motivation: 安全关键领域的强化学习需要在最大化奖励的同时严格遵守安全约束。现有方法（如拉格朗日法和投影法）往往要么无法确保接近零的安全违规，要么在面对硬约束时牺牲奖励性能。
...
Result: 在标准且具有挑战性的Safety Gymnasium任务上的实验表明，SB-TRPO相比最先进方法，始终在安全性和有意义的任务完成度之间取得最佳平衡。理论分析提供了局部安全进展的保证，并在梯度适当对齐时确保奖励改进。

Conclusion: SB-TRPO通过自适应偏向约束满足的信任区域更新，有效解决了硬约束强化学习中的安全与性能平衡问题，在理论和实验上都优于现有方法。
...
### [162] [FineFT: Efficient and Risk-Aware Ensemble Reinforcement Learning for Futures Trading](https://arxiv.org/abs/2512.23773)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法主要针对现货市场，无法直接应用于高杠杆的期货市场，面临两个主要挑战：1）高杠杆放大奖励波动，导致训练随机且难以收敛；2）先前方法缺乏对能力边界的自我认知，在新市场状态下（如黑天鹅事件）面临重大损失风险。

Method: 提出三阶段集成强化学习框架：阶段I，通过集成TD误差选择性更新Q学习器以提高收敛性；阶段II，基于盈利能力筛选Q学习器，并训练VAE识别学习器的能力边界；阶段III，根据训练好的VAE指导，从筛选后的集成策略和保守策略中选择，以保持盈利能力并降低新市场状态下的风险。

Result: 在高频交易环境（5倍杠杆）的加密货币期货实验中，FineFT在6个金融指标上优于12个SOTA基线，风险降低超过40%，同时实现比第二名更优的盈利能力。可视化显示选择性更新机制使不同智能体专注于不同的市场动态，消融研究证实VAE路由有效降低最大回撤，选择性更新改善收敛和性能。

Conclusion: FineFT通过集成强化学习框架成功解决了高杠杆期货交易中的训练不稳定和风险控制问题，实现了稳定的训练和适当的风险管理，在保持盈利能力的同时显著降低风险。
...
### [163] [Improved Bounds for Private and Robust Alignment](https://arxiv.org/abs/2512.23816)
*Wenqian Weng,Yi He,Xingyu Zhou*

Main category: cs.LG

TL;DR: 该论文从理论角度研究语言模型的私有和鲁棒对齐，建立了离线与在线设置中的次优性差距上界，分析了隐私约束和对抗性腐败下的偏好标签对齐问题。


<details>
  <summary>Details</summary>
Motivation: 研究语言模型对齐中的隐私保护和鲁棒性问题，特别是当偏好标签同时受到隐私约束和对抗性腐败影响时，需要建立理论框架来分析这两种干扰的交互作用。

Method: 采用理论分析方法，考虑隐私约束和对抗性腐败两种干扰，分析隐私优先和腐败优先两种交互模式。使用对数损失和平方损失，开发新的均匀收敛保证，研究MLE风格算法在离线与在线设置中的表现。

Result: 1) 纯隐私设置中，对数损失配合MLE风格算法达到接近最优速率；2) 联合隐私-腐败设置中，现有离线算法提供比已知更强的保证；3) 首次提出私有和鲁棒在线对齐结果；4) 为对数损失和平方损失建立了新的均匀收敛保证。

Conclusion: 该研究为语言模型对齐中的隐私和鲁棒性问题提供了系统的理论分析框架，揭示了隐私约束和对抗性腐败的交互影响，并开发了适用于学习和统计领域的通用收敛保证工具。
...
### [164] [Exploiting the Prior of Generative Time Series Imputation](https://arxiv.org/abs/2512.23832)
...
Method: 提出Bridge-TS方法，构建数据到数据生成过程，包含两个核心设计：1）专家先验：使用预训练的transformer模块作为专家提供确定性估计作为先验；2）组合先验：利用多个预训练模型提供不同估计结果，在数据到数据生成过程中组合它们实现组合先验到目标的插补过程

Result: 在ETT、Exchange和Weather等多个基准数据集上的实验表明，Bridge-TS在均方误差和平均绝对误差方面达到了新的插补精度记录

Conclusion: 通过改进先验设计，Bridge-TS展示了在生成式时间序列插补中的优越性，为时间序列插补任务提供了更准确的数据到数据生成框架
...
### [165] [Trellis: Learning to Compress Key-Value Memory in Attention Models](https://arxiv.org/abs/2512.23852)
*Mahdi Karami,Ali Behrouz,Praneeth Kacham,Vahab Mirrokni*

Main category: cs.LG

TL;DR: Trellis是一种具有有界内存的新型Transformer架构，通过动态压缩KV缓存来解决标准Transformer的二次计算复杂性和不断增长的KV缓存问题。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer存在两个主要问题：1) 二次计算复杂度，2) 注意力机制中不断增长的键值(KV)缓存。这些问题限制了模型处理长序列的能力，特别是在内存受限的环境中。

Method: Trellis用固定大小的内存替换标准KV缓存，并训练一个两遍循环压缩机制来将新键值存储到内存中。该方法采用带有遗忘门的在线梯度下降过程，使压缩内存能够递归更新，同时学习在测试时保留来自传入令牌的重要上下文信息。

Result: 在语言建模、常识推理、回忆密集型任务和时间序列上的广泛实验表明，该架构优于强基线。值得注意的是，随着序列长度的增加，其性能提升更加明显，突显了其在长上下文应用中的潜力。

Conclusion: Trellis通过有界内存和动态压缩机制有效解决了Transformer的内存和计算效率问题，为长上下文应用提供了有前景的解决方案。
...
### [166] [Flow Matching Neural Processes](https://arxiv.org/abs/2512.23853)
*Hussen Abu Hamad,Dan Rosenbaum*

Main category: cs.LG

TL;DR: 提出基于流匹配（flow matching）的神经过程模型，通过ODE求解器实现条件分布采样，在精度和运行时间之间提供可控权衡


<details>
  <summary>Details</summary>
Motivation: 现有神经过程模型在条件采样时通常需要辅助条件化方法，实现复杂且效率有限。流匹配在多种数据模态上表现出色，但尚未应用于神经过程框架

Method: 基于流匹配构建神经过程模型，遵循NP训练框架提供任意数据点的条件分布摊销预测。使用ODE求解器进行条件分布采样，无需辅助条件化方法

Result: 在合成1D高斯过程数据、2D图像和真实世界天气数据等多个基准测试中，该模型超越了先前最先进的神经过程方法

Conclusion: 基于流匹配的神经过程模型实现简单，通过ODE求解器实现高效条件采样，在精度和运行时间之间提供可控权衡，在各种基准上表现优异
...
### [167] [Yggdrasil: Bridging Dynamic Speculation and Static Runtime for Latency-Optimal Tree-Based LLM Decoding](https://arxiv.org/abs/2512.23858)
...
TL;DR: Yggdrasil是一个通过上下文感知树形草稿和编译器友好执行实现延迟最优推测解码的系统，相比现有基线实现最高3.98倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有推测解码系统存在动态推测与静态运行时假设不匹配的问题，导致性能不理想，需要设计能够实现延迟最优推测解码的系统。

Method: Yggdrasil采用协同设计方法，引入等增长树结构确保静态图兼容性，使用延迟感知优化目标进行草稿选择，并采用基于阶段的调度减少开销。

Result: Yggdrasil支持未经修改的大语言模型，在多种硬件配置下相比最先进基线实现最高3.98倍的加速。

Conclusion: 通过上下文感知树形草稿和编译器友好执行的协同设计，Yggdrasil能够实现延迟最优的推测解码，显著提升大语言模型推理性能。
...
### [168] [Probing the Limits of Compressive Memory: A Study of Infini-Attention in Small-Scale Pretraining](https://arxiv.org/abs/2512.23862)
...
Method: 采用Infini-attention架构，该架构从过去的文本段构建压缩内存同时保留局部注意力机制。在300M参数的LLaMA模型上进行小规模预训练，并进行实证研究。

Result: Infini-attention模型表现出训练稳定性，在长上下文检索任务中优于基线模型。平衡因子被确定为影响性能的关键因素，但发现随着长序列中内存压缩的重复，检索精度会下降。尽管如此，在16,384个token的上下文长度下，Infini-attention模型仍比基线模型准确率高出31%。

Conclusion: 研究结果表明，在小型语言模型中实现稳健的长上下文能力受益于像Infini-attention这样的架构化内存设计，该架构能有效补偿SLM参数有限的不足。
...
### [169] [Max-Entropy Reinforcement Learning with Flow Matching and A Case Study on LQR](https://arxiv.org/abs/2512.23870)
*Yuyang Zhang,Yang Hu,Bo Dai,Na Li*

Main category: cs.LG

TL;DR: 提出一种基于流的SAC变体算法，使用流模型参数化策略，通过在线流匹配更新策略，在最大熵线性二次调节器问题上验证了学习最优动作分布的能力。


<details>
  <summary>Details</summary>
Motivation: 传统SAC算法中基于能量的策略通常使用简单策略类进行近似以提高效率，但这牺牲了表达能力和鲁棒性。需要一种既能保持表达能力又能高效学习的策略参数化方法。

Method: 提出基于流的SAC算法变体：1) 使用流模型参数化策略；2) 利用瞬时变量变换技术评估流策略；3) 提出在线流匹配变体（ISFM）进行策略更新，仅需从用户指定的采样分布而非未知目标分布中采样；4) 开发ISFM的理论分析框架。

Result: 在最大熵线性二次调节器问题上进行案例研究，证明所提算法能够学习到最优动作分布。理论分析揭示了不同采样分布选择对学习效率的影响。

Conclusion: 基于流的SAC算法通过流模型的丰富表达能力克服了传统SAC中简单策略近似的局限性，ISFM方法实现了仅需采样分布即可进行策略更新的高效学习，在理论分析和实验验证中均表现出色。
...
### [170] [Rethinking Dense Linear Transformations: Stagewise Pairwise Mixing (SPM) for Near-Linear Training in Neural Networks](https://arxiv.org/abs/2512.23905)
*Peter Farag*
...
<details>
  <summary>Details</summary>
Motivation: 密集线性层是现代机器学习模型中计算和参数成本的主要来源，具有二次复杂度且通常与学习表示的组合结构不对齐。

Method: 引入阶段式成对混合器(SPM)，通过稀疏成对混合阶段的组合实现全局线性变换，提供正交保范旋转和通用2×2混合两种参数化，支持精确闭式前向和后向计算。

Result: 概念验证实验显示在结构化学习问题上显著减少实际计算成本并提高准确性，在真实世界基准测试中保持竞争力。

Conclusion: SPM提供计算效率的同时，其阶段式结构引入显式组合归纳偏置，当与任务结构对齐时可约束模型容量并改善泛化性能。
...
### [171] [Interactive Machine Learning: From Theory to Scale](https://arxiv.org/abs/2512.23924)
*Yinglun Zhu*

Main category: cs.LG

TL;DR: 该论文研究交互式机器学习，开发了在噪声数据主动学习、大规模动作空间序列决策和部分反馈模型选择三个维度的新算法原理和基本极限，实现了统计最优且计算高效的算法。


<details>
  <summary>Details</summary>
Motivation: 机器学习虽然取得了显著成功，但许多最有效的方法依赖于大量标记数据或广泛的在线交互。在实践中，获取高质量标签和通过试错做出决策可能成本高昂、耗时或存在风险，特别是在大规模或高风险环境中。因此需要研究交互式机器学习，让学习器主动影响信息收集或行动选择。

Method: 开发新的算法原理并建立交互式学习的基本极限，重点关注三个维度：1) 噪声数据和丰富模型类的主动学习；2) 大规模动作空间的序列决策；3) 部分反馈下的模型选择。方法包括设计计算高效的主动学习算法、上下文老虎机算法以及模型选择成本的理论表征。

Result: 1) 首次实现了无需低噪声假设即可获得指数级标签节省的计算高效主动学习算法；2) 首次开发了保证与动作空间大小无关的高效通用上下文老虎机算法；3) 首次对序列决策中模型选择的基本成本进行了紧密表征。

Conclusion: 该论文通过开发统计最优且计算高效的算法，推进了交互式学习的理论基础，同时为在大规模现实世界环境中部署交互式学习方法提供了原则性指导。
...
### [172] [Improved Balanced Classification with Theoretically Grounded Loss Functions](https://arxiv.org/abs/2512.23947)
*Corinna Cortes,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: 论文研究了类别不平衡下的多类分类问题，提出了两种先进的替代损失函数家族：广义对数调整损失和广义类别感知加权损失，并进行了全面的理论一致性分析。


<details>
  <summary>Details</summary>
Motivation: 平衡损失函数在类别不平衡的多类分类中被广泛采用，通过赋予所有类别同等重要性来促进公平性和避免少数类别被忽略。然而，直接最小化平衡分类损失通常是难以处理的，因此设计有效的替代损失函数成为一个核心问题。

Method: 提出了两种先进的替代损失函数家族：广义对数调整损失函数和广义类别感知加权损失函数。GLA损失将基于类别先验调整logits的Logit-Adjusted损失推广到更广泛的通用交叉熵损失家族；GCA损失函数通过引入类别相关的置信度边界并将标准类别加权损失扩展到通用交叉熵家族。

Result: 理论分析表明：GLA损失是贝叶斯一致的，但仅对完整（无界）假设集具有H一致性，且其H一致性边界与最小类别概率成反比，至少按1/p_min缩放；GCA损失对任何有界或完整假设集都具有H一致性，其边界按1/√p_min缩放，在类别不平衡设置中提供更强的理论保证。实验表明，两种损失函数都显著优于简单的类别加权损失和LA损失。

Conclusion: GCA损失在理论保证方面优于GLA损失，特别是在高度不平衡的设置中；GLA损失在常见基准测试中表现略好，而GCA在高度不平衡设置中略有优势。两种方法都为类别不平衡分类提供了有效的替代损失函数。
...
### [173] [DivQAT: Enhancing Robustness of Quantized Convolutional Neural Networks against Model Extraction Attacks](https://arxiv.org/abs/2512.23948)
...
<details>
  <summary>Details</summary>
Motivation: 量化CNN模型容易受到提取攻击，存在IP盗窃风险。现有防御方法多为训练后添加噪声，计算成本高，且不适用于边缘设备上的量化模型实现。

Method: 提出DivQAT算法，在量化感知训练过程中修改量化过程，将模型提取防御直接集成到训练流程中，而非作为事后补救措施。

Result: 在基准视觉数据集上的实证验证表明，该技术能有效防御模型提取攻击，且不损害模型精度。与传统QAT相比，与其他防御机制结合时效果更佳。

Conclusion: DivQAT是首个通过修改量化过程将提取防御集成到训练中的技术，为量化CNN提供了有效的模型提取攻击防御方案。
...
### [174] [Causify DataFlow: A Framework For High-performance Machine Learning Stream Computing](https://arxiv.org/abs/2512.23977)
*Giacinto Paolo Saggese,Paul Smith*

Main category: cs.LG

TL;DR: DataFlow是一个用于在无限时间序列数据上构建、测试和部署高性能机器学习系统的计算框架，通过基于有向无环图（DAG）的统一执行模型和点时间幂等性，解决了传统批处理到流式生产系统转换中的问题。


<details>
  <summary>Details</summary>
Motivation: 传统数据科学工作流假设有限数据集，从批处理原型迁移到流式生产系统时需要大量重新实现，导致因果关系违规、批处理边界伪影和实时故障难以复现等问题。

Method: 采用基于有向无环图（DAG）的统一执行模型，具有点时间幂等性：任何时间t的输出仅依赖于t之前的固定长度上下文窗口。框架通过自动跟踪所有转换的知识时间来强制执行严格因果关系，支持跨时间和特征维度的灵活分块。

Result: DataFlow确保在批处理模式下开发的模型无需代码更改即可在流式生产中完全一致地执行，消除了未来窥视错误。框架原生集成Python数据科学栈，提供在线学习的fit/predict语义、缓存和增量计算，以及基于DAG调度的自动并行化。

Conclusion: DataFlow框架有效解决了批处理到流式生产系统转换中的关键问题，在金融交易、物联网、欺诈检测和实时分析等多个领域展示了其有效性，通过统一的执行模型确保了系统的一致性和可复现性。
...
### [175] [Assured Autonomy: How Operations Research Powers and Orchestrates Generative AI Systems](https://arxiv.org/abs/2512.23978)
*Tinglong Dai,David Simchi-Levi,Michelle Xiao Wu,Yao Xie*

Main category: cs.LG

TL;DR: 论文提出生成式AI正从对话助手转向自主决策系统，这产生了自主性悖论：自主性越高，越需要形式化结构、显式约束和尾部风险控制。作者基于运筹学开发了保证自主性的概念框架，包含基于流的生成模型和对抗鲁棒性方法。


<details>
  <summary>Details</summary>
Motivation: 生成式AI正在从对话助手转向能够在工作流程中感知、决策和行动的自主系统。这种转变产生了自主性悖论：随着系统获得更大的操作自主权，它们需要更多的形式化结构、显式约束和尾部风险控制。随机生成模型在操作领域可能很脆弱，除非配备可验证的可行性、对分布偏移的鲁棒性以及高后果场景下的压力测试机制。

Method: 开发了一个基于运筹学的保证自主性概念框架，包含两个互补方法：1) 基于流的生成模型，将生成视为由常微分方程描述的确定性传输，支持可审计性、约束感知生成，并与最优传输、鲁棒优化和序列决策控制连接；2) 通过对抗鲁棒性视角制定操作安全性，在不确定性或模糊集内评估决策规则对抗最坏情况扰动的能力，将未建模风险纳入设计。

Result: 该框架阐明了随着自主性增加，运筹学的角色从求解器转变为护栏再到系统架构师，负责控制逻辑、激励协议、监控机制和安全边界。这些元素定义了在安全关键、可靠性敏感的操作领域中保证自主性的研究议程。

Conclusion: 生成式AI向自主系统的转变需要新的保证自主性框架。基于运筹学的方法，特别是基于流的生成模型和对抗鲁棒性视角，为解决自主性悖论提供了理论基础。这标志着运筹学角色从求解器到系统架构师的转变，为安全关键领域的自主系统设计提供了研究路线图。
...
### [176] [Information-Theoretic Quality Metric of Low-Dimensional Embeddings](https://arxiv.org/abs/2512.23981)
*Sebastián Gutiérrez-Bernal,Hector Medel Cobaxin,Abiel Galindo González*

Main category: cs.LG

TL;DR: 该论文提出了一种基于信息论的嵌入质量评估新指标ERPM，通过分析邻域矩阵的奇异值谱和稳定秩来量化信息损失，与现有距离和几何指标形成互补。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入评估指标（如应力、基于排名的邻域准则、局部Procrustes）主要关注距离或局部几何的失真，但未能直接评估高维数据投影到低维空间时的信息保留程度。需要从信息论角度直接量化嵌入过程中的信息损失。

Method: 提出熵秩保持度量（ERPM），这是一种基于邻域矩阵奇异值谱的香农熵和稳定秩的局部度量。ERPM通过比较原始表示与降维投影之间的不确定性变化，提供邻域级指标和全局汇总统计。使用金融时间序列和文献中常见的流形数据集进行验证，并与基于距离的MRRE和基于几何的局部Procrustes进行比较。

Result: 距离基准则与几何和谱度量显示出极低的相关性，而ERPM与局部Procrustes在平均上表现出强相关性，但在局部区域存在显著差异。ERPM能够识别信息损失严重的邻域，特别是在信息敏感应用中。

Conclusion: ERPM通过从信息论角度量化嵌入质量，补充了现有的距离和几何评估指标。它特别适用于识别信息损失严重的邻域，为信息敏感应用（如早期预警指标构建）提供了更全面的嵌入评估框架。
...
### [177] [Tracing the Heart's Pathways: ECG Representation Learning from a Cardiac Conduction Perspective](https://arxiv.org/abs/2512.24002)
*Tan Pan,Yixuan Sun,Chen Jiang,Qiong Gao,Rui Sun,Xingmeng Zhang,Zhenqi Yang,Limei Han,Yixiu Liang,Yuan Cheng,Kaiyu Guo*

Main category: cs.LG

TL;DR: CLEAR-HUG：一个两阶段框架，通过自监督学习捕获心电信号中的细微传导变化，并遵循心电图诊断指南进行疾病诊断


<details>
  <summary>Details</summary>
Motivation: 现有心电图自监督学习方法存在两个关键局限：1）只关注跨导联和心跳的一致模式，忽略了心脏传导过程中固有的心跳差异，而这些细微变化携带独特的生理特征；2）表示学习未遵循心电图诊断指南（从单个心跳到单导联再到导联组合的序列逻辑），这在预训练模型应用于下游任务时常被忽视

Method: 提出CLEAR-HUG两阶段框架：第一阶段是自监督学习模型CLEAR（传导-导联重建器），将每个心跳视为独立实体，使用稀疏注意力机制重建信号而不受其他心跳干扰，捕获特定变化和一般共性；第二阶段是HUG（分层导联统一组头），用于疾病诊断，模拟临床工作流程

Result: 在六个任务上的实验结果显示性能提升6.84%，验证了CLEAR-HUG在增强心脏传导表示和使模式与专家诊断指南对齐方面的有效性

Conclusion: CLEAR-HUG能够有效捕获跨导联的细微心脏传导变化，同时遵循心电图诊断指南，显著提升了心电图表示学习和疾病诊断性能
...
### [178] [How and Why LLMs Generalize: A Fine-Grained Analysis of LLM Reasoning from Cognitive Behaviors to Low-Level Patterns](https://arxiv.org/abs/2512.24063)
*Haoyue Bai,Yiyou Sun,Wenjie Hu,Shi Qiu,Maggie Ziyu Huan,Peiyang Song,Robert Nowak,Dawn Song*

Main category: cs.LG

TL;DR: 该研究通过引入分解推理为原子核心技能的新基准，揭示了SFT和RL调优在LLM泛化行为上的差异：SFT导致能力窄化，而RL能更好地保持推理技能。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要依赖粗粒度准确率指标，无法解释LLM在监督微调(SFT)和强化学习(RL)调优后表现出的不同泛化行为——SFT常导致能力窄化，而RL倾向于保持能力。需要更细粒度的分析框架来理解这种差异背后的原因。

Method: 1. 引入新基准，将推理分解为计算、事实检索、模拟、枚举和诊断等原子核心技能；2. 结合分布差异和参数统计等低层统计模式分析；3. 开发元探测框架，追踪不同训练阶段模型行为；4. 在数学、科学推理和非推理任务上对比SFT和RL的泛化演化。

Result: RL调优模型保持更稳定的行为特征，抵抗推理技能崩溃；SFT模型表现出更尖锐的漂移，过度拟合表面模式。新基准提供了对特定认知能力如何出现、转移和崩溃的更细粒度视图。

Conclusion: 该工作为理解LLM中推理的本质提供了新见解，并指出了设计促进广泛、稳健泛化的训练策略的原则。通过原子技能分解和元探测，能够更精确地分析训练方法对模型泛化能力的影响。
...
### [179] [Multi-Scenario Highway Lane-Change Intention Prediction: A Temporal Physics-Informed Multi-Modal Framework](https://arxiv.org/abs/2512.24075)
*Jiazhao Shi,Ziyu Wang,Yichen Lin,Shoufeng Lu*

Main category: cs.LG

TL;DR: 提出TPI-AI混合框架，融合深度时序表征与物理启发交互特征，通过Bi-LSTM编码轨迹历史，结合运动学、安全性和交互感知特征，使用LightGBM分类器进行三类别换道意图识别，在高速公路和匝道场景数据集上优于基准方法。


<details>
  <summary>Details</summary>
Motivation: 换道意图预测对自动驾驶安全至关重要，但在自然交通中面临挑战：运动学噪声、严重类别不平衡以及异构高速公路场景的泛化能力有限。现有方法在复杂多变的真实交通环境中表现不足。

Method: 提出Temporal Physics-Informed AI (TPI-AI)混合框架：1) 使用两层双向LSTM编码器从多步轨迹历史学习紧凑嵌入；2) 将这些嵌入与运动学特征、安全特征（如车头时距、碰撞时间、安全间隙指标）和交互感知特征拼接；3) 训练LightGBM分类器进行三类别换道意图识别（不换道、左换道、右换道）；4) 采用不平衡感知优化，包括重采样/加权和折叠阈值校准。

Result: 在两个大规模无人机数据集上评估：highD（直线高速公路）和exiD（匝道丰富环境）。TPI-AI在预测时域T=1,2,3秒时均优于独立的LightGBM和Bi-LSTM基线。在highD上获得宏平均F1分数0.9562、0.9124、0.8345；在exiD上获得0.9247、0.8197、0.7605。结果表明物理启发交互特征与学习时序嵌入的结合能实现鲁棒的多场景换道意图预测。

Conclusion: TPI-AI框架通过融合深度时序表征与物理启发交互特征，有效解决了自然交通中换道意图预测的挑战，在异构高速公路场景中表现出优越的泛化能力和鲁棒性，为自动驾驶安全决策提供了可靠的技术方案。
...
### [180] [Autoregressivity in the Latent Space of a GP-VAE Language Model: An Empirical Ablation Study](https://arxiv.org/abs/2512.24102)
*Yves Ruffenach*
...
<details>
  <summary>Details</summary>
Motivation: 本文旨在系统分析GP-VAE模型中潜在自回归的作用。传统语言模型依赖token级别的自回归分解，而作者先前工作提出将序列结构转移到潜在空间，使用因果高斯过程和非自回归解码器。本研究通过消融实验探究潜在自回归的具体贡献。

Method: 采用系统消融研究方法，比较三种模型：(1)完整GP-VAE模型（具有自回归潜在动态），(2)非自回归消融模型（潜在变量独立），(3)标准token级自回归Transformer。在中等规模语料库和短训练上下文条件下进行实验分析。

Result: 实验结果表明，潜在自回归能诱导出显著更符合高斯过程先验的潜在轨迹，并表现出更好的长程稳定性。移除自回归会导致潜在结构退化和长程行为不稳定。潜在自回归作为组织长程结构的有效机制，与token级自回归建模互补。

Conclusion: 潜在自回归在GP-VAE模型中起到关键作用，能有效组织长程结构并保持稳定性。本研究是对表示结构的实证分析而非新架构提案，强调了潜在自回归作为补充token级建模的有效机制。
...
### [181] [Enhancing LLM Planning Capabilities through Intrinsic Self-Critique](https://arxiv.org/abs/2512.24103)
...
TL;DR: 本文提出了一种让LLMs通过自我批评来提升规划性能的方法，在多个规划基准上实现了显著改进，超越了现有基线并创造了新的SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 尽管先前研究对LLMs自我批评方法的有效性持怀疑态度，但本文旨在证明LLMs能够通过内在的自我批评机制显著提升规划性能，而无需依赖外部验证器。

Method: 采用少样本学习技术并逐步扩展到多样本方法作为基础，然后通过迭代的校正和精炼过程进行自我批评。该方法专注于内在的自我改进能力，不依赖特定模型版本。

Result: 在Blocksworld、Logistics和Mini-grid数据集上实现了显著性能提升，超越了强基线准确率，在2024年10月的LLM检查点类别中创造了新的SOTA结果。

Conclusion: 自我批评能够显著提升LLMs的规划性能，该方法具有普适性，不依赖特定模型版本。应用于更复杂的搜索技术和更强大的模型有望获得更好的性能。
...
### [182] [Paired Seed Evaluation: Statistical Reliability for Learning-Based Simulators](https://arxiv.org/abs/2512.24145)
*Udit Sharma*

Main category: cs.LG

TL;DR: 论文提出配对种子评估设计，通过让竞争系统在相同随机种子下评估，利用共享随机性源来减少方差，提高统计效率和可靠性。


<details>
  <summary>Details</summary>
Motivation: 基于学习的模拟器在评估算法、设计选择和干预措施时，由于随机初始化和学习随机性导致结果方差大，而标准的独立评估设计未能充分利用跨替代方案的共享随机性源。

Method: 提出配对种子评估设计，让竞争系统在相同的随机种子下进行评估，从而诱导随机组件的匹配实现，当结果在种子层面正相关时实现严格的方差减少。

Result: 配对种子评估产生更紧的置信区间、更高的统计功效，在固定计算预算下获得有效的样本量增益。实证显示种子级相关性通常大且正，产生数量级的效率提升。
...
### [183] [Early Prediction of Sepsis using Heart Rate Signals and Genetic Optimized LSTM Algorithm](https://arxiv.org/abs/2512.24253)
...
<details>
  <summary>Details</summary>
Motivation: 脓毒症导致高死亡率、发病率和医疗成本，及时预测对早期干预至关重要。现有模型主要针对ICU患者，缺乏非病房环境下的早期检测方法，需要开发适用于可穿戴设备的解决方案。

Method: 开发了四种新颖的机器学习算法，通过分析心率数据预测脓毒症发作。使用遗传算法优化模型架构，平衡性能、计算复杂度和内存需求。初始预测窗口为1小时，后通过迁移学习扩展到4小时。

Result: 研究获得了令人鼓舞的结果，表明这些模型在可穿戴设备上具有实施可行性，能够准确监测心率并预测脓毒症发作，为ICU和病房外的早期检测提供了可能。

Conclusion: 该研究证明了可穿戴技术在脓毒症早期检测中的潜力，特别是在非ICU和病房环境中，为改善患者预后提供了新的技术途径。
...
### [184] [Empower Low-Altitude Economy: A Reliability-Aware Dynamic Weighting Allocation for Multi-modal UAV Beam Prediction](https://arxiv.org/abs/2512.24324)
*Haojin Li,Anbang Zhang,Chen Sun,Chenyuan Feng,Kaiqian Qu,Tony Q. S. Quek,Haijun Zhang*

Main category: cs.LG

TL;DR: SaM2B：一种面向低空无人机通信的可靠性感知动态加权语义感知多模态波束预测框架，通过动态权重分配和跨模态对比学习解决模态可靠性波动和模态失配问题。


<details>
  <summary>Details</summary>
Motivation: 低空经济快速发展，无人机通信需要快速准确的波束预测。现有多模态方法采用固定或经验权重，假设各模态在任何时刻都具有同等可靠性，但实际上不同模态的重要性随无人机运动场景剧烈波动，静态权重会放大退化模态的负面影响，且模态失配和弱对齐进一步削弱跨场景泛化能力。

Method: 提出SaM2B框架：1）利用环境视觉、飞行姿态和地理空间数据等轻量级线索；2）通过可靠性感知动态权重更新，在不同时间点自适应分配各模态贡献；3）采用跨模态对比学习，将与特定波束信息相关的"多源表示波束语义"对齐到共享语义空间，增强判别能力和模态噪声及分布偏移下的鲁棒性。

Result: 在真实世界低空无人机数据集上的实验表明，SaM2B相比基线方法取得了更令人满意的结果。

Conclusion: SaM2B通过可靠性感知动态加权和跨模态语义对齐，有效解决了多模态波束预测中模态可靠性波动和模态失配问题，提升了低空无人机通信的波束预测性能。
...
### [185] [Lifting Vision: Ground to Aerial Localization with Reasoning Guided Planning](https://arxiv.org/abs/2512.24404)
*Soham Pahari,M. Srinivas*

Main category: cs.LG

TL;DR: 提出ViReLoc框架，通过视觉推理进行定位和路径规划，无需依赖文本推理或实时GPS数据


<details>
  <summary>Details</summary>
Motivation: 现有推理系统主要依赖文本信息，限制了在空间任务（如视觉导航和地理定位）中的有效性，需要探索纯视觉推理范式

Method: 提出Geo-Consistent Visual Planning范式，开发ViReLoc框架，通过视觉表示进行规划和定位，学习空间依赖和几何关系，采用强化学习目标优化，结合对比学习和自适应特征交互对齐跨视角

Result: 在多样化导航和定位场景中实验显示，空间推理准确性和跨视角检索性能均有持续改进

Conclusion: 视觉推理是导航和定位的强大补充方法，可在无需实时GPS数据的情况下执行此类任务，提供更安全的导航解决方案
...
### [186] [Efficient Inference for Inverse Reinforcement Learning and Dynamic Discrete Choice Models](https://arxiv.org/abs/2512.24407)
...
Method: 开发半参数去偏IRL框架，将log行为策略作为伪奖励来识别策略价值差异和奖励函数。将目标形式化为行为策略和转移核的光滑泛函，建立路径可微性并推导有效影响函数。构建自动去偏机器学习估计器，允许灵活的非参数估计。

Result: 该框架实现了√n一致性、渐近正态性和半参数效率，将经典DDC模型的推断扩展到非参数奖励和现代机器学习工具，提供了统一的计算可行方法。

Conclusion: 提出的半参数去偏IRL框架统一了IRL和DDC模型的统计推断，既支持灵活的非参数估计，又提供严格的统计保证，为序列决策分析提供了计算可行且统计有效的方法。
...
### [187] [Sparse classification with positive-confidence data in high dimensions](https://arxiv.org/abs/2512.24443)
*The Tien Mai,Mai Anh Nguyen,Trung Nghia Nguyen*

Main category: cs.LG

TL;DR: 提出高维Pconf分类的稀疏惩罚框架，使用凸和非凸惩罚项解决弱监督下的高维学习问题


<details>
  <summary>Details</summary>
Motivation: 高维学习中稀疏正则化在完全监督数据中已建立，但在弱监督设置如Pconf分类中仍探索不足。现有Pconf方法不适合高维场景，需要开发适合高维弱监督的稀疏正则化方法
...
### [188] [Adaptive Learning Guided by Bias-Noise-Alignment Diagnostics](https://arxiv.org/abs/2512.24445)
...
Method: 提出诊断驱动自适应学习框架，将误差演化分解为偏差（捕捉持续漂移）、噪声（捕捉随机变异性）和对齐（捕捉重复方向激励导致超调），从损失或时序差分误差轨迹的轻量统计中在线计算

Result: 该框架为监督优化、演员-评论家强化学习和学习优化器提供了统一控制主干，在标准平滑性假设下建立了有界有效更新和稳定性特性
...
### [189] [Generative forecasting with joint probability models](https://arxiv.org/abs/2512.24446)
*Patrick Wyrod,Ashesh Chattopadhyay,Daniele Venturi*

Main category: cs.LG

TL;DR: 将混沌系统预测重构为完全生成问题：学习滞后系统状态在短时间窗口上的联合概率分布，通过边缘化获得预测，而非传统的条件下一步预测。


<details>
  <summary>Details</summary>
Motivation: 混沌动力系统对初始条件高度敏感且常包含未解析的多尺度过程，使得确定性预测存在根本限制。现有生成模型大多关注条件下一步预测，而非底层动态结构。

Method: 将预测重构为完全生成问题，学习滞后系统状态在短时间窗口上的联合概率分布；提出模型无关的训练和推理框架；引入三种互补的不确定性量化指标（集成方差、短时域自相关、累积Wasserstein漂移）。

Result: 在Lorenz-63系统和Kuramoto-Sivashinsky方程上评估，联合生成模型相比传统条件下一步模型：短期预测技能提升、保持吸引子几何结构、长期统计行为更准确。

Conclusion: 将预测重构为完全生成问题能更好地捕获非线性时间依赖、表示多步轨迹段，并产生与学习到的联合分布一致的预测，同时提供无需真实值的鲁棒性和可靠性评估框架。
...
### [190] [HOLOGRAPH: Active Causal Discovery via Sheaf-Theoretic Alignment of Large Language Model Priors](https://arxiv.org/abs/2512.24478)
*Hyunjun Kim*

Main category: cs.LG

TL;DR: HOLOGRAPH是一个基于层理论的LLM引导因果发现框架，通过将局部因果信念表示为预层截面，将全局因果结构对应为全局截面的存在性，并利用代数潜在投影处理隐藏混杂因子。


<details>
  <summary>Details</summary>
Motivation: 观测数据的因果发现受限于可识别性约束，现有LLM作为先验因果知识的方法缺乏理论基础，需要建立严格的数学框架来整合LLM的因果知识。

Method: 1. 使用层理论将局部因果信念表示为变量子集上预层的截面；2. 将全局因果结构对应为全局截面的存在性；3. 提出代数潜在投影处理隐藏混杂因子；4. 在信念流形上使用自然梯度下降进行优化。

Result: 在合成和真实世界基准测试中，HOLOGRAPH在50-100个变量的因果发现任务上取得竞争性性能。层理论分析显示：恒等、传递和粘合公理在数值精度内满足，但局部性公理在较大图中失效，表明潜在变量投影中存在非局部耦合。

Conclusion: HOLOGRAPH为LLM引导的因果发现提供了严格的数学基础，揭示了因果结构中的拓扑障碍表现为非零层上同调，并发现了潜在变量投影中的非局部耦合现象。
...
### [191] [More Than Bits: Multi-Envelope Double Binary Factorization for Extreme Quantization](https://arxiv.org/abs/2512.24545)
*Yuma Ichikawa,Yoshihiko Fujisawa,Yudai Fujimoto,Akira Sakai,Katsuki Fujisawa*

Main category: cs.LG

TL;DR: 提出MDBF方法改进双二进制分解，通过多包络设计增强幅度表达能力，在保持1位符号基的同时提升量化性能


<details>
  <summary>Details</summary>
Motivation: 双二进制分解(DBF)在极低位量化中具有高效推理优势，但其缩放参数限制过强，所有秩分量共享相同幅度分布，导致性能饱和

Method: 提出多包络DBF(MDBF)：保留共享的1位符号基，但将单一包络替换为秩-l包络；引入闭式初始化和交替优化方法

Result: 在LLaMA和Qwen系列模型上，MDBF在相同比特/权重下相比先前二进制格式显著提升困惑度和零样本准确率

Conclusion: MDBF通过增强幅度表达能力解决了DBF的性能饱和问题，同时保持了部署友好的推理原语，为极低位量化提供了有效方案
...
### [192] [From Perception to Punchline: Empowering VLM with the Art of In-the-wild Meme](https://arxiv.org/abs/2512.24555)
*Xueyan Li,Yingyi Xue,Mengjie Jiang,Qingzi Zhu,Yazhe Niu*

Main category: cs.LG

TL;DR: HUMOR框架通过分层多路径思维链增强视觉语言模型的幽默推理多样性，并结合基于模板分组的成对奖励模型进行群体强化学习，以提升幽默表情包生成质量。


<details>
  <summary>Details</summary>
Motivation: 生成幽默表情包是一个超越直接图像-描述监督的多模态挑战任务，需要微妙地推理视觉内容、上下文线索和主观幽默感。现有方法在视觉感知与幽默笑点创作之间存在差距。

Method: 1. 分层多路径思维链：从模板级意图识别开始，在不同上下文下探索多样推理路径，最后锚定到高质量、上下文特定的路径；2. 基于模板分组的成对奖励模型：在共享相同模板的表情包组内训练奖励模型；3. 群体强化学习优化：利用奖励模型进行群体级强化学习，在信任区域内保证单调改进。

Result: 实验表明HUMOR框架赋予各种视觉语言模型更优的推理多样性、更可靠偏好对齐和更高的整体表情包质量。在高质量路径保持显著概率质量的实用条件下，多路径探索与锚定能维持高期望幽默质量。

Conclusion: 该工作提出了一个通用的开放式、人类对齐多模态生成训练范式，其成功依赖于在连贯输出组内进行对比判断。该方法不仅适用于表情包生成，还可推广到其他需要主观判断的多模态生成任务。
...
### [193] [CPR: Causal Physiological Representation Learning for Robust ECG Analysis under Distribution Shifts](https://arxiv.org/abs/2512.24564)
*Shunbo Jia,Caizhi Liao*

Main category: cs.LG

TL;DR: 论文提出CPR方法，通过因果解缠框架分离心电图中不变病理特征与非因果伪影，显著提升对抗鲁棒性同时保持单次推理效率。


<details>
  <summary>Details</summary>
Motivation: 现有ECG深度学习模型对抗平滑对抗扰动脆弱，传统对抗训练计算成本高，随机平滑等方法推理延迟大，不适用于实时临床监测。作者认为脆弱性源于模型依赖非鲁棒伪相关而非不变病理特征。

Method: 提出因果生理表征学习(CPR)，在因果解缠框架中融入生理结构先验。通过结构因果模型建模ECG生成，执行结构干预严格分离不变病理形态(P-QRS-T复合波)与非因果伪影。

Result: 在PTB-XL数据集上，CPR显著优于标准临床预处理方法。在SAP攻击下，CPR获得0.632 F1分数，比中值平滑(0.541 F1)提升9.1%。CPR匹配随机平滑的认证鲁棒性，同时保持单次推理效率。

Conclusion: CPR在鲁棒性、效率和临床可解释性之间提供了优越的权衡，通过因果解缠分离不变病理特征，有效解决ECG模型对抗脆弱性问题，适用于实时临床监测场景。
...
### [194] [AutoFed: Manual-Free Federated Traffic Prediction via Personalized Prompt](https://arxiv.org/abs/2512.24625)
*Zijian Zhao,Yitong Shang,Sen Li*

Main category: cs.LG

TL;DR: AutoFed：一种无需手动超参数调优的个性化联邦学习框架，用于交通预测任务，通过联邦表征器和客户端对齐适配器实现跨客户端知识共享


<details>
  <summary>Details</summary>
Motivation: 交通预测对智能交通系统至关重要，但存在隐私问题导致数据孤岛。联邦学习虽能保护隐私，但面临非独立同分布问题。个性化联邦学习虽有前景，但现有方法需要大量超参数调优，这在现实场景中难以实现，阻碍了实际部署。

Method: 提出AutoFed框架，受提示学习启发，引入联邦表征器，使用客户端对齐适配器将本地数据提炼为紧凑的全局共享提示矩阵。该提示矩阵条件化个性化预测器，使每个客户端既能从跨客户端知识中受益，又能保持本地特异性，无需手动超参数调优。

Result: 在真实世界数据集上的广泛实验表明，AutoFed在不同场景下始终实现优越性能，验证了其有效性。

Conclusion: AutoFed成功解决了交通预测中个性化联邦学习的超参数调优难题，通过创新的提示学习机制实现了无需手动调优的高性能预测框架，具有实际部署价值。
...
### [195] [Nested Learning: The Illusion of Deep Learning Architectures](https://arxiv.org/abs/2512.24695)
*Ali Behrouz,Meisam Razaviyayn,Peilin Zhong,Vahab Mirrokni*

Main category: cs.LG

TL;DR: 提出嵌套学习（NL）新范式，将机器学习模型表示为嵌套、多级、并行优化问题，通过压缩上下文流实现学习，并开发了表达性优化器、自修改学习模块和连续记忆系统三个核心贡献。


<details>
  <summary>Details</summary>
Motivation: 尽管语言模型取得进展，但现有模型在持续学习/记忆、自我改进和寻找有效解决方案方面存在根本性挑战和未解问题。需要新的学习范式来解决这些限制。

Method: 提出嵌套学习（NL）范式，将模型表示为嵌套、多级、并行的优化问题，每个问题有自己的上下文流。通过NL视角，现有深度学习方法通过压缩自身上下文流来学习，上下文学习在大模型中自然涌现。

Result: 开发了三个核心贡献：1）表达性优化器（如Adam、SGD with Momentum等是关联记忆模块）；2）自修改学习模块（学习如何修改自身的序列模型）；3）连续记忆系统（推广传统长/短期记忆观点）。结合这些组件构建了持续学习模块Hope。

Conclusion: NL为设计更表达性的学习算法提供了哲学框架，通过更多层级实现高阶上下文学习，有望解锁有效的持续学习能力。Hope模块在语言建模、知识整合、少样本泛化、持续学习和长上下文推理任务中显示出有希望的结果。
...
### [196] [Causal Discovery with Mixed Latent Confounding via Precision Decomposition](https://arxiv.org/abs/2512.24696)
*Amir Asiaee,Samhita Pal,James O'quinn,James P. Long*

Main category: cs.LG

TL;DR: DCL-DECOR：一种用于混合潜在混杂下因果发现的模块化方法，通过精度矩阵分解分离全局和局部混杂效应，提高有向边恢复准确性


<details>
  <summary>Details</summary>
Motivation: 现实观测数据常受混合潜在混杂影响：既有广泛影响多个变量的全局混杂因素，也有仅影响少数变量的局部混杂因素。现有方法存在局限：可微DAG学习器可能将全局潜在效应误判为因果边，而潜在变量图模型只能恢复无向结构。需要新方法处理这种混合混杂场景。

Method: 提出DCL-DECOR模块化流程：1）通过精度矩阵分解将观测精度矩阵分解为结构化分量和低秩分量，分离普遍混杂效应；2）结构化分量对应去除普遍混杂后的条件分布，保留因果图和局部混杂引起的局部依赖；3）应用相关噪声DAG学习器到去混杂表示中恢复有向边，同时建模剩余结构化误差相关性；4）简单协调步骤确保弓形自由性。

Result: 提供了混合混杂下可恢复因果目标的识别性结果，表明整体问题可分解为具有模块化保证的已研究子问题。合成实验显示，在变化普遍混杂强度和维度的情况下，相比直接将相关噪声DAG学习应用于混杂数据，DCL-DECOR在有向边恢复方面持续改进。

Conclusion: DCL-DECOR通过精度引导的模块化方法有效处理混合潜在混杂，分离全局和局部混杂效应，为线性高斯系统中受混合混杂影响的因果发现提供了系统解决方案，显著提高了有向因果结构的恢复准确性。
...
### [197] [FPGA Co-Design for Efficient N:M Sparse and Quantized Model Inference](https://arxiv.org/abs/2512.24713)
*Fen-Yu Hsieh,Yun-Chang Teng,Ding-Yong Hong,Jan-Jan Wu*

Main category: cs.LG

TL;DR: 本文提出了一种结合N:M结构化剪枝和4位整数量化的自动化框架，通过软硬件协同设计在FPGA平台上生成加速器，显著降低LLM的内存占用并提升推理效率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在多种语言处理任务上表现出色，但其巨大的计算和内存需求阻碍了在资源受限环境中的部署。需要开发高效的压缩和加速技术来解决这一挑战。

Method: 提出自动化框架，结合权重剪枝和低位量化：1) 应用N:M结构化剪枝和4位整数量化减少内存占用；2) 优化反量化和矩阵乘法操作；3) 在多种硬件平台实现，包括CPU、NVIDIA GPU（密集和2:4稀疏张量核心）以及基于脉动阵列的定制FPGA加速器。

Result: 在4096×4096矩阵上，结合2:4稀疏性和量化，实现了：1) 权重存储减少4倍；2) 矩阵乘法加速1.71倍；3) 端到端延迟相比密集GPU基线降低1.29倍。在LLaMA-7B模型上的扩展分析显示，结构化稀疏性使每个token的吞吐量提升1.36倍。

Conclusion: 细粒度N:M稀疏性与量化的协同作用能够实现高效且可部署的LLM推理。提出的FPGA加速器为支持超出固定2:4硬件约束的更广泛稀疏模式提供了灵活的架构路径。
...
### [198] [From Trial to Deployment: A SEM Analysis of Traveler Adoptions to Fully Operational Autonomous Taxis](https://arxiv.org/abs/2512.24767)
*Yutong Cai,Hua Wang*

Main category: cs.LG

TL;DR: 基于武汉百度Apollo Robotaxi实际运营数据的实证研究，通过结构方程模型识别影响自动驾驶出租车采纳行为的六个潜在心理构念，发现成本敏感性和行为意向是最强正向预测因子。


<details>
  <summary>Details</summary>
Motivation: 现有研究多基于陈述偏好实验和假设场景探讨用户对自动驾驶出租车的接受度，但缺乏基于实际运营服务数据的用户行为研究。本研究旨在填补这一空白，利用实际运营数据探究真实用户行为。

Method: 在武汉（百度Apollo Robotaxi服务运营地）设计包含实际服务属性的现实调查，收集336份有效用户问卷。采用结构方程模型识别六个潜在心理构念：信任与政策支持、成本敏感性、性能、行为意向、生活方式和教育，并分析它们对采纳行为（在十个场景中选择自动驾驶出租车的频率）的影响。

Result: 成本敏感性和行为意向是自动驾驶出租车采纳的最强正向预测因子，其他潜在构念发挥更微妙的作用。模型在多个拟合指数上表现出良好的拟合优度。

Conclusion: 研究结果为支持自动驾驶出租车在实际城市环境中规模化部署的政策制定、票价设计和公众推广策略提供了实证依据。
...
### [199] [DTI-GP: Bayesian operations for drug-target interactions using deep kernel Gaussian processes](https://arxiv.org/abs/2512.24810)
*Bence Bolgár,András Millinghoffer,Péter Antal*

Main category: cs.LG

TL;DR: 提出DTI-GP模型，结合深度核学习与高斯过程，为药物-靶点相互作用预测提供概率信息，支持贝叶斯分类、拒绝、top-K选择和排序等新操作。
...
Method: 提出基于深度核学习的高斯过程架构DTI-GP，包含化学化合物和蛋白质靶点的神经嵌入模块，以及高斯过程模块。通过从预测分布采样估计贝叶斯优先矩阵，用于快速准确的筛选和排序操作。
...
### [200] [Discovering Coordinated Joint Options via Inter-Agent Relative Dynamics](https://arxiv.org/abs/2512.24827)
...
Method: 1) 提出联合状态抽象，压缩状态空间同时保留发现强协调行为所需信息；2) 基于智能体状态同步的归纳偏置，近似最大对齐的费马状态；3) 定义扩展度度量团队级不对齐；4) 使用神经图拉普拉斯估计器推导捕获智能体间状态同步模式的选项。

Result: 在多个多智能体领域场景中评估，相比其他选项发现方法，该方法产生的选项展现出更强的下游协调能力。

Conclusion: 通过联合状态抽象和费马状态近似的方法能够有效发现多智能体协调选项，解决现有方法协调性不足的问题，为多智能体强化学习中的协调行为设计提供了新思路。
...
### [201] [AODDiff: Probabilistic Reconstruction of Aerosol Optical Depth via Diffusion-based Bayesian Inference](https://arxiv.org/abs/2512.24847)
*Linhao Fan,Hongqiang Fang,Jingyang Dai,Yong Jiang,Qixing Zhang*

Main category: cs.LG

TL;DR: AODDiff：基于扩散贝叶斯推断的概率性气溶胶光学厚度重建框架，无需完整训练数据，支持不确定性量化


<details>
  <summary>Details</summary>
Motivation: 当前气溶胶光学厚度（AOD）重建模型受限于完整训练数据的稀缺性和缺乏不确定性量化能力，需要一种能够从自然不完整数据中学习并支持多种重建任务的灵活框架

Method: 提出AODDiff框架：1）采用感知损坏的训练策略从自然不完整数据中学习AOD的时空概率分布作为生成先验；2）使用解耦退火后验采样策略，有效整合异质观测作为约束来引导生成过程

Result: 在再分析数据上的实验验证表明，AODDiff在降尺度和修复任务中均表现出高效性和鲁棒性，特别是在保持高空间光谱保真度方面具有优势，并能通过多次采样实现不确定性量化

Conclusion: AODDiff作为一种生成模型，能够从自然不完整数据中学习时空先验，灵活适应多种重建任务而无需重新训练，并固有地支持不确定性量化，为下游应用提供关键置信度指标
...
### [202] [Characterization of Transfer Using Multi-task Learning Curves](https://arxiv.org/abs/2512.24866)
*András Millinghoffer,Bence Bolgár,Péter Antal*

Main category: cs.LG

TL;DR: 该研究提出使用多任务学习曲线而非梯度更新来量化迁移效应，通过数据扰动而非模型扰动来更基础地刻画迁移现象，并在药物-靶点相互作用数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 迁移效应在固定数据集训练和累积数据归纳推理中均有体现。传统方法通过梯度更新扰动模型来研究迁移，但作者认为通过增加样本扰动数据集能提供更基础的表征。需要更有效的量化方法来捕捉多任务学习中的迁移效应。

Method: 1) 使用多任务学习曲线近似不同样本量下的归纳性能来量化迁移效应；2) 提出类似任务亲和分组的高效多任务学习曲线近似方法；3) 比较统计与计算两种迁移研究方法；4) 在药物-靶点相互作用基准数据集上进行评估。

Result: 1) 学习曲线能更好地捕捉多任务学习效应；2) 多任务扩展能刻画基础模型中的成对和上下文迁移效应；3) 统计方法计算成本显著高于计算方法，但具有更好的统计功效和更广泛的适用性。

Conclusion: 通过数据扰动而非模型扰动来研究迁移效应提供了更基础的表征。多任务学习曲线是量化迁移效应的有效工具，能更好地捕捉多任务学习效应并刻画基础模型中的迁移模式。该方法在计算成本与统计功效之间提供了权衡选择。
...
TL;DR: 训练语言模型进行开放式预测，通过自动化生成预测问题数据集，开发OpenForecaster 8B模型，在准确率、校准和一致性方面匹配更大规模的专有模型


<details>
  <summary>Details</summary>
Motivation: 高风险的决策需要在不确定性下对未来进行推理，需要开发能够进行开放式预测的语言模型，但缺乏大规模的训练数据

Method: 1) 从每日新闻中自动化合成预测问题创建OpenForesight数据集；2) 使用离线新闻语料防止信息泄露；3) 训练Qwen3思维模型；4) 结合检索机制和改进的强化学习奖励函数；5) 在2025年5-8月进行保留测试

Result: OpenForecaster 8B模型在准确率、校准和一致性方面匹配更大规模的专有模型，预测训练带来的校准改进可推广到其他基准测试

Conclusion: 通过自动化数据生成和专门的训练方法，可以开发出高效的预测模型，开源模型、代码和数据将促进语言模型预测研究的广泛可及性
...
### [204] [MSACL: Multi-Step Actor-Critic Learning with Lyapunov Certificates for Exponentially Stabilizing Control](https://arxiv.org/abs/2512.24955)
*Yongwei Zhang,Yuanzhe Xing,Quan Quan,Zhikun She*

Main category: cs.LG

TL;DR: MSACL是一个将指数稳定性理论与最大熵强化学习相结合的新框架，通过多步Lyapunov证书学习实现可证明的稳定性和安全探索，在简单奖励下实现指数稳定性和快速收敛。


<details>
  <summary>Details</summary>
Motivation: 模型无关强化学习在实现可证明稳定性方面存在挑战，特别是在平衡探索与严格安全性方面。现有方法依赖复杂的奖励工程，缺乏理论保证的稳定性。

Method: MSACL整合指数稳定性理论与最大熵强化学习，通过多步Lyapunov证书学习框架。使用离策略多步数据学习满足理论稳定性条件的Lyapunov证书，引入指数稳定性标签(ESL)和λ加权聚合机制平衡多步学习的偏差-方差权衡。策略优化由稳定性感知优势函数指导，确保学习策略促进快速Lyapunov下降。

Result: 在六个基准测试（包括稳定化和非线性跟踪任务）中，MSACL优于最先进的基于Lyapunov的RL算法。在简单奖励下实现指数稳定性和快速收敛，对不确定性表现出显著鲁棒性，并能泛化到未见轨迹。敏感性分析确定多步视野n=20作为跨不同系统的稳健默认值。

Conclusion: MSACL通过将Lyapunov理论与离策略actor-critic框架连接，为可验证安全的学习型控制提供了基础。该框架在简单奖励下实现可证明的指数稳定性，平衡了探索与安全性，具有实际应用潜力。
...
### [205] [Attribution-Guided Distillation of Matryoshka Sparse Autoencoders](https://arxiv.org/abs/2512.24975)
*Cristina P. Martin-Linares,Jonathan P. Ling*

Main category: cs.LG

TL;DR: DMSAEs通过蒸馏循环训练紧凑的核心特征集，实现跨训练和稀疏度的一致性特征复用，提升稀疏自编码器的可解释性和可重用性。


<details>
  <summary>Details</summary>
Motivation: 传统稀疏自编码器学习的特征存在冗余性，且在不同训练运行和稀疏度下特征不一致，导致解释难以迁移和复用。

Method: 提出蒸馏Matryoshka稀疏自编码器：迭代蒸馏循环训练共享核心的Matryoshka SAE，使用梯度X激活度量特征对下一词损失的贡献，保留最小特征子集解释固定比例的归因，仅核心编码器权重跨循环传递。

Result: 在Gemma-2-2B第12层残差流激活上，7个蒸馏循环得到197个重复选择的蒸馏核心特征，使用该核心训练显著改善了多个SAEBench指标。

Conclusion: DMSAEs证明了可以通过蒸馏获得一致的潜在特征集，这些特征能够跨稀疏度迁移，提升了稀疏自编码器的特征可重用性和解释稳定性。
...
### [206] [Efficiently Estimating Data Efficiency for Language Model Fine-tuning](https://arxiv.org/abs/2512.24991)
*Gyung Hyun Je,Colin Raffel*

Main category: cs.LG

TL;DR: 提出一种基于梯度余弦相似度的任务数据效率预测方法，无需增量标注即可估计达到目标性能所需的微调样本量


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在零样本场景下表现有限，通常需要微调来提升性能，但任务的数据效率（达到目标性能所需的微调样本量）难以预测，导致昂贵的增量标注和重新训练循环

Method: 引入量化任务数据效率的具体指标，提出使用低置信度样本的梯度余弦相似度来基于少量标注样本预测数据效率

Result: 在30个专门任务上验证，数据效率预测误差为8.6%，通常能为每个任务消除数百个不必要的标注

Conclusion: 提出的梯度余弦相似度方法能有效预测任务数据效率，显著减少标注成本，实验代码已在GitHub开源
...
### [207] [Diffusion Language Models are Provably Optimal Parallel Samplers](https://arxiv.org/abs/2512.25014)
*Haozhe Jiang,Nika Haghtalab,Lijie Chen*

Main category: cs.LG

TL;DR: DLMs通过并行采样和CoT能模拟任何并行算法，但需要remasking或revision来优化空间复杂度，revision使DLMs表达能力更强。


<details>
  <summary>Details</summary>
Motivation: 为扩散语言模型作为自回归模型替代方案提供理论依据，证明其通过并行采样实现更快推理的潜力，并探索如何优化空间复杂度。

Method: 形式化并行采样模型，证明DLMs结合多项式长度CoT可模拟任何并行采样算法；引入remasking（将未掩码token转为掩码）和revision（将未掩码token转为其他未掩码token）来优化空间复杂度。

Result: DLMs结合CoT能以最优顺序步骤数模拟并行算法；加入remasking或revision后能实现最优空间复杂度；证明revision或remasking的DLMs比没有这些功能的DLMs表达能力更强。
...
### [208] [Generative Classifiers Avoid Shortcut Solutions](https://arxiv.org/abs/2512.25034)
*Alexander C. Li,Ananya Kumar,Deepak Pathak*

Main category: cs.LG

TL;DR: 生成式分类器通过建模所有特征（包括核心和伪相关特征）而非主要依赖伪相关特征，能够避免判别式分类器在分布偏移下的失败问题，在图像和文本分布偏移基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 判别式分类器在分类时经常学习到只在训练分布内有效的"捷径"特征（伪相关特征），这些特征在分布发生轻微偏移时就会失效。这种失败模式源于对与标签伪相关特征的过度依赖。

Method: 使用基于类条件生成模型（如扩散模型和自回归模型）的生成式分类器。这些分类器建模所有特征（包括核心和伪相关特征），而非主要依赖伪相关特征。方法简单，无需专门的数据增强、强正则化、额外超参数或对特定伪相关特征的先验知识。

Result: 扩散模型和自回归生成式分类器在五个标准图像和文本分布偏移基准上达到最先进性能，并在医疗和卫星数据集等实际应用中减少了伪相关特征的影响。通过高斯玩具设置的分析，阐明了生成式分类器的归纳偏置以及决定其优于判别式分类器的数据特性。

Conclusion: 生成式分类器通过建模所有特征而非主要依赖伪相关特征，能够有效避免判别式分类器在分布偏移下的失败问题，提供了一种简单而强大的替代方案，在多种分布偏移场景中表现出色。
...
### [209] [On the geometry and topology of representations: the manifolds of modular addition](https://arxiv.org/abs/2512.25060)
...
<details>
  <summary>Details</summary>
Motivation: 反驳Clock和Pizza解释所声称的不同架构设计会产生不同模块加法电路的观点，揭示这些架构实际上实现的是相同算法。

Method: 超越单个神经元和权重的解释，识别所有对应每个学习表示的神经元，将其作为整体实体研究；将学习表示视为流形，运用拓扑学工具分析；通过统计方法分析数百个电路的学习表示。

Result: 均匀注意力和可训练注意力架构通过拓扑和几何等价的表示实现相同算法；从常见深度学习范式中自然产生的学习模块加法电路具有相似性。

Conclusion: Clock和Pizza解释所声称的架构差异导致不同电路的观点不成立，两种架构实际上实现了相同的算法，具有等价的表示形式。
